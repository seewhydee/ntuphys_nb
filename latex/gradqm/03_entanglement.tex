...
Suppose two quantum particles are labeled $A$ and $B$.  By the
postulates of quantum mechanics, the possible states of each particle
can be described by vectors in Hilbert spaces.  Let us denote the
Hilbert spaces as $\mathscr{H}_A$ and $\mathscr{H}_B$, respectively;
since the nature of the $A$ and $B$ particles may be different, the
two spaces may be distinct mathematical objects (e.g., they can have
different dimensions).

The combined system should be describable using some Hilbert space
$\mathscr{H}$.  We assert that this space is constructed from
$\mathscr{H}_A$ and $\mathscr{H}_B$ by a so-called \textbf{tensor
  product}, denoted by
\begin{equation}
  \mathscr{H} = \mathscr{H}_A\otimes \mathscr{H}_B.
\end{equation}
The tensor product is a mathematical procedure for combining two
vector spaces to form a larger vector space.  It is most easily
understood by referring to explicit basis vectors.  Let
$\mathscr{H}_A$ have a basis $\{|\mu_1\rangle, |\mu_2\rangle,
|\mu_3\rangle, \dots\}$, and let $\mathscr{H}_B$ have a basis
$\{|\nu_1\rangle, |\nu_2\rangle, |\nu_3\rangle, \dots\}$.  Then
consider the set of all \textit{pairwise combinations} of basis
vectors for $\mathscr{H}_A$ and $\mathscr{H}_B$, denoted by
\begin{equation}
  \big\{\;\,|\mu_i\rangle\otimes|\nu_j\rangle \;\;\big| \;\;\textrm{all}\;i,\, j\;\,\big\}.
  \label{ABbasis}
\end{equation}
This notation has an implicit ordering: the $A$ vector must be on the
left of the $\otimes$ symbol, and the $B$ vector on the right.  Then
$\mathscr{H}_A \otimes \mathscr{H}_B$ is defined as the space spanned
by \eqref{ABbasis}: i.e., for any $|\psi\rangle \in \mathscr{H}_A
\otimes \mathscr{H}_B$,
\begin{equation}
  |\psi\rangle = \sum_{ij} \, \psi_{ij}\; |\mu_i\rangle \otimes |\nu_j\rangle
  \label{psispan}
\end{equation}
for some vector components $\psi_{ij} \in \mathbb{C}$.

According to the quantum postulates, we also need
$\mathscr{H}_A\otimes \mathscr{H}_B$ to have a well-defined inner
product.  In Dirac bra-ket notation, let us denote the dual of
$|\mu_i\rangle\otimes|\nu_j\rangle$ by $\langle\mu_i| \otimes
\langle\nu_j|$.  Note that the ordering in the $\otimes$ symbol
remains unchanged: $A$ is on the left, and $B$ on the right.  Thus,
the dual of Eq.~\eqref{psispan} is
\begin{equation}
  \langle \psi| = \sum_{ij} \, \psi_{ij}^*\; \langle\mu_i| \otimes \langle\nu_j|.
\end{equation}
We devise the inner product operation for $\mathscr{H}_A\otimes
\mathscr{H}_B$ so that it obeys the rule
\begin{equation}
...
performed ``slot by slot'', making use of the inner products of
$\mathscr{H}_A$ and $\mathscr{H}_B$.  Then, for any $|\psi\rangle,
|\psi'\rangle \in \mathscr{H}_A \otimes \mathscr{H}_B$, with vector
components $\psi_{ij}$ and $\psi'_{ij}$ respectively,
\begin{align}
  \langle \psi| \psi'\rangle &=
  \left(\sum_{ij} \, \psi_{ij}^*\; \langle\mu_i| \otimes \langle\nu_j|\right)
  \left(\sum_{i'j'} \, \psi_{ij}'\; |\mu_{i'}\rangle \otimes |\nu_{j'}\rangle\right) \\
  &=\sum_{iji'j'} \, \psi_{ij}^*\, \psi_{ij}'
...
\end{align}
From this discussion, we can also see that the dimension of the tensor
product space is
\begin{equation}
  \mathrm{dim}(\mathscr{H}_A\otimes
  \mathscr{H}_B) = \mathrm{dim}(\mathscr{H}_A) \,
  \mathrm{dim}(\mathscr{H}_B).
\end{equation}
For a more mathematically precise derivation of the tensor product
concept, see \hyperref[ex:innerprod]{Exercise 1}.

Let us illustrate the tensor product with a simple example.  Let
$\mathscr{H}_A$ and $\mathscr{H}_B$ both be 2D Hilbert spaces
