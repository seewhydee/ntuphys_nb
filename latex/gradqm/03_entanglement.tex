...
EPR ``paradox'' is regarded as an important tool for thinking about
...
\begin{figure}[h]
  \centering\includegraphics[width=0.7\textwidth]{epr}
\end{figure}
...
record Alice's and Bob's axis choices and measurement results, like
this:
...
After all the rounds are finished, the experimental records are
brought together and examined.  Assuming the results are consistent
with quantum theory, whenever the experimentalists \textit{happened}
to choose the same axis, they must have obtained opposite results.
For instance, in the above example, both experimentalists chose axis
$S_3$ during Round 4; Alice got $-$, and Bob got $+$.
...
S_2 = +\hbar/2, \, S_3 = -\hbar/2$, which we can denote more concisely
as $+, +, -$.  To be consistent with quantum theory's predictions, for
every spin axis, the two particles' hidden variables must have
opposite values.  Hence, there are $8$ possible hidden-variable
states, which we denote as follows:
\begin{align*}
  \begin{aligned}
    \hvbox[+++]{---} \;\;\;
    \hvbox[++-]{--+} \;\;\;
  ...
  \end{aligned}
\end{align*}
For instance, $\hvbox[++-]{--+}$ indicates that particle $A$ has $S_1
= S_2 = +\hbar/2$ and $S_3 = -\hbar/2$, while particle $B$ has $S_1 =
S_2 = -\hbar/2$ and $S_3 = +\hbar/2$.  The hidden-variable theory
postulates that the quantum two-particle state \eqref{bellsinglet} is
actually some kind of statistical distribution, i.e., a set of
probabilities for each of the 8 hidden-variable states:
\begin{equation*}
...
probability $< 1/3$, violating Bell's inequality, that would prove
that quantum theory is fundamentally inconsistent with a broad class
of hidden-variable theories.  It does not matter how complicated the
inner workings of the hidden-variable theory are; so long as it meets
the modest assumptions presented above (e.g., there are local
deterministic hidden variables for each measurement, the
hidden-variable states are associated with probabilities summing to
one, etc.), such a theory would be considered disproven.

To finish up, we must find a set of spin operators $\{S_1, S_2, S_3\}$
such that quantum theory predicts a violation of Bell's inequality.
One way to accomlpish this is to align $S_1$ with the $z$ axis, and
place $S_2$ and $S_3$ in the $x$-$z$ plane at $120^\circ$ ($2\pi/3$
radians) from $S_1$, as shown below:
...
their measurement results for the rounds in which they chose different
axes.  (These rounds are not needed for the secret key, so the
measurement results are okay to reveal.)  From this information, each
of them can verify whether Bell's inequality is violated; if so, that
is a guarantee that no eavesdropping has taken place.  For details,
refer to Ref.~[\ref{cite:ekert}].
