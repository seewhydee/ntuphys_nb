...
rather than posited, from a quantum description of both the system
being measured \textit{and} the system doing the measurement.

We can use a toy model to illustrate this idea [\ref{cite:albrecht}].
Consider a spin-$1/2$ particle, whose Hilbert space is
$\mathscr{H}_S$.  Suppose we have an apparatus that measures $S_z$ on
that particle.  This ``apparatus'' has a vast number of degrees of
freedom, for it comprises a macroscopic device that performs the $S_z$
measurement (and, optionally, an experimentalist who looks at the
result, etc.).  Therefore, its Hilbert space must have a
mind-bogglingly large dimension:
\begin{equation}
...
is ``measuring'' $S_z$, if it simply evolves according to a
Hamiltonian of the form
\begin{equation}
  \hat{H} = \hat{S}_z \otimes \hat{V}.
  \label{V_albrecht}
\end{equation}
The demonstration only uses unitary time evolution via the
Schr\"odinger equation, \textit{without} the measurement postulate.
Also, it turns out that we will not need to make any special choices
for $|\Psi\rangle$ in Eq.~\eqref{psi0_albrecht}, or $\hat{V}$ in
Eq.~\eqref{V_albrecht}, so we leave these unspecified for now.

As the Hamiltonian \eqref{V_albrecht} is time-independent, the state
after some time $t$ is
\begin{equation}
  |\psi(t)\rangle = e^{-i\hat{H}t/\hbar}\, |\psi(0)\rangle.
  \label{mwi_psit}
\end{equation}
To calculate this, observe how the Hamiltonian \eqref{V_albrecht} acts
on each term of the initial state defined in
Eq.~\eqref{psi0_albrecht}:
\begin{align}
...
Based on Eq.~\eqref{psit}, we would like to interpret
$|\Psi_\uparrow(t)\rangle$ and $|\Psi_\downarrow(t)\rangle$ as the
states of the apparatus corresponding to each possible measurement
result.  For example, $|\Psi_\uparrow(t)\rangle$ may represent a state
in which the device has a message saying ``$S_z = + \hbar/2$'' on its
screen, and the experimentalist has read the message and concluded
that the particle is spin-up.  For this interpretation to hold,
$|\Psi_\uparrow(t)\rangle$ and $|\Psi_\downarrow(t)\rangle$ must
describe utterly different microscopic states of the apparatus,
meaning they ought to be orthogonal.  In fact, this condition is easy
to satisfy.  We will show (and will later verify numerically) that so
long as $d_A \gg 1$,
\begin{equation}
  \langle\Psi_\uparrow(t) | \Psi_\downarrow(t)\rangle \rightarrow 0
\end{equation}
after a relatively short time $t$ has elapsed.
...
For this demonstration, it turns out that we do not need to make any
special choices for the apparatus state $|\Psi\rangle$ in
Eq.~\eqref{psi0_albrecht}, or the operator $\hat{V}$ in
Eq.~\eqref{V_albrecht}.  Let us therefore choose them in a generic
way.  In some basis for $\mathscr{H}_A$, let $|\Psi\rangle$ be a
random complex vector, with each component having its real and
imaginary parts drawn from the normal distribution $N(0,1)$:
\begin{equation}
...
theory of random matrices [\ref{cite:edelman}].)
