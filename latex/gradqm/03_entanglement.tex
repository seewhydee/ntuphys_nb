...
use $\hat{\rho}_A$ to directly calculate the probabilities for
measurements performed on $A$.  As a corollary,
\begin{equation}
...
We can illustrate this using a toy model formulated by Albrecht
...
state
\begin{equation}
  |\psi(0)\rangle = \Big(a_+ |\!\uparrow\rangle + a_- |\!\downarrow\rangle\Big) \otimes |\Psi\rangle,
\end{equation}
...
is $\hat{S}_z$, the result necessarily has the form
\begin{align}
  \begin{aligned}|\psi(t)\rangle &= \hat{U}(t)\Big(a_+ |\!\uparrow\rangle + a_- |\!\downarrow\rangle\Big) \otimes |\Psi\rangle \\ &= a_+ |\!\uparrow\rangle \otimes |\Psi_+(t)\rangle \;+\; a_- |\!\downarrow\rangle \otimes |\Psi_-(t)\rangle. \end{aligned}
\end{align}
Here, $|\Psi_+(t)\rangle$ and $|\Psi_-(t)\rangle$ are apparatus states
that are ``paired up'' with the $|\!\uparrow\rangle$ and
$|\!\downarrow\rangle$ states of the spin-$1/2$ subsystem.  At $t=0$,
both $|\Psi_+(t)\rangle$ and $|\Psi_-(t)\rangle$ are equal to
...
sufficiently complicated, we can guess (and we will verify
numerically) that the two state vectors rotate into completely
different parts of the state space, so that
\begin{equation}
