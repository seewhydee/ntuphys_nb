...
would provide a method to instantaenously transmit messages from Alice
to Bob, violating the theory of relativity!
...
\begin{itemize}
\item
  Suppose Alice measures along axis $n$, and Bob measures along $n'$.
  Regardless of $n$ and $n'$, both Alice and Bob have probabilities
  $\{1/2, 1/2\}$ to obtain the measurement results $+\hbar/2$ and
  $-\hbar/2$.
...
outcomes with 50/50 probability.  He cannot extract any information
...
\textit{spirit} of relativity, since the state collapse is nonlocal
and instantaneous.  Even if it is unobservable, there is an ingredient
of the theory (the quantum state) that changes faster than light can
...
\end{equation}
where $\hat{U}$ is a unitary rotation matrix.  Derive expressions for
the probabilities of the two possible outcomes, $P(A_+)$ and $P(A_-)$,
as well as the post-collapse states.

\item For each of the two outcomes for (a), find the conditional
  probabilities for the outcomes of a subsequent $S_z$ measurement on
