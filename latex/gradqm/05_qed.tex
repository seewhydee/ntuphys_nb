...
Having established the classical Hamiltonian, we can promote it into a
quantum Hamiltonian in the usual way, replacing $\mathbf{r}$ by the
position operator $\hat{\mathbf{r}}$ and $\mathbf{p}$ by the momentum
operator $\hat{\mathbf{p}}$:
\begin{framed}
  \begin{equation}
...
dealing with the operator combination $\hat{p}_j A_j(\mathbf{r},t)$,
the gradient in the momentum operator applies to everything on its
right, i.e., both $A_j$ and the wavefunction.  By the product rule,
\begin{equation}
