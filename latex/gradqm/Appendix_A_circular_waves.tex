...
sinuoidal; the decay is caused by the $1/\sqrt{r}$ in
Eq.~\eqref{Jasymptote}, which upholds the conservation of flux in the
$r$ direction (see Chapter 1, Sec.~1.4).  For small $kr$, the value of
$H_m^+(kr)$ diverges (specifically, its imaginary part, which is
$Y_m$).  This is also consistent with flux conservation, because as
the ``area'' (circumference) vanishes with $r$, the flux density must
diverge to keep the total flux independent of $r$.
...
waves, and we can use $J_m$ and $Y_m$ to make circular waves that act
as standing waves in the $r$ direction.  According to
Eq.~\eqref{JYHrelation}, each Bessel wave can be interpreted as a
superposition of an incoming and outgoing Hankel wave, just as a 1D
standing wave is a superposition of two counter-propagating plane
waves via Eq.~\eqref{sincos}.
...
wavefronts are spread over an area $4\pi r^2$, so $|\Psi_{\ell
  m}(r)|^2 \cdot 4\pi r^2$ is $r$-independent).
...
may be evanescent.)  We aim to derive the $S$ matrix, as defined in
Eq.~\eqref{srelation}.

First, note that the scattering potential is isotropic, so angular
momentum is conserved.  Thus, it is helpful to consider a total
wavefunction (incident plus scattered wavefunction) with definite
angular momentum:
\begin{equation}
...
Eqs.~\eqref{psirdecomp}--\eqref{srelation}, we see that the scattering
matrix must have the form
\begin{equation}
  S_{\ell m, \ell' m'} \,=\, s_{\ell} \; \delta_{\ell \ell'} \, \delta_{mm'}.
\end{equation}
In other words, $S$ is diagonal, with the diagonal entries dependent
only on $\ell$ (not $m$).  The exterior solution \eqref{aell} can thus
be rewritten as
\begin{equation}
