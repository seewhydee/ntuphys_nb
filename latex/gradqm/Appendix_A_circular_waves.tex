...
wavefunction must be finite.  Hence, we cannot use the $y_\ell$ and
$h_\ell^\pm$ functions, which diverge at $r = 0$.  We are forced to
use spherical Bessel functions of the first kind, which are
non-divergent at $r = 0$:
\begin{equation}
...
Plugging this result into the scattering amplitude formulat
