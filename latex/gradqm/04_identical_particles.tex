...
Recall that any permutation $p$ can be generated by starting with the
trivial sequence $\{1, 2, \dots, N\}$, and applying a series of
transpositions (i.e., choosing two slots in the sequence, and swapping
their contents).  The parity $s(p)$ is defined to be $+1$ if $p$ is
constructed from an even number of transpositions, and $-1$ for an odd
number of transpositions.  Even though different series of
transpositions can be used to generate a given $p$, they all give the
same $s(p)$---a result known as the ``parity theorem''.

Let's look at a couple of concrete examples.

\begin{framed}
\noindent
\textit{Example}---Consider the case of $N=2$.  The sequence $\{1,2\}$
...
To verify that Eq.~\eqref{Nfermions} is antisymmetric, let us apply
$\hat{P}_{ij}$ (for any $i \ne j$) to any term in the sum,
corresponding to some permutation $p$:
\begin{align}
  \begin{aligned}
  ...
  \label{fermionswap}
\end{align}
In this tensor product state, the single-particle states for $p(i)$
and $p(j)$ have exchanged places, so this corresponds to a different
permutation, denoted by $p'$.  Since $p'$ differs from $p$ by a single
transposition (of slots $i$ and $j$), they have opposite parities:
$s(p') = - s(p)$.  Hence, \eqref{fermionswap} is the negative of
another term in the sum in Eq.~\eqref{Nfermions}.  Conversely,
applying $\hat{P}_{ij}$ to the $p'$ term yields the negative of the
$p$ term.  We thus conclude that
\begin{equation}
  \hat{P}_{ij} |\phi_1,\dots,\phi_N\rangle = - |\phi_1,\dots,\phi_N\rangle.
\end{equation}

Note also that in Eq.~\eqref{Nfermions}, if $|\phi_1\rangle, \dots,
|\phi_N\rangle$ are not all distinct single-particle states, the terms
in the sum over $p$ cancel out pairwise.  In that case, the
construction yields the zero vector, which is not a valid quantum
state.  This is consistent with the Pauli exclusion principle.
