...
$c_m^\pm \in \mathbb{C}$.  Each circular wave is a solution to the 2D
Helmholtz equation with angular momentum quantum number $m \in
\mathbb{Z}$.  Its $r$-dependence is given by $H_m^\pm$, a
\textbf{Hankel function} of the ``first kind'' ($+$) or ``second
kind'' ($-$).  For real inputs, these are complex conjugates of each
other
\begin{equation}
  \left[H^+_m(kr)\right]^* = H^-_m(kr) \quad\mathrm{for}\;\,kr \in \mathbb{R}.
\end{equation}
Some Hankel functions of the first kind are plotted below:
...
\end{equation}
Thus, the $\pm$ index specifies whether the circular wave is
\textbf{outgoing} ($+$) or \textbf{incoming} ($-$) along the radial
direction.
...
More discussion about these circular and spherical waves can be found
in Appendix A.
...
The free-particle Green's function is the operator
$\hat{G}_0=\big(E-\hat{H}_0\big)^{-1}$.  We will call its
representation in the position basis,
$\langle\mathbf{r}|\hat{G}_0|\mathbf{r}'\rangle$, the
\textbf{propagator}.  As shown in the preceding section, when the Born
...
The propagator is a solution for
\begin{align}
...
\begin{equation}
  ...
  \label{kdef}
\end{equation}
denotes the wavenumber corresponding to the energy of the incident
particle.  Up to a factor of $2m/\hbar^2$, the propagator is the
Green's function for the $d$-dimensional Helmholtz equation (see
Section~\ref{sec:2d3d_scattering}).  Note that the $\nabla^2$ acts
upon the $\mathbf{r}$ coordinates, ignoring $\mathbf{r}'$.

Appendix A discusses the process of solving Eq.~\eqref{propagatoreq}.
Here, we will skip over most of the details of the solution, which
involves using the momentum basis, imposing an appropriate choice of
coordinates (e.g., spherical coordinates in 3D), and contour
integration.  Along the way, it turns out we have to modify the
definition of the Green's function slightly, in order to get a
sensible set of solutions.  This involves replacing
Eq.~\eqref{G0unregulated} with
\begin{framed}
  \begin{equation}
...
\end{framed}
\noindent
With this tweak, the propagators for 1D, 2D, and 3D are
\begin{framed}
  \begin{equation}
...
part to $k$, thus ensuring that outgoing waves of the form
$\sim\exp(ikr)$ are integrable.)
