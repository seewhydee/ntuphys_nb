...
\textbf{scattering amplitude}.  This complex function describes how
the particle is scattered in various directions, and is the
fundamental quantity of interest in scattering experiments.
...
\noindent
Let the flux of incident classical particles be the same as the value
obtained from the incident wavefunction \eqref{psiin},
\begin{equation}
  J_{i} = \frac{\hbar k}{m} \;|\Psi_i|^2.
  \label{incidentflux}
\end{equation}
Note that this has units of $[x^{1-d}t^{-1}]$ (i.e., number per unit
time per unit ``area'' in $d$ dimensions).  We assume the scatterer
only interacts with the particles striking it directly, and the rate
...
for the $d$-dimensional Helmholtz equation.  Note that the $\nabla^2$ acts on
