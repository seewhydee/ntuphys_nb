...
annihilation operators $\hat{c}_\mu$ and $\hat{c}_\mu^\dagger$.  In
the remainder of this subsection, we will use the notation for bosonic
...
\end{equation}
This makes sense. Second, we can write the single-particle states as
as $|\mu\rangle = \hat{a}^\dagger_\mu|\varnothing\rangle$ (with some
abuse of notation), so
\begin{equation}
  \begin{aligned}\langle\mu|\hat{H}|\nu\rangle
...
occupation number of single-particle state $\mu$.  Thus, the total
energy is the sum of the single-particle energies, as expected for a
...
$\hat{a}_\mu^\dagger$ do so.  For fermions, Eqs.~\eqref{atrans1} and
\eqref{atrans2} hold using $c$ operators in place of $a$ operators,
and with anticommutation rather than commutation relations.
...
\end{equation}
consistent with Eq.~\eqref{Hdiag}.  Applying Eqs.~\eqref{atrans1} and
\eqref{atrans2},
\begin{equation}
  \hat{H} = \sum_{\mu\nu} \hat{a}_\mu^\dagger
  ...
  \label{Htransformed}
\end{equation}
Now consider the single-particle bra-ket
\begin{align}
  \langle\mu | \hat{H}_1 | \nu\rangle \equiv H_{\mu\nu}
  &= \sum_{\alpha\beta} \langle \alpha | U_{\alpha\mu} \hat{H}_1 U_{\beta\nu}^* |\beta\rangle \\
  &= \sum_{\alpha\beta} U_{\alpha\mu} U_{\beta\nu}^* \, E_\beta \, \delta_{\alpha\beta} \\
  &= \sum_{\alpha} E_\alpha\, U_{\alpha\mu} U_{\alpha\nu}^*,
\end{align}
which precisely matches the term in parentheses in
