...
states like $|\mu_1\rangle|\mu_2\rangle$ that are physically
impossible (as it is not symmetric or antisymmetric), and uses such
states to define a ``particle exchange'' operation that has no
physical meaning.  To get around this, Leinaas and Myrheim (1977) have
developed an interesting formulation of particle indistinguishability
that avoids the concept of particle exchange.  In this view, in a
multi-particle wavefunction the coordinates $(\mathbf{r}_1, \dots,
\mathbf{r}_N)$ are not to be regarded as an ordinary vector, but as a
mathematical object in which interchanging entries leaves the object
invariant.  Bosonic or fermionic states can then be constructed by
carefully analyzing the topological structure of wavefunctions defined
on such configuration spaces.  For more details, the interested reader
is referred to the paper by Leinaas and Myrheim [\ref{cite:leinaas}].
In this course, however, we will adopt the usual formulation based on
particle exchange.
...
\end{equation}
This evidently satisfies the required symmetry condition $\hat{P}_{12}
|\varphi,\varphi\rangle = |\varphi,\varphi\rangle$.  Next, suppose the
two bosons occupy \textit{different} single-particle states,
$|\varphi_1\rangle$ and $|\varphi_2\rangle$.  It would be wrong to
write the two-boson state as $|\varphi_1\rangle |\varphi_2\rangle$,
...
$|\varphi_1\rangle$, and one particle in state $|\varphi_2\rangle$.
To express the three-particle state, define $\{|\phi_1\rangle,
|\phi_2\rangle, |\phi_3\rangle\}$ where $|\phi_1\rangle =
|\phi_2\rangle = |\varphi_1\rangle$ and $|\phi_3\rangle =
|\varphi_2\rangle$.  Then
\begin{align}
  \begin{aligned}
  ...
  \end{aligned}
\end{align}
The exchange symmetry operators then have the following effects:
\begin{align}
  \begin{aligned}
    ...
    |\varphi_1\rangle|\varphi_2\rangle|\varphi_1\rangle\Big)
    ...
    |\varphi_2\rangle|\varphi_1\rangle|\varphi_1\rangle +
    |\varphi_1\rangle|\varphi_2\rangle|\varphi_1\rangle +
    |\varphi_1\rangle|\varphi_1\rangle|\varphi_2\rangle\Big)
    = |\phi_1,\phi_2,\phi_3\rangle.
  \end{aligned}
\end{align}
\end{framed}
...
states $|\varphi_1\rangle$ and $|\varphi_2\rangle$.  The appropriate
...
\begin{equation}
  \hat{P}_{12} |\varphi_1,\varphi_2\rangle = \frac{1}{\sqrt{2}} \Big(|\varphi_2\rangle|\varphi_1\rangle - |\varphi_1\rangle|\varphi_2\rangle\Big) = - |\varphi_1,\varphi_2\rangle.
\end{equation}
Eq.~\eqref{two_fermions} doesn't work if $|\varphi_1\rangle$ and
$|\varphi_2\rangle$ are the same; the two terms would cancel to give
the zero vector, which is not a valid quantum state.  This is a
manifestation of the \textbf{Pauli exclusion principle}, which states
that two fermions cannot occupy the same state.  In other words, each
...
$\{|\varphi_1\rangle, |\varphi_2\rangle,\dots,|\varphi_N\rangle\}$.
Then the appropriate $N$-fermion state is
\begin{equation}
  |\varphi_1,\dots,\varphi_N\rangle = \frac{1}{\sqrt{N!}} \sum_p s(p)\, |\varphi_{p(1)}\rangle |\varphi_{p(2)}\rangle \cdots |\varphi_{p(N)}\rangle.
  \label{Nfermions}
\end{equation}
...
If $\{|\varphi_1\rangle, \dots, |\varphi_N\rangle\}$ are not all
...
\begin{align}
  \begin{aligned}
    |\varphi_1, \varphi_2, \varphi_3\rangle
    &= \frac{1}{\sqrt{6}} \Big(
    |\varphi_1\rangle |\varphi_2\rangle |\varphi_3\rangle
    - |\varphi_2\rangle |\varphi_1\rangle |\varphi_3\rangle \\
    & \qquad\; + |\varphi_2\rangle |\varphi_3\rangle |\varphi_1\rangle
    - |\varphi_3\rangle |\varphi_2\rangle |\varphi_1\rangle \\
    & \qquad\; + |\varphi_2\rangle |\varphi_3\rangle |\varphi_1\rangle
    - |\varphi_1\rangle |\varphi_3\rangle |\varphi_2\rangle \Big).
  \end{aligned}
\end{align}
\end{framed}
...
\begin{align}
  \begin{aligned}
    \hat{P}_{ij}|\varphi_1,\dots,\varphi_N\rangle
    &= \frac{1}{\sqrt{N!}} \sum_p s(p)\, \hat{P}_{ij}
    \big[\cdots |\varphi_{p(i)}\rangle \cdots
      |\varphi_{p(j)}\rangle \cdots\big] \\
    &= \frac{1}{\sqrt{N!}} \sum_p s(p)\, \big[\cdots |\varphi_{p(j)}\rangle
      \cdots |\varphi_{p(i)}\rangle \cdots\big].
  \end{aligned}
\end{align}
...
$|\varphi_1,\dots,\varphi_N\rangle$, since the sum runs over all
...
$\hat{P}_{ij}|\varphi_1,\dots,\varphi_N\rangle = -
|\varphi_1,\dots,\varphi_N\rangle$ for any choice of $i \ne j$.
...
\begin{equation}
  ...
  \label{EPR}
\end{equation}
and discussed performing a partial measurement of $\hat{S}_z$ on
particle $A$.  If the measurement result is $+\hbar/2$, the
two-particle state collapses to $|\!+\!z\rangle |\!-\!z\rangle$.  Is
that previous discussion invalidated if the particles are identical?

The answer is no, because the particle exchange symmetry refers to an
exchange of \textit{all} the degrees of freedom of each particle.  If
we divide the particle degrees of freedom into subsets (e.g., position
and spin) and ``fix'' one of the subsets, that serves as an effective
means of distinguishing the particles.

To understand this, let us re-consider the EPR thought experiment in
detail, assuming that the two particles are identical fermions (the
discussion for bosons is similar).  We previously focused on each
...
Centauri ($A$) and a particle at Betelgeuse ($B$).  We previously did
not explicitly account for this position degree of freedom, but we
shall now do so.  Let the single-particle Hilbert space be
\begin{equation}
...
so that $\mathscr{H}_{\mathrm{position}}$ is a 2D space spanned by the
basis $\{|A\rangle, |B\rangle\}$.
...
product, and $B$ refers to the right-hand slot.  If we want to account
for the particles' fermionic nature and the fact that $A$ and $B$ are
quantum degrees of freedom, here is how we write the same state:
\begin{equation}
  ...
  \label{asym1}
\end{equation}
Here, the kets are written in the following order:
\begin{equation}
  \Big[
...
Hence, Eq.~\eqref{asym1} is explicitly antisymmetric:
\begin{align}
  \begin{aligned}
...
particle at $B$, the fermionic two-particle state is
\begin{equation}
  |-\!z, A;\; +z, B\rangle = \frac{1}{\sqrt{2}}
  \Big(|\!-\!z\rangle|A\rangle |\!+\!z\rangle|B\rangle
  - |\!+\!z\rangle|B\rangle |\!-\!z\rangle|A\rangle \Big).
...
\begin{align}
  \begin{aligned}
    ...
    &\qquad\;\;- |\!-\!z\rangle|A\rangle |\!+\!z\rangle|B\rangle
    + |\!+\!z\rangle|B\rangle |\!-\!z\rangle|A\rangle \Big).
    \label{epr1}
  \end{aligned}
\end{align}
This looks like a mess, but we can clarify it with some careful
re-ordering.  Instead of using the ordering \eqref{order1}, suppose we
separate the spin and position:
\begin{equation}
  \Big[
...
\end{equation}
In other words, the spin degrees of freedom are exactly as described
by our old notation, Eq.~\eqref{EPR}.  The position degrees of
freedom, which we previously did not monitor, form a symmetric
combination, and this makes the overall two-particle state
antisymmetric.
...
$|+\!z\rangle|-\!z\rangle$.  This goes to show that particle exchange
symmetry is fully compatible with the concepts of partial
measurements, entanglement, etc., that we discussed in the previous
chapter.
...
that form a complete orthonormal basis for the single-particle Hilbert
space $\mathscr{H}^{(1)}$.  Then, we build multi-particle states by
specifying how many particles are in state $|\varphi_1\rangle$,
denoted $n_1$; how many are in state $|\varphi_2\rangle$, denoted
$n_2$; and so on.  Thus,
\begin{equation*}
...
both particles in state $|\varphi_2\rangle$.  This is only possible if
the particles are bosons, since fermions cannot share the same state.
Written out in tensor product form, the symmetric state is
\begin{equation}
...
has one particle each occupying $|\varphi_1\rangle$,
$|\varphi_2\rangle$, and $|\varphi_3\rangle$.  If the particles are
bosons, this corresponds to the symmetric state
\begin{align}
...
\delta_{\mu\nu}$; the others are left as an exercise.
...
is just one particle ($N=1$), let $\hat{H}_1$ denote the
single-particle Hamiltonian, which is a Hermitian operator acting on
the single-particle Hilbert space $\mathscr{H}^{(1)}$.  For general
$N$, the multi-particle Hamiltonian $\hat{H}$ is a Hermitian operator
acting on the Fock space $\mathscr{H}^F$.  Given $\hat{H}_1$, what is
$\hat{H}$?
...
and annihilation operators.  For fermions, the expression is exactly
the same, but with the $\hat{a}_\mu$ and $\hat{a}_\mu^\dagger$
operators replaced by the fermion creation and annihilation operators
$\hat{c}_\mu$ and $\hat{c}_\mu^\dagger$.  In the remainder of this
...
its matrix elements with respect to various states.  First, for the
vacuum state $|\varnothing\rangle$,
\begin{equation}
  \langle\varnothing|\hat{H}|\varnothing\rangle = 0.
\end{equation}
This makes sense. Second, single-particle states can be written as
...
the generator of time evolution.  Eq.~\eqref{twobosonH} describes an
infinitesimal time step that consists of a superposition of
alternative evolution processes.  Each term in the superposition,
...
\hyperref[ex:n_conserved]{Exercise 5}).  It follows directly from the
...
way.  Let $\hat{A}_1$ be a Hermitian operator acting on
single-particle states, corresponding to some physical observable $A$.
For a multi-particle system, the operator corresponding to the ``total
$A$'' is
\begin{equation}
...
%% interaction, such as the electrostatic potential function.  
An operator of this form can be added to a Hamiltonian to describe a
