...
mass.  (These relations imply, by the way, that we can deduce the
spacing of the slits from the diffraction pattern, if the incident
energy $E$ is known.)
...
about the scatterer.  Scattering experiments, as a class, constitute a
large proportion of the methods used to probe the quantum world---from
...
incoming wave and a scattered wave.  The third condition says that far
from the scatterer ($|\mathbf{r}|\rightarrow \infty$), the incident
wave has a fixed wavelength determined by the energy $E$.  The final
condition says that the scattered part of the wavefunction must
correspond to a wave moving out to infinity.
...
the calculation, but part of the input---it is a parameter describing
the energy of the incoming particles in the experiment.
...
eigenvalues coalesce into a continuum.  To properly handle the
momentum eigenstates in this limit, we re-normalize them by taking
\begin{equation}
...
\vskip -0.15in
\noindent
The integrals are now taken over infinite spaces.  The position and
momentum eigenstates are now on a similar footing; both are
delta-function normalized.  Note that in deriving the above equations,
it is helpful to use the formula
\begin{equation}
...
non-singular functions, consisting of increasingly tall and narrow
gaussians centered at $x=0$.  For any non-singular potential, each
solution $\psi(x)$ is continuous and has well-defined first and second
derivatives.  In the delta function limit, the second derivative of
$\psi(x)$ blows up at $x=0$, while the first derivative becomes
discontinuous but finite; however, $\psi(x)$ itself remains
continuous.

To formally describe this, let us integrate the Schr\"odinger wave
...
$\psi(x)$, at $x = 0$, gives
\begin{equation}
  \Psi_i\left[ik(1+f_\pm) - ik(1-f_\pm)\right] =  \Psi(1+f_\pm) \gamma.
\end{equation}
...
The result is plotted above.  There are several notable features.
First, for fixed potential strength $\gamma$, the scattering strength
decreases monotonically with $E$---i.e., higher-energy particles are
scattered less easily.  Second, for given $E$, the scattering strength
...
scattering can occur ``to the side''.
...
plane wave can't be said to be ``outgoing''.  Therefore, we turn to
curvilinear coordinates.

In 2D, the relevant curvilinear coordinates are polar coordinates
$(r,\phi)$.  We will skip the mathematical details of how to solve the
2D Helmholtz equation in these coordinates.  The result is that the
...
\begin{figure}[h!]
  \centering\includegraphics[width=0.67\textwidth]{besselh}
\end{figure}
...
has the form of a superposition of outgoing and incoming spherical
waves:
\begin{equation}
...
exactly, whereas $\hat{G}$ typically has no finite exact expression.
...
for the right-hand side involves the unknown total state
$|\psi\rangle$ rather than the known incident state $|\psi_i\rangle$.
