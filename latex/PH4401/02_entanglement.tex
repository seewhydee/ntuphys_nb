...
was first raised by \hyperref[cite:ekert]{Ekert (1991)}, and it has
led to a huge amount of research into \textbf{quantum cryptography},
which is poised to be one of the most important technological
...
1), called a ``key'', such that it is impossible for anyone to
secretly learn the key by eavesdropping on their communications.  Once
Alice and Bob have established a secret shared key, it can be used to
encrypt subsequent messages between them, which nobody else can
decipher (e.g., by using
...
measurement axis ($S_1$, $S_2$, or $S_3$), and measure the spin along
that axis.
...
measurement results during these rounds are guaranteed to be exact
opposites of each other, and hence they have established a random
string that is known to each other but no one else.

How might an eavesdropper, Eve, attempt to foil this scheme?  Suppose
Eve intercepts the particle $B$ destined for Bob.  She might try to
substitute her own measurements, in some manner that could let her
work out the secret key.  However, Eve is hampered by the fact that
she is unable to predict or influence Bob's choices of measurement
axes (i.e., the choices are truly random), nor is she able to
impersonate Bob during the announcements of those axis choices (i.e.,
the classical communication channel is unjammable).  Under these
assumptions, it can be shown that any attempt by Eve to substitute her
own measurements can be detected by Alice and Bob, by performing a
statistical analysis of their measurement results in the rounds with
different different axis choices (this analysis is essentially the
detection of a Bell's inequality violation).  For details, refer to
\hyperref[cite:ekert]{Ekert (1991)}.

Another possibility for Eve is to ``clone'' the quantum state of
particle $B$ before passing it along to Bob.  If this can be done, Eve
could retain the cloned quantum state, wait for Bob to announce his
choice of measurement axis for that round, and then perform the
corresponding measurement to learn Bob's result.  Though plausible at
first glance, this turns out to be fundamentally unworkable, for it is
incompatible with the laws of quantum mechanics.

The proof of the \textbf{no-cloning theorem} is as follows.  Eve
desires to clone an arbitrary state of a spin-half particle $B$ onto
another spin-half particle $C$.  The two-particle Hilbert space is
$\mathscr{H}\otimes\mathscr{H}$.  With particle $C$ initially prepared
in some state $|0\rangle$, Eve must devise a unitary operation
$\hat{U}$ such that
\begin{equation}
  \hat{U} |\psi\rangle | 0\rangle = e^{i\phi} |\psi\rangle |\psi\rangle
  \label{clone}
\end{equation}
for all $|\psi\rangle \in \mathscr{H}$.  Note that the arbitrary phase
factor $\phi$ might depend on $|\psi\rangle$, and its presence does
not affect the outcomes of measurements.  Now replace $|\psi\rangle$
in the above equation with two arbitrary states denoted by
$|\psi_1\rangle$ and $|\psi_2\rangle$, and take their inner product.
According to Eq.~\eqref{clone},
\begin{align}
...
\end{align}
Note that $\langle 0 | 0\rangle = 1$ by normalization.  Comparing the
magnitudes of \eqref{clone1} and \eqref{clone2},
\begin{equation}
...
this cannot be true for \textit{any} choice of $|\psi_1\rangle$ and
$|\psi_2\rangle$.  For instance, for a two-dimensional space spanned
by the orthonormal basis $\{|0\rangle, |1\rangle\}$, we can pick
\begin{equation}
