...
which will be the subject of the next chapter.  (If you're not sure
...
To further understand the properties of $\hat{\rho}_A$, let us write
...
Eq.~\eqref{rhoform} is the general form for a density operator.  It is
a weighted sum of projection operators associated with normalized
state vectors $|\tilde{\varphi}_j\rangle$, which are \textit{not}
necessarily orthogonal to each other.  Since the weighting
coefficients $P_j$ are real numbers in the range $[0,1]$ and summing
to 1, we can interpret them as probabilities, meaning that
$\hat{\rho}_A$ describes an ensemble of quantum states weighted by a
set of classical probabilities.

A density operator that can be written in the form of
Eq.~\eqref{rho_pure} can be regarded as a special case with only one
quantum state in the ensemble.  Such density operators are said to
represent \textbf{pure states}, describing quantum systems that are
not entangled with any outside system.  Density matrices not
representing pure states are said to represent \textbf{mixed states},
describing systems that are entangled with something else.

Any linear operator of the form given by Eq.~\eqref{rhoform} has the
...
\item The eigenvalues of $\hat{\rho}_A$ are all real numbers in the
  range $[0,1]$, and sum to 1 (i.e., they can be interpreted as
  probabilities).  This implies that $\mathrm{Tr}[\hat\rho_A] = 1$.
\end{enumerate}

\noindent
Property 3 can be used to deduce the probability of obtaining any
...
Eq.~\eqref{rho_prob}; to show this, take $\hat{Q} = |\mu_i\rangle
\langle \mu_i|$ in Eq.~\eqref{prop3}.  Property 4 follows from
Property 3 by (i) taking $\hat{Q} = |\varphi\rangle\langle\varphi|$,
where $|\varphi\rangle$ is any eigenvector of $\hat\rho_A$, and (ii)
taking $\hat{Q} = \hat{I}_A$.
