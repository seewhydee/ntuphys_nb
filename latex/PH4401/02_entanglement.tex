...
\item \textbf{Second outcome: $-\hbar/2$}.  The projection operator is
  ...
  the post-collapse state is $|\!-\!z\rangle |\!+\!z\rangle$.
\end{itemize}
The two possible measurement outcomes, $+\hbar/2$ and $-\hbar/2$,
occur with equal probability.  In either case, the two-particle state
collapses so that particle $A$ is in the observed spin eigenstate, and
particle $B$ gets the opposite spin.  After the collapse, the
...
it the \textbf{density matrix}, based on the understanding that
...
$\mathscr{H} = \mathscr{H}_A \otimes \mathscr{H}_B$.  This operation
yields an operator acting on $\mathscr{H}_A$.

To better understand the implications of Eq.~\eqref{rhoa_def}, let us
work in an explicit basis.  Let $\{|\mu_1\rangle,
|\mu_2\rangle,\cdots\}$ be a basis for $\mathscr{H}_A$, and
$\{|\nu_1\rangle, |\nu_2\rangle,\cdots\}$ be a basis for
$\mathscr{H}_B$.  Any given state of the combined system can be
written in the tensor product basis:
\begin{equation}
...
$\hat{\rho}_A$, the eigenvalues are $\rho_{ii}^A$.  We can thus prove
that:
\begin{align}
  \begin{aligned}
...
A mixed state can also be interpreted as a \textit{classical
  probability distribution} of quantum states.  To see this, let us
return to Eq.~\eqref{rhoa_explicit}:
\begin{align}
...
\begin{equation}
  \hat{\rho}_A = \sum_j p_j |\psi''_j\rangle \langle \psi''_j|
  \;\;\;\mathrm{where}\;\;
  \begin{cases}
    \langle \psi''_j|\psi''_j \rangle &= 1 \\
    p_j &= \sum_i |\psi_{ij}|^2 \\
    |\psi''_j \rangle &= \frac{1}{\sqrt{p_j}} |\psi_j'\rangle.
  \end{cases}
\end{equation}
This can be regarded as a weighted sum of different pure state density
operators, where each pure state corresponds to some state
$|\psi_j''\rangle \in \mathscr{H}_A$, and the weights are classical
probabilities $p_j$ that sum to unity.  Note, however, that this
decomposition is not unique.  A density operator can generally be
expressed using different choices of pure state density operators.
For an example, see \hyperref[ex:rho_decomp]{Exercise 3}.
...
Entanglement entropy quantifies the uncertainty arising from a quantum
subsystem's lack of a definite quantum state (due to it being
entangled with another subsystem).  But we need to be careful when
extending classical notions of probability to quantum systems.  As
described above, when performing a measurement on $A$ whose possible
outcomes are $\{\mu_1, \mu_2, \dots\}$, the probability of getting
$\mu_i$ is $p_i = \langle \mu_i | \hat{\rho}_A|\mu_i\rangle$.
...
choice of measurement).  Eq.~\eqref{entropy} bypasses this problem by
using the trace, which is basis-independent.
...
1; hence, $S_{A} = 0$.  On the other hand, if we find that a system
has $S_{A} \ne 0$, that implies that it is entangled, and lacks a
definite quantum state.

By analogy with the classical entropy, the entanglement entropy has
the following bounds:
\begin{equation}
  0 \le S_A \le k_b\ln(d_A),
\end{equation}
where $d_A$ is the dimension of $\mathscr{H}_A$.  A system is said to
be \textbf{maximally entangled} if it saturates the upper bound, $S_A
= k_b \ln(d_A)$.  This occurs if and only if the eigenvalues of the
density operator are all equal: i.e., $p_i = 1/d_A$.
