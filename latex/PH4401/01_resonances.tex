...
entire space.  Both types can co-exist in a single system.  A simple
model exhibiting both bound states and free states is the \textbf{1D
  finite square well}.  Consider the Hamiltonian
\begin{equation}
...
\begin{figure}[h]
  \centering\includegraphics[width=0.65\textwidth]{squarewell}
\end{figure}

We wish to solve the time-independent Schr\"odinger equation for this
Hamiltonian.  A numerical procedure called the \textbf{transfer matrix
  method} can generate solutions for this potential and other similar
1D potentials.  This method is described in Appendix B.  Here, we will
bypass the details of the calculation, and instead zoom in on certain
key aspects.

The first thing to note is that when solving the Schr\"odinger wave
equation, we must specify the boundary conditions at infinity.  This
choice determines whether we get a bound state or free state.  The
wavefunction of a bound state diminishes exponentially as $x
\rightarrow \pm\infty$; thus, in the exterior region (i.e., $x < -a$
or $x > a$), it satisfies
\begin{equation}
...
It is possible to show that the bound state energies are
\textit{discrete}: the energy spacing decreases with $a$, but so long
as $a$ is finite, the spacing is non-vanishing.  Moreover, the
...
but outside it simply consists of superpositions of left-moving and
...
\begin{equation}
  \frac{\hbar^2k^2}{2m} = E,
\end{equation}
so free states can only occur for $E \ge 0$.  In fact, the solutions
form a continuum: there are free states for \textit{every} $E \ge 0$.
Since $|\psi(x)|^2$ does not diminish exponentially at infinity, the
integral $\int_{-\infty}^\infty |\psi(x)|^2\, dx$ is divergent, so
such wavefunctions have no finite normalization.
...
generalized to more complicated potential wells.  In cases where the
...
There is an important a proviso to bear in mind.  If we vary the
...
as we make the potential well shallower.  Note, however, that one
...
angular momentum quantum numbers.  For details about this phenomenon,
see \hyperref[ex:boundstate3d]{Exercise 2}.
...
In the 1D finite square well model, there is a clear distinction
between bound states and free states.  In certain situations, however,
there can appear a special class of so-called ``\textbf{quasi-bound
  states}''.  Like a bound state, a quasi-bound state is localized in
one region of space.  However, it is an \textit{approximate}
eigenstate of the Hamiltonian and lies within the energy range of the
free state continuum.  As we shall see, quasi-bound states have
extremely important effects on scattering experiments.
...
potential $V(x)$, which does not support bound states!  These
approximate solutions are the quasi-bound states.

Let us analyze the potential $V(x)$ using the \textbf{scattering
  experiment} framework from the previous chapter.  Consider a
particle incident from the left, with energy $E > 0$, described by the
incident wavefunction
\begin{equation}
  \psi_i(x) = \Psi_i \, e^{ik_i x}.
\end{equation}
This generates a scattered wavefunction $\psi_s(x)$ such that the
total wavefunction $\psi_i(x) + \psi_s(x)$ satisfies the Schr\"odinger
wave equation with energy $E$.  The scattered wavefunction is outgoing
(as discussed in the previous chapter), so in the exterior region it
takes the form
\begin{equation}
...
energies, we find something else interesting.  The figure below plots
...
various features of the quasi-bound state, which supplies important
...
equally important, if not more so, in 2D and 3D systems.  A useful
tool for studying them in general contexts is the quantum Green's
...
compute the scattering amplitudes.  We will calculate them by using
the states $|\varphi\rangle$ and $\{|\psi_k\rangle\}$ as a basis---but
