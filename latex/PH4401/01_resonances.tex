...
states it decays into; it is calculated using the quantity
$\langle\psi_{k}| \hat{V}_1|\varphi\rangle$, which called the
\textbf{transition amplitude}, using free states that meet the
resonance condition $E_k = E_{\mathrm{res}}$.  The stronger the
coupling, the faster the decay.  The second factor,
$\rho(E_{\mathrm{res}})$, is the number of free states per unit energy
at energy $E_{\mathrm{res}}$.  The more free states are available for
the quasi-bound state to decay into, the faster the decay.

The two factors that go into Fermi's golden rule are typically
calculated using further approximations.  Often, they can be estimated
using the free states of empty space, which are plane waves.

When plane waves are used to approximate the free states
$\{|\psi_k\rangle\}$, the $k$ labels are simply $d$-dimensional
wave-vectors.  It is then convenient to amend Eq.~\eqref{FGR} to treat
$k$-space as a continuum.  This follows the same procedure discussed
in Section II of the previous chapter.  We introduce a $k$-space
discretization $dk = 2\pi/L$, where $L$ is the system size in each
direction.  As $L\rightarrow \infty$, the discrete sum $\sum_k$
appearing in Eq.~\eqref{FGR}, in the definition $\rho(E)$, can be
turned into an integral.  In place of $\rho(E)$, we define
\begin{equation}
  \mathcal{D}(E) = \int \frac{d^d k}{(2\pi)^d} \; \delta(E - E_k).
\end{equation}
This is called the \textbf{density of states}, and it counts the
number of free states with energy $E$ per unit energy and per unit
volume (check the units!).
...
we re-normalize the free states used in the definition of $|W|^2$.  If
we follow the delta normalization convention discussed in Section II
...
Let us apply Fermi's golden rule to the simple 1D model from
Section~\ref{sec:resonances}, and see how well it works.  The model
potential is
\begin{equation}
...
simplicity, we focus on the ground state.  Let $E_0$ be the energy of
the ground state.  Its wavefunction has the form
\begin{equation}
...
we can derive a transcendental equation for $E_0$.  Knowing $E_0$, we
can derive $q$ and $\eta$.  By matching $\varphi(x)$ across the
interface at $x=a$, we get
\begin{equation}
  \frac{\mathcal{A}}{\mathcal{B}} = \frac{\exp(-\eta a)}{\cos(qa)}.
\end{equation}
Moreover, the wavefunction must be normalized to unity, so
\begin{equation}
  1 = \mathcal{A}^2 \int_{-a}^a \cos^2(qx) + 2\mathcal{B}^2 \int_a^\infty dx\; \exp(-2\eta x)
  %\\ &= \mathcal{A}^2 \left(a+\frac{\sin(2qa)}{2q}\right) + \mathcal{B}^2\, \frac{\exp(-2\eta a)}{\eta}.
\end{equation}
...
\end{equation}
where $\psi_k(x)$ is a free eigenstate for $V_0(x)$.  We can use a
trick to avoid calculating $\psi_k(x)$ explicitly.  Since $\psi_k(x)$
and $\varphi(x)$ must be orthogonal,
\begin{equation}
