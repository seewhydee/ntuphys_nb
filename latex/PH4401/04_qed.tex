...
recommended texts are Dyson's 1951 lecture notes [\ref{cite:dyson}]
and Zee's introductory textbook \textit{Quantum Field Theory in a
  Nutshell} [\ref{cite:zee}].
...
In the classical regime, the electromagnetic field acts on the
particle via the Lorentz force
\begin{equation}
...
\begin{equation}
  ...
  \label{eom}
\end{equation}
where $m$ is the particle's mass.  To quantize this, we seek to
...
What does this mean?  Let us consider what ``momentum'' means for a
...
So far, we have been using $p^2/2m$-type Hamiltonians describing
non-relativistic electrons.  In 1928, Paul Dirac formulated a
Hamiltonian that can describe electrons moving close to the speed of
light, thus successfully combining quantum theory with special
relativity. Another triumph of Dirac's theory is that it accurately
predicts the magnetic moment of the electron.
