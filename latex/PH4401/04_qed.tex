...
using the anticommutator $\{\hat{A},\hat{B}\} \equiv \hat{A}\hat{B} +
\hat{B}\hat{A}$:
\begin{equation}
  \{\hat{\alpha}_\mu, \hat{\alpha}_\nu\} = 2\delta_{\mu\nu},
  \;\;\; \textrm{for} \;\;\mu,\nu=0,1,2,3,
...
denote the usual Pauli matrices.  Since the $\hat{\alpha}_\mu$ are
...
such as the electromagnetic field.  Since the negative-energy states
extend to $E\rightarrow -\infty$, the electron would be able to
repeatedly transfer energy to the other system, such as by emitting
photons.  Then it keeps hopping to states with ever more negative
energies, shedding an infinite amount of energy in the process.  This
is obviously problematic.  However, let us wait for a while (till
Section~\ref{sec:positrons}) to discuss how the stability problem
might be resolved.

Let us first take some time to discuss the interpretation of the Dirac
...
thereof) consitutes two degrees of freedom.  Each individual band must
then posssess two independent degrees of freedom (so that $2\times 2 =
4$); these turn out to be associated with the electron's spin.
...
individual wavefunction components.  We will use the Dirac
...
by the upper two components, $\psi_A$, with $\psi_B \rightarrow 0$.
Conversely, for the lower band ($E \lesssim -mc^2$),
Eq.~\eqref{dirac_nonrel2} tells us that the wavefunction is dominated
by $\psi_B$, with $\psi_A \rightarrow 0$.  We can thus associate the
upper ($A$) and lower ($B$) components with the two-fold band degree
of freedom.  However, please note that this interpretation only holds
in the non-relativistic limit!  In the relativistic case, upper band
states do acquire non-vanishing values in the lower two wavefunction
...
wavefunction, we must understand how the electron interacts with an
...
\end{equation}
In the Dirac representation \eqref{alpha_matrices}, this reduces to a
pair of two-component equations:
\begin{align}
...
\end{align}
where $\psi_A$ and $\psi_B$ are the previously-introduced upper and
...
electromagnetic fields are weak but not necessarily vanishing; then
$\Psi_A$ and $\Psi_B$ are envelope functions that vary slowly in $t$.
...
theory of quantum electrodynamics, and matches experiment to around
one part in $10^9$, making it one of the most precise theoretical
predictions in physics.  For details, see \hyperref[cite:zee]{Zee
  (2010)}.

\subsection{Positrons and Dirac Field Theory}
\label{sec:positrons}

Hence, the number of components
is $2\times2 = 4$.  It is noteworthy that we did not set out to
include spin in the theory, yet it has popped up, seemingly
unavoidably, in the process of satisfying the relativistic
...
important issue of how they behave under Lorentz transformations and
other coordinate transformations.  For these details, the reader is
