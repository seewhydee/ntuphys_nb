...
Unlike in classical electrodynamics, in quantum electrodynamics it is
the scalar and vector potentials that enter directly into the theory,
not the electric and magnetic fields.  One interesting consequence is
that when a quantum charged particle resides in a region of space
where the magnetic field is zero, the particle can still be acted on
by the magnetic \textit{vector potential}, if that is nonzero.  This
is called the \textbf{Aharanov-Bohm effect}.

A simple setting for observing the Aharanov-Bohm effect is shown in
Fig.~???.  A particle is trapped in a ring-shaped region (an
``annulus''), of radius $R$ and width $d \ll R$.  Outside the annulus,
we set $|\!-e\Phi|\rightarrow\infty$ so that the wavefunction
vanishes; within the annulus, $\Phi = 0$.  We ignore the
$z$-dependence of all fields and wavefunctions, so that the problem is
two-dimensional.

Now, suppose we thread magnetic flux (e.g., using a solenoid) through
the interior region enclosed by the annulus.  This can be described
using the vector potential
\begin{equation}
  \mathbf{A}(r,\phi) = \frac{\Phi_B}{2\pi r} \, \hat{e}_\phi.
  \label{Asolenoid}
\end{equation}
We can easily verify from Eq.~\eqref{Asolenoid} that the magnetic flux
through any loop of radius $r$ enclosing the origin is $(\Phi_B/2\pi
r)(2\pi r) = \Phi_B$, independent of $r$.  Hence, $\mathbf{B} = 0$
everywhere except at the origin, whereas $\mathbf{A}$ is nonzero
everywhere.
...
\begin{align}
  E_n &= \frac{1}{2m} \left[
    ...
    + \left(\frac{\pi}{d}\right)^2 \right] \\
  &= \frac{e^2}{8\pi^2mR^2} \left(\Phi_B + \frac{nh}{e} \right)^2
  + \frac{\pi^2}{2md^2}.
\end{align}

Energy levels vary continuously with $\Phi_B$.  This is the
Aharanov-Bohm effect.

Integer magnetic flux quanta bring the spectrum back to itself.
Change in vector potential can be undone by a gauge transformation.
