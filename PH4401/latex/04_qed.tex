...
As we shall see, however, Dirac's theory contains the seeds of its own
destruction, in the form of unphysical ``negative energy'' solutions.
We now know that these unphysical solutions arise from the fact that a
consistent theory of \textit{single-particle} relativistic quantum
mechanics is ultimately impossible.  The right way to marry relativity
and quantum mechanics is to use the framework of quantum field theory,
which can describe states with arbitrary numbers of electrons and
photons.  When the Dirac Hamiltonian is translated into this
framework, and coupled to a quantized electromagnetic field, the
resulting theory of quantum electrodynamics (QED) constitutes the most
fundamental and successful theory of electromagnetism known to us.
...
We know that the time and space derivatives of wavefunctions are
related to energy and momentum by
\begin{equation}
    ...
    p.
\end{equation}
And we also know that the energy and momentum of a relativistic
particle are related by
\begin{equation}
  E^2 = \sum_{j=1}^3 p_j^2c^2 + m^2c^4,
  \label{Erelativistic}
\end{equation}
where $m$ is the rest mass and $c$ is the speed of light.  (We adopt
the index labels $\{1,2,3\}$ for the spatial coordinates $\{x,y,z\}$,
as is the usual practice in relativity theory.)  Notably, $E$ and $p$
appear to the same order in Eq.~\eqref{Erelativistic}---unlike the
non-relativistic kinetic energy $E = p^2/2m$, where $E$ is first-order
while $p$ is second-order.  Since the left side of the Schr\"odinger
equation has a first-order time derivative, we expect that a
relativistic Hamiltonian $\hat{H}$ should contain only first-order
spatial derivatives (momentum operators).  So we make the guess
\begin{equation}
...
\end{equation}
where the $\hat{p}_j$ operators have been replaced with definite
numbers.  If $\psi$ is a scalar wavefunction, this would imply that
$\alpha_0 mc^2 + \sum_{j}\alpha_j p_j c = E$, which does not match the
relativistic energy-mass-momentum relation \eqref{Erelativistic}.
However, we can get it to work if $\psi$ is a multi-component
wavefunction, and the $\alpha$'s are not ordinary numbers, but
non-commuting matrices.  In that case, applying the Hamiltonian twice
gives
\begin{equation}
...
\begin{equation}
  ...
  = E^2\, \hat{I},
\end{equation}
where $\hat{I}$ is the identity matrix.  Multiplying out the squared
term on the left side gives (and taking care of the fact that the
$\hat{\alpha}_\mu$ matrices need not commute) yields
\begin{equation}
...
\end{equation}
This reduces to the relativistic energy-mass-momentum relation
\eqref{Erelativistic} if the $\hat{\alpha}_\mu$ matrices satisfy
\begin{align}
  \begin{aligned}
  ...
  \end{aligned}
\end{align}
(Here, we adopt the convention from relativity theory of using Greek
symbols to label indices ranging over $\{0,1,2,3\}$, and Latin symbols
for indices ranging over $\{1,2,3\}$.)  The above relations can be
...
\end{equation}
Here, $\{\cdots,\cdots\}$ denotes the anticommutator,
$\{\hat{A},\hat{B}\} \equiv \hat{A}\hat{B} + \hat{B}\hat{A}$.

It turns out that the simplest set of four matrices that can satisfy
Eq.~\eqref{Dirac_anticomm} are $4\times4$ matrices.  This implies that
the wavefunction $\psi(\mathbf{r})$ is not a complex scalar field, but
a complex four-component field.  We have previously seen two-component
wavefunctions used to describe non-relativistic particles with a
spin-$1/2$ degree of freedom, with the two components being used
represent ``spin-up'' and ``spin-down'' quantum amplitudes.  It is
natural to guess that the four components of Dirac's wavefunctions are
(at least partly) related to spin.  This suggests that in relativistic
quantum mechanics, spin has to be incorporated into the theory at a
fundamental level, rather than being an ``optional'' component like in
non-relativistic quantum mechanics.

There is no unique choice for matrices satisfying
Eq.~\eqref{Dirac_anticomm}.  A typical choice is
\begin{align}
  \begin{aligned}
...
denote the usual Pauli matrices.  Different choices for the
$\hat{\alpha}_\mu$ matrices correspond to different choices of
wavefunction representation.  This is similar to how, in the
non-relativistic spin-$1/2$ case, we can choose the two wavefunction
components to represent the spin amplitudes for the $+z$ and $-z$
basis, or the $+x$ and $-x$ basis, or any other basis.
