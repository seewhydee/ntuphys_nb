...
satisfy Eq.~\eqref{Dirac_anticomm} are $4\times4$ matrices.  Thus,
$\psi(\mathbf{r})$ cannot be a scalar field; it must have four
components.

What might these four components represent?  First of all, note that
Eq.~\eqref{Erelativistic} only gives $E$ up to a $\pm$ sign:
\begin{equation}
...
and a negative value.  This implies that there has to be an even
number of wavefunction components: one set describing quantum
amplitudes corresponding to the upper band, and the other set
describing quantum amplitudes corresponding to the lower band.
...
turns out to come from the particle's spin.  We are already familiar
with the fact that two-component wavefunctions can be used to describe
the spin-$1/2$ degree of freedom for non-relativistic particles; the
components can represent, for instance, the quantum amplitudes for
``spin up'' and ``spin down'' ($+z$ and $-z$), or whatever choice of
spin basis we choose to adopt.  It is highly noteworthy that we did
not set out to include spin in the theory, yet it has popped up,
seemingly unavoidably, in the process of satisfying the relativistic
energy-mass-momentum relation.  This is a hint that relativistic
quantum mechanics is much more theoretically ``rigid'' than
non-relativistic quantum mechanics.  Features such as spin are not
optional parts of the theory, but need to be included at a fundamental
level.

There is no unique way to specify the $\hat{\alpha}_\mu$ matrices.
Different choices correspond to different wavefunction
representations, similar to the choice of spin basis in the familiar
two-component non-relativistic theory.  A typical choice is
\begin{align}
  \begin{aligned}
...
denote the usual Pauli matrices.  

These matrices are nice and simple to remember, but be careful when
using them!  Contrary to the simple description given above (``two
bands, with two-fold spin in each band''), the wavefunction components
in this representation are actually \textit{not} neatly divided into
``upper band'' and ``lower band'' components.  The band degree of
freedom and the spin degree of freedom are scrambled together, such
that even if a state is, say, entirely in the upper band with a
definite spin, all four wavefunction components are typically nonzero.
(See \hyperref[cite:foldy]{Foldy and Wouthuysen (1949)} for a cunning
and non-trivial way around this problem.)
