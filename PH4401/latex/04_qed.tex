...
Previously, we have seen how a simple scalar boson field can be
quantized (see Chapter 3, Section V.C, ``A scalar boson field''): the
classical field is decomposed into its normal modes, and each normal
mode is promoted into an oscillator-like quantum degree of freedom,
with its own creation and annihilation operators.  By comparing the
classical and quantum energies, we can then derive the Hermitian
operator corresponding to the classical field variable, expressed in
terms of the creation and annihilation operators.  With only minor
adjustments, this basic strategy can also be used to quantize the
electromagnetic field.

In this section, we will restrict our attentions to the case of a
``source-free'' electromagnetic field---i.e., all electric charges and
currents are absent.  In this scenario, Maxwell's equations (in SI
units) reduce to:
\begin{align}
  ...
  \label{max4}
\end{align}
Once again, we introduce the electromagnetic scalar potential $\Phi$
and vector potential $\mathbf{A}$:
\begin{align}
  \mathbf{E} &= - \nabla \Phi - \frac{\partial\mathbf{A}}{\partial t} \\
  \mathbf{B} &= \nabla \times \mathbf{A}.
  \label{Bfield}
\end{align}
With two relations, Eqs.~\eqref{max2} and \eqref{max3} are satisfied
automatically, via vector identities.  The other two Maxwell
equations, \eqref{max1} and \eqref{max2}, become:
\begin{align}
...
Hence, in the Coulomb gauge Eq.~\eqref{max5} is automatically
satisfied, and the remaining field equation \eqref{max6} simplifies to
\begin{equation}
  \left(\nabla^2 - \frac{1}{c^2}\frac{\partial^2}{\partial t^2}\right)
  \mathbf{A} = 0. \label{max8}
\end{equation}
Hence, we can guess the normal modes of the electromagnetic field:
they are \textbf{plane waves}, for which the vector potential has the
form
\begin{equation}
  \mathbf{A}(\mathbf{r},t) = \mathcal{A}\, \hat{e} \, e^{i(\mathbf{k}\cdot\mathbf{r} - \omega t)} +
  \mathcal{A}^* \, \hat{e}\, e^{-i(\mathbf{k}\cdot\mathbf{r} - \omega t)},
\end{equation}
where $\mathcal{A}$ is a complex number, and $\hat{e}$ is a real unit
vector specifying the direction along which the vector potential is
polarized.  Referring to Eq.~\eqref{max8}, we see that the frequency
$\omega$ and wave-vector $\mathbf{k}$ must satisfy
\begin{equation}
  \omega = c|\mathbf{k}|.
\end{equation}
...
\end{equation}
In other words, the polarization vector is perpendicular to the plane
wave's direction of propagation.  For a given $\mathbf{k}$, there are
two orthogonal polarization, which we can label by some index $\lambda
= 1,2$.  Hence, the decomposition of the vector potential into a
superposition of plane wave normal modes has the form
\begin{equation}
  \mathbf{A}(\mathbf{r},t) = \sum_{\mathbf{k},\lambda} 
  \mathcal{A}_{\mathbf{k},\lambda} \, \hat{e}_{\mathbf{k},\lambda}
  \, e^{i(\mathbf{k}\cdot\mathbf{r} - \omega t)} +
  \mathcal{A}_{\mathbf{k},\lambda}^* \, \hat{e}_{\mathbf{k},\lambda}\,
  e^{-i(\mathbf{k}\cdot\mathbf{r} - \omega t)}, \;\;\; \mathrm{where}
  \;\;\;\omega = c|\mathbf{k}|.
\end{equation}
