...
\begin{equation}
\boxed{\qquad \hat{H} = \hat{\alpha}_0 mc^2 + \sum_{j=1}^3 \hat{\alpha}_j \hat{p}_j c, \;\; \mathrm{where}\;\; \hat{p}_j \equiv -i\hbar\frac{\partial}{\partial x_j}.\qquad}
\label{Dirac}
\end{equation}
...
Contrary to the above hand-waving description (``two bands, with
two-fold spin in each band''), the components of the Dirac
wavefunction are generally \textit{not} neatly grouped into ``upper
band'' and ``lower band'' components---even for a seemingly-simple
choice of $\hat{\alpha}_\mu$ matrices like Eq.~\eqref{alpha_matrices}.
For a detailed discussion about the structure and interpretation of
the Dirac Hamiltonian and its wavefunction (including the important
issue of how they behave under rotations and Lorentzian
transformations), refer to \hyperref[cite:dyson]{Dyson (1951)}.

\subsection{Relativistic electrons in an electromagnetic field}

Having obtained a Hamiltonian that describes the dynamics of a
relativistic electron, we are naturally interested in putting the
electron in an electromagnetic field.  The non-relativistic theory
from Section~\ref{sec:nonrel} provides a natural guess for how to do
this.  There, we simply add $-e\Phi(\mathbf{r},t)$ as a scalar
potential function, and insert the vector potential via the
substitution
\begin{equation}
  \hat{\mathbf{p}} \rightarrow \hat{\mathbf{p}}
  + e\mathbf{A}(\hat{\mathbf{r}},t).  
\end{equation}
If we apply this procedure to the Dirac Hamiltonian (\ref{Dirac}), the
result is
\begin{equation}
  \hat{H} = \hat{\alpha}_0 mc^2 -e\Phi(\mathbf{r},t)
  + \sum_{j=1}^3 \hat{\alpha}_j \left[-i\hbar\frac{\partial}{\partial x_j}
    +eA_j(\mathbf{r},t) \right] c.
\end{equation}
