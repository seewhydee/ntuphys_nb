...
where $f(\mathbf{k}\rightarrow \mathbf{k}')$ is used to denote the
scattering amplitude to scatter from $\mathbf{k}$ to $\mathbf{k}'$.
We can now plug in the Born series,
$$\begin{aligned}|\psi\rangle = \left[\hat{I} + \hat{G}_0 \hat{V} + (\hat{G}_0 \hat{V})^2 + (\hat{G}_0 \hat{V})^3 + \cdots\right]|\psi_i\rangle, \\
\end{aligned}$$
together with our definition of the incident wavefunction.  The result
is
$$\boxed{\;\;\;f(\mathbf{k}\rightarrow \mathbf{k}') = - \frac{2m}{\hbar^2} \,\cdot \, 2\pi^2 \,\cdot\, \big\langle \mathbf{k}'\big| \hat{V} + \hat{V}\hat{G}_0 \hat{V} + \hat{V} \hat{G}_0 \hat{V} \hat{G}_0\hat{V} + \cdots \big|\mathbf{k}\big\rangle.\;\;\;}$$

The above equation is the culmination of the numerous definitions and
