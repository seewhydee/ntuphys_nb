...
\hbar^2 k_i^2 / 2m$, where $k \in \mathbf{R}^+$ is the wavenumber in
free space.  Then the above equation can be written as
$$\left[\nabla^2 + k_i^2\right] \psi_s(\mathbf{r}) = 0,$$
...
$|\mathbf{k}| = k_i$.  But we're looking for an outgoing solution, and
...
$$\psi(\mathbf{r})=\sum_{\pm}\sum_{m=-\infty}^\infty c_m^\pm\,\Psi_m^\pm(r,\phi), \;\;\;\mathrm{where}\;\;\,\Psi_m^\pm(r,\phi) = H_m^\pm(k_ir)\,e^{im\phi}.$$
...
$$\psi(\mathbf{r})=\sum_{\pm}\sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell c_{\ell m}^\pm \,\Psi_{\ell m}^\pm(r,\theta,\phi)\;\;\;\mathrm{where}\;\;\Psi_{\ell m}^\pm(r,\theta,\phi) = \,h_\ell^\pm(k_ir)\,Y_{\ell m}(\theta,\phi).$$
...
$$\psi_s(\mathbf{r}) = \begin{cases} \displaystyle\sum_{m} c_m^+\,H_m^\pm(k_ir)\,e^{im\phi}, &d=2\\ \displaystyle\sum_{\ell m} c_{\ell m}^+\,h_\ell^+(k_ir)\,Y_{\ell m}(\theta,\phi),&d=3.\end{cases}$$
For large $r$, the outgoing wavefunction's $r$-dependence can be
written as
$$\psi_s(\mathbf{r}) \; \overset{r\rightarrow\infty}{\sim} \; r^{\frac{1-d}{2}} \,\exp\left(ik_ir\right).$$
...
$$\begin{aligned}J_r \; &\overset{r\rightarrow\infty}{\sim} \; \mathrm{Im}\left[r^{\frac{1-d}{2}} e^{-ik_ir} \frac{\partial}{\partial r}\left(r^{\frac{1-d}{2}} e^{ik_ir}\right)\right] \\ &\;\;=\;\;\;\mathrm{Im}\left[\frac{1-d}{2}\, r^{-d} + ik_i r^{1-d}\right]\\ &\;\;=\;\;\; k_i \,r^{1-d}.\end{aligned}$$
In $d$ dimensions, the area of a wave-front scales as $r^{d-1}$, so
the probability flux goes as $J_r \,r^{d-1} \sim k_i$, which is
...
$$f(\mathbf{k}_i \rightarrow k_i \hat{\mathbf{r}}).$$
This emphasizes that the incident wave-vector is $\mathbf{k}_i$, and
that the particle gets scattered in a direction specified by the unit
position vector $\hat{\mathbf{r}}$ (which can be expressed with
directional coordinates $\Omega$).  This can be regarded as the
scattering of the particle into the momentum state specified by
$\mathbf{k}_f = k_i \hat{\mathbf{r}}$.
...
$$\begin{aligned}\langle\mathbf{r} |\big(E-\hat{H}_0\big) \hat{G}_0 |\mathbf{r}'\rangle &= \langle\mathbf{r}|\hat{I}|\mathbf{r}'\rangle \\ = \left(E + \frac{\hbar^2}{2m}\nabla^2 \right) \langle\mathbf{r} |\hat{G}_0 |\mathbf{r}'\rangle &= \delta^d(\mathbf{r}-\mathbf{r}') \\ \Rightarrow \qquad\quad \left(\nabla^2 + k_i^2\right) \langle\mathbf{r} |\hat{G}_0 |\mathbf{r}'\rangle &= \frac{2m}{\hbar^2} \delta^d(\mathbf{r}-\mathbf{r}'),\end{aligned}$$
where
$$k_i = \sqrt{\frac{2mE}{\hbar^2}}$$
is the wavenumber of plane waves in free space for energy $E$ (the
energy of the incident particle).  Therefore, up to a factor of
$2m/\hbar^2$, the propagator is the Green's function for the
$d$-dimensional Helmholtz equation (see
Section~\ref{sec:2d3d_scattering}).  Note that the $\nabla^2$ acts
upon the $\mathbf{r}$ coordinates, not $\mathbf{r}'$.
...
real-$k$ line, and since $k_i \in \mathbb{R}^+$, there are two poles
on the contour (at $\pm k_i$).  Hence, the value of the integral, as
written, is singular.
...
$$\begin{aligned} \psi_i(\mathbf{r}) &= \Psi_i \, e^{i\mathbf{k}_i\cdot\mathbf{r}}, \\ \psi_s(\mathbf{r}) &= \langle\mathbf{r}| \hat{G}_0 \hat{V} |\psi\rangle \;\; \overset{r\rightarrow\infty}{\longrightarrow}\;\; \Psi_i \, r^{\frac{1-d}{2}} \, e^{ik_ir} \, f(\mathbf{k}_i\rightarrow k_i\hat{\mathbf{r}}).
\end{aligned}$$
...
$$\begin{aligned}\psi_s(\mathbf{r}) &\;\;= \;\; \int d^3r'\; \langle\mathbf{r}|\hat{G}_0|\mathbf{r}'\rangle\, V(\mathbf{r}')\, \psi(\mathbf{r}') \\ &\overset{r\rightarrow\infty}{\approx} \, - \frac{2m}{\hbar^2} \, \frac{e^{ikr}}{4\pi r}\; \int d^3r' \exp\left(-ik_i \, \hat{\mathbf{r}} \cdot \mathbf{r}'\right)\, V(\mathbf{r}')\, \psi(\mathbf{r}') \\ &\;\;=\;\; - \frac{2m}{\hbar^2} \, \frac{e^{ikr}}{4\pi r} \; (2\pi)^{3/2} \; \big\langle k_i \hat{\mathbf{r}} \big|\hat{V}\big|\psi\big\rangle. \end{aligned}$$
Here, $|k_i\hat{\mathbf{r}} \rangle$ denotes a momentum eigenstate
with wave-vector $\mathbf{k} = k_i \hat{\mathbf{r}}$.  We now plug in
$$|\psi\rangle = \left(\hat{I} + \hat{G}\hat{V}\right)|\psi_i\rangle,$$
and obtain
$$\begin{aligned}\psi_s(\mathbf{r}) \; &\overset{r\rightarrow\infty}{\longrightarrow} \; - \frac{2m}{\hbar^2} \, \frac{e^{ikr}}{r} \; \sqrt{\frac{\pi}{2}} \; \big\langle k_i \hat{\mathbf{r}} \big|\hat{V} + \hat{V}\hat{G}\hat{V}\big|\psi_i\big\rangle \\ &= - \frac{2m}{\hbar^2} \, \Psi_i\, \frac{e^{ikr}}{r} \; 2\pi^2 \; \big\langle k_i \hat{\mathbf{r}} \big|\hat{V} + \hat{V}\hat{G}\hat{V}\big|\mathbf{k}_i\big\rangle.\end{aligned}$$
This can be compared to the earlier definition of the scattering amplitude,
$$\psi_s(\mathbf{r}) \; \overset{r\rightarrow\infty}{\longrightarrow} \; \Psi_i \frac{e^{ik_ir}}{r} f(\mathbf{k}_i\rightarrow k_i\hat{\mathbf{r}} ).$$
Hence, we get
$$\boxed{\;\;\;\begin{aligned}f(\mathbf{k}\rightarrow \mathbf{k}') &= - \frac{2m}{\hbar^2} \,\cdot \, 2\pi^2 \,\cdot\, \big\langle \mathbf{k}'\big| \hat{V} + \hat{V}\hat{G} \hat{V} \big|\mathbf{k}\big\rangle \\ &= - \frac{2m}{\hbar^2} \,\cdot \, 2\pi^2 \,\cdot\, \big\langle \mathbf{k}'\big| \hat{V} + \hat{V}\hat{G}_0 \hat{V} + \hat{V} \hat{G}_0 \hat{V} \hat{G}_0\hat{V} + \cdots \big|\mathbf{k}\big\rangle,  \end{aligned}\quad\; \mathrm{where}\;\; |\mathbf{k}| = |\mathbf{k}'|.\;\;\;}$$
In deriving the last line, we used the Born series
$$|\psi\rangle = \left[\hat{I} + \hat{G}_0 \hat{V} + (\hat{G}_0 \hat{V})^2 + (\hat{G}_0 \hat{V})^3 + \cdots\right]|\psi_i\rangle.$$

The above result is the culmination of the numerous definitions and
