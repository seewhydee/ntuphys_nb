...
$$V(\mathbf{r}) = \begin{cases}-U, & |\mathbf{r}| \le R \\ \;\;\;\,0, & |\mathbf{r}| > 0. \end{cases}$$
...
problem can be solved exactly.  The details are described in Appendix
A, and the resulting scattering amplitudes are
...
The figure below compares the Born approximations, computed for the
uniform potential well via Monte Carlo integration, to the ``exact''
solution.  It plots $|f|^2$ versus the scattering energy $E$, for the
case of $90^\circ$ scattering (i.e., $\mathbf{k}_f$ perpendicular to
$\mathbf{k}_i$), for two wells of different depths $U$ and the same
radius $R = 1$.  We take computational units $\hbar = m = 1$, and each
...
important.  In fact, if the potential is too strong (what ``too
strong'' means is a complicated issue that we can't discuss here),
even taking the Born approximation to higher orders might not work,
because the Born series can become non-convergent.  In those cases,
different methods must be brought to bear.  (We will see an example in
the next chapter, in the form of phenomena known as ``scattering
resonances''.)
