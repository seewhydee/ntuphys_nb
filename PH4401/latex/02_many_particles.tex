...
To conclude this chapter, we discuss a logical but controversial set
of implications that arise from the phenomenon of quantum
entanglement.  This is the \textbf{many-worlds interpretation}, a set
of ideas that cut to the heart of our understanding of quantum theory.

One of the postulates of quantum mechanics, which we have relied on so
far, is the \textbf{measurement postulate}, which states the
...
The measurement postulate has two rather strange features.  First, it
is the only place where randomness, or ``indeterminism'', creeps into
quantum theory.  The other postulates do not refer to probabilities.
In particular, the time-dependent Schr\"odinger equation
...
$t$ by solving the Schr\"odinger equation.  There is no randomness
involved in doing that calculation.  Another way of saying this is
that the time evolution operator $U(t,t_0)$, defined by
$$U(t,t_0)\,|\psi(t_0)\rangle = |\psi(t)\rangle,$$
always satisfies the unitary property $U^{-1} = U^\dagger$.  By
contrast, state collapse is \textit{not} a unitary process.
\textcolor{red}{[How to show?]}  It seems strange that quantum theory
describes two ways for a quantum state to change, time evolution and
measurement, which appear be completely distinct.

The second noteworthy feature of the measurement postulate is that it
is completely silent on the issue of what constitutes a
``measurement''.
