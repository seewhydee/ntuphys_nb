...
its energy $E = -\Delta$.  Furthermore, let us denote the continuum of
...
$$\begin{aligned}\big(\hat{T} + \hat{V}_0\big) |\varphi\rangle &= -\Delta |\varphi\rangle \\ \big(\hat{T} + \hat{V}_0\big) |\psi_k\rangle &= E_k |\psi_k\rangle,
\end{aligned}$$
...
\langle\varphi|\hat{G}|\varphi\rangle \left(1 - \frac{\langle\varphi|\hat{V}_1|\varphi\rangle}{E + \Delta + i\varepsilon}\right) &= \lim_{\varepsilon\rightarrow0^+} \frac{1}{E + \Delta + i\varepsilon} \left(1+ \int dk\, \langle\varphi|\hat{G}|\psi_k\rangle \, \langle\psi_k| \hat{V}_1|\varphi\rangle \right) \\
\lim_{\varepsilon\rightarrow0^+} \langle\varphi|\hat{G}|\varphi\rangle \Big(E + \Delta -\, \langle\varphi|\hat{V}_1|\varphi\rangle \, & + i\varepsilon\Big) - \int dk\, \langle\varphi|\hat{G}|\psi_k\rangle \, \langle\psi_k| \hat{V}_1|\varphi\rangle = 1.\end{aligned}$$
...
$$\lim_{\varepsilon\rightarrow0^+} \left[\langle\varphi|\hat{G}|\varphi\rangle \left(E + \Delta -\, \langle\varphi|\hat{V}_1|\varphi\rangle \, + i\varepsilon\right) - \int dk\, \frac{\langle\varphi|\hat{G}|\varphi\rangle\langle\varphi|\hat{V}_1|\psi_k\rangle}{E-E_k+i\varepsilon} \, \langle\psi_k| \hat{V}_1|\varphi\rangle\right] \approx 1.$$
Hence,
$$\boxed{\begin{aligned}\quad \langle\varphi|\,\hat{G}(E)\,|\varphi\rangle \approx \lim_{\varepsilon\rightarrow0^+} \frac{1}{\displaystyle E + \Delta - \langle\varphi|V_1|\varphi\rangle - \Sigma(E)} \qquad \\
\mathrm{where}\;\;\Sigma(E) \equiv \int dk\, \frac{\displaystyle\langle\varphi|\hat{V}_1|\psi_k\rangle \langle\psi_k| \hat{V}_1|\varphi\rangle}{\displaystyle E-E_k+i\varepsilon}. \qquad
\end{aligned}}$$

This is an extremely significant result.  It tells us that
$\langle\varphi|\hat{G}(E)|\varphi\rangle$ is large when the
denominator is as close to zero as possible.  As we vary $E$, the
energy of the incident particle in the scattering experiment, this is
satisfied when $E$ meets the \textbf{resonance condition}
$$E \approx - \Delta + \langle\varphi|\hat{V}_1|\varphi\rangle + \mathrm{Re}\big[\Sigma(E)\big].$$
The first term in this expression is the energy of the bound state in
the potential well, while the second term describes how this bound
state energy is shifted by the barrier potential $\hat{V}_1$.  The
third term involves the energy-dependent quantity $\Sigma(E)$, called
the \textbf{self-energy} of the resonance.  It describes how the
resonance energy is altered by the influence of the continuum states.
