...
$V_b-U < E < V_b$.  These bound states' wavefunctions diminish
...
systems, and we will see examples in subsequent chapters.  As a
demonstration of its utility, let us apply it to the simple 1D model
from Section~\ref{sec:resonances}.  We will compare the predictions of
Fermi's golden rule (which, remember, is an approximate result) to the
``exact'' resonance widths observed in a scattering experiment (as
computed via the transfer matrix method).

The model potential is
$$\begin{aligned}V(x) &= V_0(x) + V_1(x), \;\;\;\mathrm{where} \\ &V_0(x) = -U \, \Theta(a-|x|), \;\; V_1(x) = V_b\, \Theta(b-|x|).\end{aligned}$$
The potential well $V_0$ supports one or more bound states.  For
simplicity, we focus on its ground state, which becomes the lowest
quasi-bound state once $V_1$ is introduced.
