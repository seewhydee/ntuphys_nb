...
The first term is the non-relativistic kinetic energy of the
electrons, and the second term is the potential energy of their
Coulomb interaction.  Operators $\hat{\mathbf{p}}_1$ and
...
that commute with the Hamiltonian.  Exchange symmetry can thus be
represented by an operator $\hat{P}$, defined as follows: let
$\{|\mu_i\rangle\}$ be a basis for the single-electron Hilbert space
$\mathscr{H}^{(1)}$; then $\hat{P}$ interchanges the basis vectors for
the two electrons:
...
symmetry, apply an arbitrary exchange operator $\hat{P}_{ij}$:
...
thus symmetric under every exchange operation.
...
The \textbf{Pauli exclusion principle} states that fermions cannot
occupy the same state; or, equivalently, that each single-particle
state is either unoccupied, or occupied by exactly one fermion.
...
states $\{|\varphi_1\rangle,|\varphi_2\rangle,\dots\}$ used for
constructing the particle creation and annihilation operators.  Let us
now suppose that they are quantum states of a particle moving in
space, representable by a wavefunction.  Let $|\mathbf{r}\rangle$
denote a position eigenstate for a $d$-dimensional space.  Then, for
each single-particle state $|\varphi_\mu\rangle$, the wavefunction is
$$\varphi_\mu(\mathbf{r}) = \langle\mathbf{r}|\varphi_\mu\rangle.$$
Due to the completeness and orthonormality of the basis, the
...
Using the wavefunctions and the creation/annihilation operators, we
can construct a new and extremely interesting set of operators.  For
now, suppose the particles are bosons.  Define
$$\begin{aligned}\hat{\psi}(\mathbf{r}) = \sum_\mu \varphi_\mu(\mathbf{r}) \, \hat{a}_\mu \\ \hat{\psi}^\dagger(\mathbf{r}) = \sum_\mu \varphi_\mu^*(\mathbf{r}) \, \hat{a}_\mu^\dagger. \end{aligned}$$
Using the aforementioned wavefunction properties, we can also invert
the above relations, obtaining
$$\begin{aligned}\hat{a}_\mu &= \int d^dr \; \varphi_\mu^*(\mathbf{r}) \, \hat{\psi}(\mathbf{r}) \\ \hat{a}_\mu^\dagger &= \int d^dr \; \varphi_\mu(\mathbf{r}) \, \hat{\psi}^\dagger(\mathbf{r}).\end{aligned}$$

The $\hat{\psi}(\mathbf{r})$ and $\hat{\psi}^\dagger(\mathbf{r})$
operators are called \textbf{field operators}.  Think of the classical
notion of a field, which assigns to each point of space, $\mathbf{r}$,
a set of numbers that corresponding to physical quantities, such as
the electric field components $E_x(\mathbf{r})$, $E_y(\mathbf{r})$,
and $E_z(\mathbf{r})$.  In the present case, we assign to each point
of space not a \textit{number}, but a \textit{quantum mechanical
  operator} $\hat{\psi}(\mathbf{r})$ and its Hermitian conjugate
$\hat{\psi}^\dagger(\mathbf{r})$.  The field operators allow us to
extend the classical concept of a field to quantum theory---i.e., to
formulate a \textbf{quantum field theory}.

Using the commutation relations between the bosonic
creation/annihilation operators, we can show that the field operators
obey
...
With these, we can use the field operators to re-express the second
quantized observables from the previous section.  For any Hermitian
operator $\hat{A}_1$ acting on single-particle, we saw that there is a
``total'' observable, applicable to states with arbitrary numbers of
particles, of the form $\hat{A} = \sum_{\mu\nu} \hat{a}^\dagger_\mu
A_{\mu\nu} \hat{a}_\nu$, where $A_{\mu\nu} =
\langle\varphi_\mu|\hat{A}_1|\varphi_\nu\rangle$.  In terms of the
field operators,
...
single-particle position and momentum operators.  We can show that the
corresponding second quantized operators for the total kinetic energy
and the total potential energy are
...
To further clarify the relationship between the classical and quantum
field theories, we can re-arrange the field operators by defining
$\hat{Q}(\mathbf{r})$ and $\hat{P}(\mathbf{r})$ such that
$$\begin{aligned}\begin{cases}\hat{Q}(\mathbf{r}) &= \displaystyle\frac{1}{\sqrt{2}}\left(\hat{\psi}(\mathbf{r}) + \hat{\psi}(\mathbf{r})\right) \\ \hat{P}(\mathbf{r}) &= \displaystyle -\frac{i\hbar}{\sqrt{2}}\left(\hat{\psi}(\mathbf{r}) - \hat{\psi}(\mathbf{r})\right) \end{cases} \;\;\;\Leftrightarrow\;\;\; \begin{cases}\hat{\psi}(\mathbf{r}) &= \displaystyle\frac{1}{\sqrt{2}}\left(\hat{Q}(\mathbf{r})+\frac{i}{\hbar}\hat{P}(\mathbf{r})\right) \\ \hat{\psi}^\dagger(\mathbf{r}) &= \displaystyle\frac{1}{\sqrt{2}}\left(\hat{Q}(\mathbf{r})-\frac{i}{\hbar}\hat{P}(\mathbf{r})\right).\end{cases}\end{aligned}$$
The $\hat{Q}(\mathbf{r})$ and $\hat{P}(\mathbf{r})$ operators are
Hermitian, and are therefore valid observables.  They serve as quantum
analogues of the classical notion of the ``value of a field at a point
in space''.  Using the commutation relations for the
$\hat{\psi}(\mathbf{r})$ and $\hat{\psi}^\dagger(\mathbf{r})$
operators, we can show that
$$\left[\hat{Q}(\mathbf{r}), \hat{Q}(\mathbf{r}')\right] = \left[\hat{P}(\mathbf{r}), \hat{P}(\mathbf{r}')\right] = 0, \quad \left[\hat{Q}(\mathbf{r}), \hat{P}(\mathbf{r}')\right] = i\hbar\,\delta(\mathbf{r}-\mathbf{r}').$$
These are generalizations of the canonical commutation relations
between position and momentum operators.  Hence, one feature of
quantum fields is that certain field observables \textit{at the same
  point} cannot be simultaneously observed, in the same way that
position and momentum are not simultaneously observable in ordinary
quantum mechanics.
