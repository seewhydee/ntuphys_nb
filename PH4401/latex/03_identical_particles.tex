...
construct the particle creation and annihilation operators.  Let us
now consider the special case where these quantum states are
representable by a wavefunction.  Let $|\mathbf{r}\rangle$ denote a
position eigenstate for a $d$-dimensional space; for each
single-particle state $|\varphi_\mu\rangle$, the wavefunction is
...
to construct a new and interesting set of operators.  \textit{For now,
  suppose the particles are bosons.}  Let us define
...
Using the commutation relations for the bosonic $a_\mu$ and
...
Using the $\hat{\psi}(\mathbf{r})$ and
$\hat{\psi}^\dagger(\mathbf{r})$ operators, second quantized
observables can be written in a manner independent of the choice of
single-particle wavefunctions.  As discussed in the
\hyperref[sec:second_quant_op]{previous section}, for any Hermitian
single-particle operator $\hat{A}_1$, we can define a corresponding
many-particle observable $\hat{A} = \sum_{\mu\nu} \hat{a}^\dagger_\mu
A_{\mu\nu} \hat{a}_\nu$, where $A_{\mu\nu} =
\langle\varphi_\mu|\hat{A}_1|\varphi_\nu\rangle$.  Then we can show that
...
momentum operators (single-particle observables).  We can show that
the corresponding second quantized operators for the total kinetic
energy and the total potential energy are
...
construct a Hermitian operator is to multiply a creation operator with
an annihilation operator: this gives the operator
$\hat{\psi}^\dagger(\mathbf{r})\hat{\psi}(\mathbf{r})$, which is an
observable corresponding to the local particle density.  However,
there is another (possibly less obvious) class of observables that we
can construct by taking \textit{linear superpositions} of creation and
annihilation operator.  For example,
$$\psi(\mathbf{r}) + \psi(\mathbf{r})^\dagger$$
is necessarily Hermitian.  More generally, consider an operator of the
form
$$F(\mathbf{r}) = \int d^dr' \Big(f(\mathbf{r},\mathbf{r}') \hat{\psi}(\mathbf{r}) + f^*(\mathbf{r},\mathbf{r}') \hat{\psi}^\dagger(\mathbf{r}') \Big),$$
where $f(\mathbf{r},\mathbf{r}')$ is some complex function.  This type of
Hermitian operator is the quantum analogue of the classical value of a
field at a point.

This situation is closely analogous to the quantum harmonic
oscillator.  There, the creation and annihilation operators are
non-Hermitian operators satisfying $[\hat{a},\hat{a}^\dagger]$, with
the Hermitian operator $\hat{a}^\dagger\hat{a}$ corresponding to the
number of oscillator quanta excited.  Moreover the creation and
annihilation operators can be related to another pair of Hermitian
observables,
...
which obey the canonical commutation relation $[\hat{x},\hat{p}] =
i\hbar$.  These operators correspond, of course, to the position and
momentum coordinates of the underlying oscillator ($m$ and $\omega_0$
denote the oscillator's mass and natural frequency).
