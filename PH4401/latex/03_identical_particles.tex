...
express multi-particle quantum operators clearly and succinctly, by
making use of the creation and annihilation operators defined in
Sections~\ref{sec:second_quantized_bosons}--\ref{sec:second_quantized_fermions}.
This is particularly important for formulating multi-particle
Hamiltonians.

We start by considering a system of \textit{non-interacting}
particles.  In the case where the system contains just one particle
($N=1$), let $\hat{H}_1$ denote the single-particle Hamiltonian, which
is some Hermitian operator acting on the single-particle Hilbert space
$\mathscr{H}^{(1)}$.  For general $N$, the multi-particle Hamiltonian
$\hat{H}$ is a Hermitian operator acting on the Fock space
$\mathscr{H}^F$.  Given $\hat{H}_1$, what is $\hat{H}$?
...
Secondly, for single-particle states, they can be written as
...
exactly matching the definition of $H_{\mu\nu}$ in our expression for
$\hat{H}$.

Thirdly, consider the special case where
$\{|\varphi_1\rangle,|\varphi_2\rangle,\dots\}$ is an eigenbasis of
$\hat{H}_1$.  Then,
...
occupation number of single-particle state $\mu$.  Therefore, the above
expression for $\hat{H}$ states the total energy is equal to the sum
of the single-particle energies, which is exactly what we expect for a
system of non-interacting particles.

As we know, the Hamiltonian is the generator of time evolution.
Hence, one way to interpret the expression $\hat{H} = \sum_{\mu\nu}
\hat{a}^\dagger_\mu H_{\mu\nu} \hat{a}_\nu$ is that each infinitesimal
time step is a superposition of alternative evolution processes.  Each
process, $\hat{a}^\dagger_\mu H_{\mu\nu} \hat{a}_\nu$, describes a
particle being annihilated in state $\nu$, and immediately re-created
in state $\mu$, which is equivalent to transferring a particle from
$\nu$ to $\mu$.  The quantum amplitude for this process is described
by the matrix element $H_{\mu\nu}$.  This description applies not only
to single-particle states, but also to multi-particle states
containing an arbitrary number of particles.

Note also that in this description, the number of particles does not
change.  Any time a particle is annihilated in some state $\nu$, it is
immediately replaced by a particle created in some state $\mu$.  This
implies that the Hamiltonian commutes with the total particle number
operator:
$$[\hat{H}, \hat{N}] = 0, \;\;\;\mathrm{where}\;\;\hat{N} \equiv \sum_\mu \hat{a}_\mu^\dagger a_\mu.$$
The formal proof for this is left as an exercise to the reader (see
...
The result can also be extended to any kind of single-particle
observable, including momentum, angular momentum, and so forth.  In
general, let $\hat{A}_1$ be a Hermitian operator acting on
single-particle states, corresponding to a physical observable $A$.
For multi-particle states, the operator corresponding to the ``total
$A$'' is
$$\hat{A} = \sum_{\mu\nu} \hat{a}^\dagger_\mu A_{\mu\nu} \hat{a}_\nu, \;\;\; \mathrm{where}\;\; A_{\mu\nu} = \langle\varphi_\mu|\hat{A}_1|\varphi_\nu\rangle.$$

\textcolor{red}{[Interaction energy]}

\textcolor{red}{[Particle non-conserving terms]}
...
Then, using the creation and annihilation operators
