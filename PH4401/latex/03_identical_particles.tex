...
So far, we have seen how Hermitian operators corresponding to the
kinetic and potential energy can be constructed via \textit{products}
of $\hat{\psi}^\dagger(\mathbf{r})$ with $\hat{\psi}(\mathbf{r})$.
But there is another type of Hermitian operator that we can construct
by taking \textit{linear combinations} of of
...
the classical notion of the value of a field at a point.

\subsection{Revisiting the harmonic oscillator}

To better understand how Hermitian operators of the form
\eqref{fieldop} are related to classical field variables, we first
need to take a more detailed look at the harmonic oscillator.  In
particular, we need to know how the concept of a \textbf{mode of
  oscillation} carries over from classical to quantum mechanics.
(Later, we will generalize this to the modes of oscillation of a
field.)
...
\begin{equation}
  ...
  \label{xt0}
\end{equation}
This describes an oscillation of frequency $\omega$ (the natural
frequency of the harmonic oscillator).  It is parameterized by the
\textbf{mode amplitude} $\mathcal{A}$, a complex number that
determines the magnitude and phase of the oscillation.

For the quantum harmonic oscillator, the $x$ and $p$ variables are
replaced by the Hermitian operators $\hat{x}$ and $\hat{p}$.  As we
know, the annihilation and creation operators $\hat{a}$ and
$\hat{a}^\dagger$ can then be defined:
\begin{align}
  \left\{
  \begin{aligned}
...
Here, let us see how the creation and annihilation operators relate to
the \textit{dynamics} of the quantum harmonic oscillator.  In the
Heisenberg picture (with $t = 0$ as the reference time), we define the
time-dependent operator
\begin{equation}
  \hat{x}(t) = \hat{U}^\dagger(t) \,\hat{x}\, \hat{U}(t), \;\;\;
  \hat{U}(t) \equiv \exp\left(-\frac{i}{\hbar}\hat{H}t\right).
\end{equation}
Here, we adopt the notation that those operators written with an
explicit time dependence are Heisenberg picture operators, while those
without are Schr\"odinger picture operators; hence, $\hat{x} \equiv
\hat{x}(0)$.  We can likewise define the Heisenberg picture creation
and annihilation operators, $\hat{a}^\dagger(t)$ and $\hat{a}(t)$,
which are related to $\hat{x}(t)$ by
\begin{equation}
...
annihilation operators, we would like to construct an observable
corresponding to an oscillator variable.  We take the ansatz
\begin{equation}
  \hat{x}(t) = 
  ...
  \label{xtc}
\end{equation}
which is manifestly Hermitian.  How should the constant $\mathcal{C}$
be chosen?

We can answer this question by identifying how the oscillator variable
relates to the oscillator energy \textit{in the classical limit}.  For
a standard classical harmonic oscillator, the energy satisfies
\begin{equation}
...
Eqs.~\eqref{Hclass}--\eqref{xt0}, but one way to remember this result
is that for a harmonic oscillator, the average kinetic energy equals
the average potential energy (this is an instance of the virial
...
re-defined so that this holds).  We get
\begin{equation}
...
\infty$ (corresponding to energies much larger than $\hbar\omega$).
Combining Eqs.~\eqref{xavg2}, \eqref{Havg1}, and \eqref{xavg1} in this
limit gives
\begin{align}
  \hbar \omega |\alpha|^2 &= m\omega^2 \cdot 2\mathcal{C}^2|\alpha|^2 \\
  \Rightarrow\;\;\; \mathcal{C} &= \sqrt{\frac{\hbar}{2m\omega}}.
\end{align}
This is precisely the scale factor found in Eq.~\eqref{xtquantum}.

\subsection{A scalar boson field}

We now have the tools available to establish the connection between
the simplest possible classical field and its quantum field theory.
The classical field variable $f(x,t)$ is a scalar defined in one
spatial dimension, whose classical equation of motion is the wave
equation:
\begin{equation}
...
\end{equation}
The Hamiltonian is that of a collection of decoupled quantum harmonic
oscillators:
\begin{equation}
...
\end{equation}
Thus, suppose we define a Schr\"odinger picture field operator
\begin{equation}
...
Then the corresponding Heisenberg picture operator is
\begin{equation}
...
To determine $\mathcal{C}_n$, we again follow the procedure from the
previous section, comparing the energy of the quantum system's
classical limit and the energy of the classical field theory given by
Eqs.~\eqref{classical_energy_density}--\eqref{classical_energy}.  In
the end, we find that the field operator corresponding to the
classical field variable is
\begin{equation}
...
%% \subsection{The vacuum energy problem}
