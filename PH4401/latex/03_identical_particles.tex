...
state $|\beta\rangle$, etc.:
$$|n_\alpha,n_\beta,n_\gamma,\dots\rangle$$
The corresponding multi-particle state is then \textit{defined} as the
appropriate symmetric (for bosons) or antisymmetric (for fermions)
...
For fermions, the multi-particle states are antisymmetric, and this
gives rise to a complication: for the occupation number representation
to be well-defined, it is necessary to choose an ordering for the
single-particle states.

In the bosonic context, it doesn't really matter whether we write
$|n_\alpha,n_\beta,n_\gamma,\dots\rangle$ or
$|n_\beta,n_\alpha,n_\gamma,\dots\rangle$, since the multi-particle
states are symmetric.  But for fermions, each multi-particle state
involves a sum that contains parity coefficients $s(p)$.  So, for
example, if we order the states as
$|n_\alpha,n_\beta,n_\gamma,\dots\rangle$, then
$$|1,1,0,0,\dots\rangle \equiv \frac{1}{\sqrt{2}} \Big(|\alpha\rangle|\beta\rangle - |\beta\rangle |\alpha\rangle\Big).$$
Whereas if we order the states as $|n_\beta,n_\alpha,n_\gamma,\dots\rangle$, then
$$|1,1,0,0,\dots\rangle \equiv \frac{1}{\sqrt{2}} \Big(|\beta\rangle |\alpha\rangle - |\alpha\rangle|\beta\rangle\Big).$$
This means that we need to decide how the single-particle states are
ordered.  It does not matter which ordering we choose, so long as we
make \textit{some} choice, and stick to it consistently.  To emphasize
this, we will change our notation slightly.  Instead of writing the
occupation number states as $|n_\alpha,n_\beta,\dots\rangle$, we will
write them as $|n_1,n_2,n_3,\dots\rangle$, where $n_\mu$ denotes the
occupation number of the $\mu$-th single-particle state in our ordered
list.

The fermion creation operator can now be defined as follows:
...
In other words, if state $\mu$ is unoccupied, then $\hat{c}_\mu^\dagger$
increments the occupation number to 1, and multiplies the state by an
overall factor of $(-1)^{n_1 + n_2 + \cdots + n_{\mu-1}}$ (i.e, $+1$
if the number of occupied states preceding $\mu$ is even, and $-1$ if
it is odd).  The purpose of this factor will become more apparent
later.  If $\mu$ is occupied, then applying $\hat{c}_\mu^\dagger$
gives the zero vector.  In this way, the occupation numbers are
forbidden from exceeding one.
