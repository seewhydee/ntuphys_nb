...
To better understand the relationship between classical fields and
quantum fields, we first need to take a more detailed look at the
harmonic oscillator.  In particular, we need to know how the concept
of a \textbf{mode of oscillation} carries over from classical to
quantum mechanics.
...
\textbf{photons}, elementary bosonic particles of light that are
generated and destroyed by creation and annihilation operators.  We
can define Hermitian field operators that correspond to the classical
electromagnetic field variables (i.e., the components of the electric
and magnetic fields at each point of space).
...
Moreover, the ether is not composed of a single quantum field, but
several interacting fields.  The various excitations of these fields
give rise not only to light (photons), but also other force-carrying
particles like gluons, as well as the particles that make up matter
(e.g.,~quarks and electrons).  The latter are described by fermionic
fields, which are inherently quantum mechanical entities with no
straightforward classical counterpart.

It is then quite natural to regard the quantum fields, not particles,
as fundamental objects.  In this view, the fact that particles of the
same type all have identical physical properties, and are
indistinguishable from each other, is a natural feature.  The
particles are not really distinct objects, but excitations of more
fundamental, underlying entities---quantum fields.
