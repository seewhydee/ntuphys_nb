...
With these, we can use the field operators to express the second
quantized observables from the previous section, in a way that does
not refer to the single-particle wavefunctions anymore.  For any
Hermitian operator $\hat{A}_1$ acting on single-particle, we
previously saw that the corresponding many-particle observable has the
form $\hat{A} = \sum_{\mu\nu} \hat{a}^\dagger_\mu A_{\mu\nu}
\hat{a}_\nu$, where $A_{\mu\nu} =
\langle\varphi_\mu|\hat{A}_1|\varphi_\nu\rangle$.  The expression in
