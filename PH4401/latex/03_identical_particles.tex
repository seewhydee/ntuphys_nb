...
of Hermitian operator corresponds to the observable value of a field
at a given point $\mathbf{r}$.

\subsection{Revisiting the harmonic oscillator}

To better understand how Hermitian operators of the form
\eqref{fieldop} are related to the classical notion of a field
variable, we first need to take a more detailed look at the harmonic
oscillator.  In particular, we are interested in how the concept of a
``mode of oscillation'' carries over from classical to quantum
mechanics (later, this will be generalized to the independent modes of
oscillation of a field).

The Hamiltonian of a classical harmonic oscillator is
\begin{equation}
  H(x,p) = \frac{p^2}{2m} + \frac{1}{2}m\omega^2x^2,
  \label{Hclass}
\end{equation}
where $x$ is the oscillator variable, $p$ is the corresponding
momentum variable, $m$ is the mass, and $\omega$ is the natural
frequency of oscillation.  We know that the classical equation of
motion has the general form
\begin{equation}
  x(t) = \mathcal{A}\, e^{-i\omega t} + \mathcal{A}^*\, e^{i\omega t}.
  \label{xt0}
\end{equation}
This motion is completely parameterized by the ``mode amplitude''
$\mathcal{A}$, a complex number that determines the magnitude and
phase of the oscillation.

For the quantum harmonic oscillator, the position and momentum
variables are promoted to Hermitian operators, $\hat{x}$ and
$\hat{p}$.  As we have previously seen, these can be used to define
...
\begin{equation}
  \hat{x} = \sqrt{\frac{\hbar}{2m\omega}} \, \left(\hat{a}+\hat{a}^\dagger\right), \;\;\;\hat{p} = -i \sqrt{\frac{m\omega\hbar}{2}} \, \left(\hat{a}-\hat{a}^\dagger\right).
\end{equation}
...
\end{equation}
From these, the energy spectrum of the quantum harmonic oscillator can
be derived; we will not go through the details again here.

Let us focus on the \textit{dynamics} of the quantum harmonic
oscillator.  In the Heisenberg picture (with $t = 0$ as the reference
time), we define the time-dependent operator
\begin{equation}
...
\begin{equation}
  ...
  \label{xtquantum}
\end{equation}
Comparing this with the classical solution \eqref{xt0}, we see that
$\hat{a}$ plays the role of the classical mode amplitude
$\mathcal{A}$, up to a scale factor.

Now, suppose we come at things from the opposite end: we have creation
and annihilation operators satisfying
Eq.~\eqref{create_annihilate_qho}, from which follow
Eqs.~\eqref{heisenbergeq}--\eqref{at}.  Using the creation and
annihilation operators, we can construct Hermitian operators of the
form
\begin{equation}
  \hat{x}(t) = 
  ...
  \label{xtc}
\end{equation}
where $\mathcal{C}$ is any constant.  As a matter of convention, we
will take $\mathcal{C}$ to be real.  The question is, what is the
appropriate $\mathcal{C}$ so that this operator corresponds to the
position variable of an oscillator?

One way to answer this question is to look at the classical limit, and
identify how the position variable of a classical oscillator relates
to its energy.  It can be shown that the energy of a classical
oscillator
is
\begin{equation}
...
\begin{equation}
  ...
  \label{Havg1}
\end{equation}
Moreover, using Eq.~\eqref{coherent} with \eqref{xtc} gives
\begin{equation}
...
\begin{equation}
  ...
  \label{xavg1}
\end{equation}
The classical limit is reached for $|\alpha|\rightarrow \infty$, which
corresponds to energies much larger than the energy quantum
...
\subsection{A 1D scalar boson field}
