...
the single-particle states.  This is called the \textbf{occupation
  number representation}.  To do this, we first enumerate the possible
single-particle states by $\{|\varphi_1\rangle, |\varphi_2\rangle,
|\varphi_3\rangle, \cdots\}$.  Together, these form an orthonormal
basis set for the single-particle Hilbert space $\mathscr{H}^{(1)}$.

To construct a multi-particle state, we can specify how many particles
are in state $|\varphi_1\rangle$, which we call $n_1$; how many are in
state $|\varphi_2\rangle$, which we call $n_2$; and so forth.  Then
...
both particles in state $|\varphi_2\rangle$.  Assuming that the
...
Thus far, we have been agnostic about the nature of the
single-particle states, i.e., what the vectors in the Hilbert-space
$\mathscr{H}^{(1)}$ represent.  Often, we are interested in the case
where these states represent particles moving through space

single-particle states can be represented by wavefunctions.
