...
\textit{positions} commute.  The interpretion is that the operators
$\hat{\psi}^\dagger(\mathbf{r})$ and $\hat{\psi}(\mathbf{r})$
respectively create and annihilate one particle at a given point
$\mathbf{r}$.
...
$\mathbf{r}$ is associated with a distinct pair of
$\hat{\psi}(\mathbf{r})$ and $\hat{\psi}^\dagger(\mathbf{r})$
operators.  This concept of $\mathbf{r}$-dependent operators is a
generalization of the classical concept of a \textbf{field}.  In a
classical field theory, each point $\mathbf{r}$ is assigned a set of
numbers corresponding to physical quantities, such as the electric
field components $E_x(\mathbf{r})$, $E_y(\mathbf{r})$, and
$E_z(\mathbf{r})$.  The present theory, where each $\mathbf{r}$ is
assigned a set of quantum operators, is called a \textbf{quantum field
  theory}.
...
A_{\mu\nu} \hat{a}_\nu$, where $A_{\mu\nu} =
\langle\varphi_\mu|\hat{A}_1|\varphi_\nu\rangle$.  This can be
re-written in terms of the $\hat{\psi}(\mathbf{r})$ and
$\hat{\psi}^\dagger(\mathbf{r})$ operators as
