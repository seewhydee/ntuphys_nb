...
``entangled'' if it cannot be expressed as definite states of the
individual particles.  It would be nice to make this statement more
precise, by formulating an expression for the ``level of
entanglement'' in any given system.  In fact, physicists have come up
with several ways to do this; in this section, we will describe the
most commonly-used one, called \textbf{entanglement entropy}, which is
closely related to the concept of entropy in statistical mechanics and
information theory.
...
This is nothing but the projection operator (matrix) associated with
$|\psi\rangle$, but in this context we call it a \textbf{density
  matrix}.  We use the notation $\hat{\rho}(\psi)$ to emphasize that
the density matrix is dependent on the chosen state $|\psi\rangle$.
Evidently, it is a Hermitian operator; one eigenvalue is 1 (with
eigenvector $|\psi\rangle$), and the other $d-1$ eigenvalues are all
$0$ (with eigenvectors given by $d-1$ vectors spanning the subspace
orthogonal to $|\psi\rangle$).

The density matrix can be used to calculate the probabilities of
various measurement outcomes.  Suppose the system is in state
$|\psi\rangle$, and we perform a measurement corresponding to a
Hermitian operator $\hat{Q}$, which has eigenvalues
...
Hence, expectation values of $\hat{\rho}$ specify how much of the
state $|\psi\rangle$ lies along any given direction $|q_j\rangle$ in
the Hilbert space.

Next, suppose the system that we are considering consists of two
subsystems, whose respective Hilbert spaces are $\mathscr{H}_A$ and
$\mathscr{H}_B$, where $\mathscr{H} = \mathscr{H}_A \otimes
\mathscr{H}_B$.  Starting from a density matrix of the combined
system, $\hat{\rho}(\psi)$, we can define a \textbf{reduced density
...
which traces over the Hilbert space of subsystem B.  What's left after
this partial trace is an operator acting solely on the Hilbert space
$\mathscr{H}_A$.

To understand the meaning of the partial trace and the reduced density
matrix, let us work in an explicit basis.  Let $\{|\mu_1\rangle,
|\mu_2\rangle,\cdots\}$ be a basis for $\mathscr{H}_A$, and
$\{|\nu_1\rangle, |\nu_2\rangle,\cdots\}$ a basis for $\mathscr{H}_B$.
A given state of the combined system can be written in the tensor
product basis:
...
The reduced density matrix, obtained by tracing over the states of
subsystem B, is
...
It is easily verified that $\hat{\rho}_A(\psi)$ is a Hermitian
operator.

The reduced density matrix can be used to calculate the outcome
probabilities for \textit{partial} measurements performed on subsystem
A.  Suppose the overall system is in state $|\psi\rangle$, and we
perform a measurement on subsystem A corresponding to a Hermitian
operator $\hat{\mu}$, which has eigenvectors
...
Entropy in information theory and stat mech.

Spin-$1/2$ system and maximum entropy states.
