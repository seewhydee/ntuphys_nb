...
decay away from the potential well:
...
non-singular, we can be assured that

$$\int_{-\infty}^{\infty} dx\; |\psi(x)|^2 < \infty,$$

so the state is normalizable to unity. With a bit more mathematical
analysis, one can also prove that:

* The bound states form a discrete set, with a discrete (though
possibly degenerate) eigenenergies $\{E_0, E_1, E_2, \cdots\}$.
...
for some $\alpha_\pm, \beta_\pm \in \mathbb{C}$. Such solutions include solutions to the scattering problem (@sec-scattering-theory), and they are called **free states**.  They are *not* normalizable, because the integral $\int_{-\infty}^\infty |\psi(x)|^2 \, dx$ diverges.^[For a nonzero superposition of traveling waves, $|\psi(x)|^2$ oscillates with $x$ with a nonzero average value.  Thus, when we evaluate $\int_{-\infty}^{-a} |\psi(x)|^2 \, dx$ or $\int_{a}^{\infty} |\psi(x)|^2 \, dx$, the integration range is infinite while the average value of the integrand is nonzero, so the integral diverges.  The only way for the integral not to vanish is if $\psi(x) = 0$ everywhere outside the potential well; this case corresponds to a scattering problem with zero inputs and outputs, implying that $\psi(x) = 0$ everywhere, which is not a valid quantum wavefunction.] In just the same way as we deal with momentum eigenstates (@sec-momentum-eigenstates), we can treat these solutions as generalized eigenstates of $\hat{H}$, even though they do not, strictly speaking, lie in the Hilbert space.

However, it is *almost* normalizable in the sense that $|\psi|^2$ does
not blow up as $|x| \rightarrow \pm\infty$.  We can thus treat it as a
...
for @eq-psiwaves so as to satisfy the Schrödinger wave equation.  It
turns out that these four coefficients are not independent; for
example, if we fix $\alpha_\pm$, that uniquely determines $\beta_\pm$.
As this can be done for all $E \ge 0$, @eq-psiwaves represents a
continuous family of free state solutions.
...
For instance, at $U = 30$, there are five bound states.  If we make
the well shallower, the bound state energies increase until the
largest one reaches zero, whereupon that bound state ceases to exist.
As we proceed, the bound states continue to disappear one by one,
until only one remains.  (This one remaining bound state persists as
$U \rightarrow 0$; there is a theorem that any purely-attractive 1D
potential must support at least one bound state.  See [Exercise
