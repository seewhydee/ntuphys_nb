...
## Quasi-bound states and resonances {#sec-resonances}
...
particularly important role in scattering experiments.
...
The vertical axis shows the **transmittance** $|1+f_+|^2$, which is
the probability for the particle to pass through the scatterer.  The
horizontal axis is the particle energy $E$.  Unsurprisingly, for $E <
V_b-U$ the transmittance approaches zero, and for $E \gtrsim V_b$ it
approaches unity.  For $V_b-U < E \lesssim V_b$, the transmittance
...
$$\begin{aligned}\big(\hat{T} + \hat{V}_0\big) |\varphi\rangle \; &= E_0 |\varphi\rangle \\ \big(\hat{T} + \hat{V}_0\big) |\psi_k\rangle &= E_k |\psi_k\rangle,\end{aligned}$$
...
where $\sum_k$ denotes a sum over all free states.  Since the free
states form a continuum, this really ought to be an integral, similar
to how we treat momentum eigenstates (@sec-momentum-eigenstates).  For
ease of presentation, however, we will write it as a sum for now.

Using Dyson's equations, the causal Green's function can be written as

$$\begin{aligned}\hat{G} &= \hat{G}_0 + \hat{G} \hat{V}_1 \hat{G}_0,\\ \hat{G}_0(E) &= \lim_{\varepsilon\rightarrow0^+} \Big(E - \hat{T} - \hat{V}_0 + i\varepsilon\Big)^{-1}.\end{aligned}\\$${#eq-Gdyson}

Note that $\hat{G}_0$ here denotes the causal Green's function for the
system with potential $\hat{V}_0$, not the free-space Green's function
used in @sec-green-fun; nonetheless, you can check that this Dyson
equation is correct.  Now, if we can obtain $\hat{G}(E)$, we can
determine the scattering amplitudes.  Let us therefore calculate its
matrix elements, using $|\varphi\rangle$ and $\{|\psi_k\rangle\}$ as a
convenient basis.
...
$$\begin{aligned} \hat{G} &= \hat{G}_0 + \hat{G} \hat{I} \hat{V}_1 \hat{G}_0 \\ &= \hat{G}_0 + \hat{G} \left(|\varphi\rangle\langle\varphi|
+ \sum_k\, |\psi_k\rangle\langle\psi_k|\right) \hat{V}_1 \hat{G}_0. \end{aligned}$$
...
\langle\varphi|\hat{G}|\varphi\rangle \left(1 - \frac{\langle\varphi|\hat{V}_1|\varphi\rangle}{E - E_0 + i\varepsilon}\right) &= \lim_{\varepsilon\rightarrow0^+} \frac{1}{E  - E_0 + i\varepsilon} \left(1+ \sum_k \, \langle\varphi|\hat{G}|\psi_k\rangle \, \langle\psi_k| \hat{V}_1|\varphi\rangle \right)\end{aligned}$$

After multiplying both sides by $E - E_0 + i\varepsilon$, we arrive at

$$\lim_{\varepsilon\rightarrow0^+} \langle\varphi|\hat{G}|\varphi\rangle \Big(E - E_0 -\, \langle\varphi|\hat{V}_1|\varphi\rangle \, + i\varepsilon\Big) - \sum_k\, \langle\varphi|\hat{G}|\psi_k\rangle \, \langle\psi_k| \hat{V}_1|\varphi\rangle = 1.$$ {#eq-psiGphi}
...
So far, the equations have all been exact, but now we apply an
approximation.  In the last line of the above equation, let us assume
the factor of $\langle\varphi|G|\varphi\rangle$ is large, so that the
first term in the sum is dominant.  It will be shown below that
...
Combining this with @eq-psiGphi gives
...
After pulling out the common factor of
$\langle\varphi|\hat{G}|\varphi\rangle$, and moving everything to the
right side, we finally obtain
...
\end{aligned}$$ {#eq-phiGphi_result}

This is the key result we wanted to derive, but it contains an
enigmatic quantity $\Sigma(E)$.  Before proceeding, let us spend some
time understanding what $\Sigma(E)$, which is called the **self
energy**, represents.

### The self energy
<!-- \label{sec:self_energy} -->

Let us look carefully at the expression for
$\langle\varphi|\hat{G}|\varphi\rangle$ on the first line of
@eq-phiGphi_result.  If we treat the self-energy $\Sigma$ as a
constant rather than an $E$-dependent function, then this formula
looks like the Green's function for a simple system containing a
single energy eigenstate $|\varphi\rangle$, whose eigenenergy is

$$E_0 + \langle\varphi|\hat{V}_1|\varphi\rangle + \Sigma.$$

It is natural to interpret this as the energy of the quasi-bound
state.  The first term is the energy of the original bound state, and
the second term is an energy shift coming from the deconfining
potential $\hat{V}_1$.  The third term, the self energy $\Sigma$, can
be interpreted as an extra contribution arising from the interplay
between $|\varphi\rangle$ and the free states $\{|\psi_k\rangle\}$
(see the second line of @eq-phiGphi_result): the presence of the free
states exerts a collective effect upon the quasi-bound state,
modifying its energy.

There is an important complication: $\Sigma$ turns out to be complex,
not real!  From the second line of @eq-phiGphi_result, taking the
...
width $2\varepsilon$.  The area under the curve is $\pi$, independent
...
localized state that is *almost*, but not exactly, an energy
eigenstate.  Therefore, after some time, it escapes the scatterer and
disappears to infinity.

## Scattering resonances {#sec-scattering_resonances}

In @sec-3d-scat, we derived the following relationship between the
...
$$\begin{aligned}\Delta f(\mathbf{k}\rightarrow\mathbf{k}') &\propto \langle \mathbf{k}'|\hat{V}|\varphi\rangle\langle\varphi|\hat{G}|\varphi\rangle\langle\varphi|\hat{V}|\mathbf{k}\rangle \;=\; \frac{\langle \mathbf{k}'|\hat{V}|\varphi\rangle \, \langle\varphi|\hat{V}|\mathbf{k}\rangle}{\displaystyle E - E_{\mathrm{res}} - i \mathrm{Im}[\,\Sigma\,]}, \\
E_{\mathrm{res}} &\equiv E_0 + \langle\varphi|\hat{V}_1|\varphi\rangle + \mathrm{Re}\big[\,\Sigma\,\big] \;\in\; \mathbb{R}.\end{aligned}$$ {#eq-deltaf}

At resonance, the denominator is small so $\Delta f$ is the dominant
part of $f$.  (Note, by the way, that $\Delta f$ gets contributions
from all terms in the Born series, not just low-order terms; i.e.,
this expression is not limited to weak scatterers!)  The graph below
shows how it varies with energy:

![](fig/resonance.png){fig-align="center" width="500" fig-alt="Graphs of the differential scattering amplitude's squared amplitude (left) and phase (right), versus energy.  The former exhibits a Lorentzian peak, and the latter a phase shift."}
...
overlaid on a background caused by non-resonant processes.  This can
be seen, for example, in the plot below from the CMS experiment at the
Large Hadron Collider (LHC), declaring the experimental observation of
the Higgs boson in 2012.

![](fig/higgs.png){fig-align="center" width="500" fig-alt="Screencap of an experimental result from the LHC showing a resonance corresponding to the Higgs boson."}

## Fermi's Golden Rule {#sec-goldenrule}
...
circumstances, called **Fermi's Golden Rule**.

### Derivation from Green's functions
...
To quantify the decay of the quasi-bound state, we imagine performing
a measurement that has $|\varphi\rangle$ as one of its eigenstates, at
time $t > 0$.  The probability that the particle is observed to still
be in state $|\varphi\rangle$ is

$$P(t) = \Big|\langle\varphi|\exp\left(-i\hat{H}t/\hbar\right)|\varphi\rangle\Big|^2.$$

To help us calculate $P(t)$, let us define

$$f(t) = \begin{cases} \langle\varphi|\exp\left(-i\hat{H}t/\hbar\right)|\varphi\rangle \,e^{-\varepsilon t}, & t \ge 0 \\ 0, & t < 0,\end{cases}$$ {#eq-fregulated}
...
function is designed so that it vanishes at times prior to start of
the experiment, as well as vanishing as $t\rightarrow\infty$ (due to
...
Now insert a resolution of the identity, $\hat{I} = \sum_n |n\rangle\langle n|$, where $\{|n\rangle\}$ denotes the exact eigenstates of $\hat{H}$ (for a continuum of free states, the sum goes to an integral in the usual way):
...
(@sec-3d-propagator).  Its appearance can be traced to @eq-fregulated,
which defines $f(t)$ to be nonzero only for $t \ge 0$.

In @sec-scattering_resonances, we saw that when the system is at or
close to resonance,
...
$$\begin{aligned} \lim_{\varepsilon\rightarrow 0^+} f(t) &= \lim_{\varepsilon\rightarrow 0^+} \int_{-\infty}^{\infty} \frac{d\omega}{2\pi} \; e^{-i\omega t} \, F(\omega) \\
  ...
  &\approx \exp\left(-\frac{iE_{\mathrm{res}}t}{\hbar}\right)\, \exp\left(-\frac{|\mathrm{Im}[\Sigma]|}{\hbar}\,t\right), \end{aligned}$$
...
(@eq-fermigr1), which is reproduced here for convenience:

$$\mathrm{Im}\big[\Sigma(E)\big]
= - \pi \sum_k \; \big| \langle\psi_k| \hat{V}_1|\varphi\rangle\big|^2
\; \delta(E-E_k).$$ {#eq-fermigr1a}

In the sum over $k$, each term is a non-negative real number
consisting of two factors: a $k$-dependent weight $| \langle\psi_k|
\hat{V}_1|\varphi\rangle|^2$, and a delta function that picks out the
energy $E_k = E$.  Because only a subset of the free states are
relevant to the sum, it is convenient to define
...
those with $E = E_k$).  Then we can approximate @eq-fermigr1a by
pulling the weights out of the sum:

$$\mathrm{Im}\big[\Sigma(E)\big]
\approx - \pi \; \overline{\big| \langle\psi_{k(E)}| \hat{V}_1|\varphi\rangle\big|^2} \; \sum_k \; \delta(E-E_k).$$
<!-- \label{fermigr2} -->

This allows us to write the decay rate as
...
\end{aligned}\right.$$ {#eq-FGR}

This result is Fermi's golden rule.  It states that the decay rate of
a quasi-bound state is determined by the product of two real factors:

* $\overline{|W|^2}$, which is obtained by taking the absolute square
  of the **transition amplitude** $\langle\psi_{k}|
...
approximations.  One very common approximation is to treat the free
states as plane waves.  In this case, we take the $k$ labels to be
$d$-dimensional wave-vectors.  As the wave-vectors are continuous, we
convert the sum over $k$ into an integral in the usual way, by
defining the discretization step $dk = 2\pi/L$ where $L \rightarrow
\infty$ (see @sec-momentum-eigenstates):

$$\begin{aligned} \rho(E) &= \sum_k \delta(E-E_k) \nonumber \\
&= L^d \underbrace{\int \frac{d^d k}{(2\pi)^d} \; \delta(E - E_k).}_{\equiv \displaystyle \mathcal{D}(E)}\end{aligned}$$ {#eq-densityofstates}

The quantity $\mathcal{D}(E)$ is called the **density of states**, and
it counts the number of free states of energy $E$, per unit energy and
per unit volume.  Note that it has different dimensions from
$\rho(E)$.  We also need to re-normalize the transition amplitude, via
the usual delta-normalization convention:
...
&= \left(\frac{2\pi}{L}\right)^{d/2} \langle\psi_{k}| \hat{V}_1|\varphi\rangle^{(\textrm{new})}. \end{aligned}$$ {#eq-psiknew}

Plugging @eq-densityofstates and @eq-psiknew into the Fermi's golden
rule formula @eq-FGR, we can re-state the golden rule as
...
$$\kappa
    ...
    \end{aligned}\right.$$ {#eq-FGR2}

This result no longer contains $L$, since the factors in
@eq-densityofstates and @eq-psiknew have cancelled (which is the point
of the delta-normalization and new definitions).  Note that the
newly-defined $\mathcal{W}$ contains a $(2\pi)^{d/2}$ factor that
accounts for the factors in the delta-normalized $|\psi_k\rangle$ and
newly-defined $\mathcal{D}(E)$.  As before,
$\overline{\big|\mathcal{W}\big|^2}$ is averaged over all plane waves
satisfying the resonance condition $E_k = E$.

### Example: resonance in a 1D potential
<!-- \label{sec:fgr1d} -->

Let us apply Fermi's golden rule to the simple 1D model of
@sec-resonances.  The potential is
...
we can derive $E_0$, $\mathcal{A}$, and $\mathcal{B}$.  Next, we want
the transition amplitude $\langle\psi_k|\hat{V}_1|\varphi\rangle$,
where $\psi_k(x)$ is a representative free eigenstate of $V_0(x)$ that
...
\;\approx\; 2 V_b^2 \;
\left| \int_{b}^\infty dx \, \psi_k^*(x)\, \varphi(x)\right|^2.$$ {#eq-overlapintegral}
...
Plugging this into @eq-overlapintegral and solving the integral, we
obtain

$$\big| \langle\psi_k|\hat{V}_1|\varphi\rangle \big|^2
\;\approx \; \frac{1}{\pi}\, V_b^2 \, \mathcal{B}^2\,
\frac{e^{-2\eta b}}{k^2 + \eta^2}.$$ {#eq-overlap1d}
...
&= 2 \cdot \frac{1}{2\pi} \int_0^\infty dE' \, \frac{dk}{dE'} \, \delta(E-E') \\
&= \sqrt{\frac{m}{2\pi^2\hbar^2 E}}.\end{aligned}$$ {#eq-dos1d}

(In the second line, the factor of 2 accounts for the fact that for
each $E$, both positive and negative $k$ contribute to the density of
states.)  We can now plug @eq-overlap1d and @eq-dos1d into Fermi's
golden rule, @eq-FGR2.  The figure below shows results for $U = 6$,
$V_b = 30$, and $a = 1$ (using the usual computational units $\hbar =
m = 1$).  The orange curve shows how $\kappa$, the decay rate from
Fermi's golden rule, varies with the barrier thickness $b-a$.  For
comparison, the blue dots show the values of $\kappa$ derived from the
resonant scattering peak (see @sec-resonances), by computing the
transmittance using the transfer matrix method and numerically
extracting the peak width.

![](fig/goldenrule.png){fig-align="center" width="500" fig-alt="Plot of decay rate versus barrier width, using two methods: the width of the scattering peak width, and Fermi's golden rule"}
...
useful since the scattering problem can be solved easily.  However, in
higher dimensions, nonperturbative scattering problems are typically
much less tractable.  In such cases, Fermi's golden rule provides a
very convenient way to estimate the widths of resonance peaks.
...
$$\begin{aligned}
  ...
  \;+\, B \int_{-\infty}^\infty dx\; V(x) \;e^{-2\gamma x^2}
  \right],
\end{aligned}$$
...
2. In this problem, you will investigate the existence of bound states
  in a 3D potential well that is finite, uniform, and
  spherically-symmetric.  The potential function is
$$V(r,\theta,\phi) = -U\Theta(a-r),$$
where $a$ is the radius of the spherical well, $U$ is the depth, and
$(r,\theta,\phi)$ are spherical coordinates defined in the usual way.
...
where $Y_{\ell m}(\theta,\phi)$ are [spherical harmonics](https://en.wikipedia.org/wiki/Spherical_harmonics),
and the integers $l$ and $m$ are angular momentum quantum numbers
satisfying $l \ge 0$ and $-l \le m \le l$.

    Substituting into the Helmholtz equation yields
$$r^2\frac{d^2f}{dr^2} + 2r \frac{df}{dr}+\left[q^2r^2-l(l+1)\right] f(r) = 0,$$
    which is the spherical Bessel equation.  The solutions to
    this equation that are non-divergent at $r = 0$ are $f(r) =
    j_\ell(qr)$, where $j_\ell$ is a spherical Bessel function
    of the first kind.

    Similarly, solutions for the second equation can be written as
    $\psi(r,\theta,\phi) = g(r) \, Y_{\ell m}(\theta,\phi),$ yielding an
    equation for $g(r)$ called the modified spherical Bessel
