...
Note, however, that this is *not* the energy eigenbasis of
$\hat{H}$.
...
the factor of $\langle\varphi|G|\varphi\rangle$ be large, so that the
...
right side, we finally arrive at the result
...
The enigmatic quantity $\Sigma(E)$ is called the **self energy**, and
we will spend the next section investigating its meaning.
...
constant rather than an $E$-dependent function, then this looks like
the formula for the Green's function for a simple system: a single
energy eigenstate $|\varphi\rangle$, whose eigenenergy is

$$E_0 + \langle\varphi|\hat{V}_1|\varphi\rangle + \Sigma.$$

We can interpret this as the energy of the quasi-bound state.  The
first term is the energy of the original bound state, and the second
term is an energy shift coming from the deconfining potential
$\hat{V}_1$.  This implies that the third term, the self energy
$\Sigma$, is an extra contribution arising from the interplay between
$|\varphi\rangle$ and the free states $\{|\psi_k\rangle\}$ (see the
second line of @eq-phiGphi_result).  As the quasi-bound state disturbs
the free states, the free states collectively act back upon it,
shifting its energy.
