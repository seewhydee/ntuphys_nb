...
solutions called **bound states**. To understand how such solutions
arise, note that outside the potential well ($|x| > a$), the
time-independent Schrödinger equation reduces to

$$-\frac{\hbar^2}{2m}\,\frac{d^2\psi}{dx^2} = E \psi(x) \qquad (\text{for}\;\;|x| > a).$$

Hence, the wavefunctions can be set to exponentials that decay away
from the potential well:

$$\psi(x) = c_\pm\, e^{\mp\kappa x},\;\;\;\mathrm{where}\;\;
c_\pm \in \mathbb{C}, \;\; \kappa = \sqrt{-\frac{2mE}{\hbar^2}} \in \mathbb{R}^+.$$
...
a discrete set of eigenenergies within the range $-U \le E < 0$.
...
\quad\mathrm{where}\;\;\;
k = \sqrt{\frac{2mE}{\hbar^2}} \in \mathbb{R}^+.$${#eq-psiwaves}

Such a solution is called a **free state**.  It cannot be normalized
to unity since $\int_{-\infty}^\infty |\psi(x)|^2 dx$ diverges.
...
For $U = 30$, there are five bound states.  As we make the well
shallower, these bound states disappear one by one, until there is
only one left.  (This single bound state persists as $U \rightarrow
0$; there is a theorem that any purely-attractive 1D potential must
support at least one bound state.  See [Exercise
1](#sec-resonances-exercises).)

Many features of this simple model are generalizable to more
complicated cases, including higher spatial dimensions.  However, in
3D, an potential well that is too shallow might not support any bound
states.  This can be seen in the figure below, where we plot the bound
state energies for a 3D uniform spherical well of radius $R=1$ (the
$l$ labels are angular momentum quantum numbers):
