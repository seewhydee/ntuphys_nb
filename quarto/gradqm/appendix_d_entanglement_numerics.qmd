# Numerical Tensor Products {#sec-appendix-d}
...
[NumPy](https://numpy.org/) module.  The
discussion is also applicable, with minor modifications, to GNU Octave
or MATLAB.  We assume the reader is familiar with the
[basics of Python/NumPy](https://docs.scipy.org/doc/numpy/user/quickstart.html), e.g.~how vectors can be represented by 1D arrays,
linear operators (matrices) can be represented by 2D arrays, etc.

Tensor products are implemented by the [`numpy.kron`](https://docs.scipy.org/doc/numpy/reference/generated/numpy.kron.html)
function, which performs an operation called a **Kronecker
...
following NumPy expressions involving [`numpy.kron`](https://docs.scipy.org/doc/numpy/reference/generated/numpy.kron.html):
...
$$\texttt{kron}(a, b) = \Big(a_0b_0,\,\dots,\, a_0 b_{N-1},\, a_1 b_0,\, \dots,\, a_1 b_{N-1},\, \dots,\, a_{M-1}b_{N-1}\Big).$${#eq-appD-kronabdef1}
We can think of this as *taking each component of $a$, and
  multiplying it by the entire $b$ array*:
$$\texttt{kron}(a, b) = \Big(a_0b,\, a_1 b,\, \dots,\, a_{M-1}b\Big).$${#eq-appD-kronabdef2}
...
$$\big[\, \texttt{kron}(a, b) \,\big]_{\mu} = a_m \, b_n \;\;\;\mathrm{where}\;\;\;\mu = mN+n.$${#eq-appD-kronab}
...
@eq-appD-kronabdef1 and @eq-appD-kronabdef2.
...
$$\Big\{|\mu\rangle \equiv |m\rangle |n\rangle\Big\}
  ...
  \begin{cases}
    ...
    m \!\!\!\!&= 0,1,\dots,M-1 \\
    n \!\!\!\!&= 0,1, \dots, N-1.
  \end{cases}$${#eq-appD-mubasis}
Using @eq-appD-kronab, we can show that
...
$$|a\rangle\otimes|b\rangle \;\;\leftrightarrow\;\;
  \texttt{kron}(a,b).$${#eq-appD-result1}
Likewise, for bras,
$$\langle a| \otimes \langle b| \;\;\leftrightarrow\;\;
  \texttt{kron}(a^*,b^*).$${#eq-appD-result1a}
...
the form
$$\texttt{kron}(A,B) = \begin{bmatrix} A_{00}B & \cdots & A_{0,M-1}B \\ \vdots & \ddots & \vdots \\ A_{M-1,0}B & \cdots & A_{M-1,M-1}B\end{bmatrix}.$${#eq-appD-kronAB-explicit}
...
notation as
$$\big[\,\texttt{kron}(A,B)\,\big]_{\mu\mu'} = A_{mm'} B_{nn'}\;\;\;\mathrm{where}\;\;\;\mu = mN+n, \; \mu' = m'N+n'.$${#eq-appD-kronAB}
...
Then we can show using @eq-appD-kronAB that
$$\begin{aligned}
  \hat{A}\otimes\hat{B} &=
...
@eq-appD-mubasis.  Thus,
$$\hat{A}\otimes\hat{B} \;\;\leftrightarrow\;\;
  \texttt{kron}(A,B).$${#eq-appD-result2}
This result, like @eq-appD-result1, is nice because it means that
...
$$|a\rangle \otimes \hat{B} = \sum_{\mu n'} |\mu\rangle \,
  a_m B_{nn'} \, \langle n'|, \qquad \mu = mN + n.$${#eq-appD-aBprod}
This does not quite match @eq-appD-kronaB!  The basic problem is
that the Kronecker product treats $a$ as a row vector.  However, we can
patch things up by massaging @eq-appD-kronaB a bit:
$$\begin{aligned}
    \left.[\right.\texttt{kron}(a, B^T)^T]_{\mu n'} &=
    [\texttt{kron}(a, B^T)]_{n' \mu} \\
    &= a_{m} (B^T)_{n'n} \quad\;\;\mathrm{where}\;\; \mu = mN + n \\
    &= a_{m} B_{nn'}.
  \end{aligned}$$
This is an appropriate match for @eq-appD-aBprod, so we conclude
that
$$|a\rangle \otimes \hat{B} \;\;\leftrightarrow\;\;
  \texttt{kron}(a, B^T)^T.$$
...
Comparing this to @eq-appD-kronaB yields
$$\langle a| \otimes \hat{B} \;\;\leftrightarrow\;\;
  \texttt{kron}(a^*, B).$$
