...
An unbounded $d$-dimensional space is described by coordinates
$\mathbf{r}$, with a scatterer near $\mathbf{r} = 0$.  An incoming
quantum particle, with energy $E$, is governed by the Hamiltonian
...
momentum operators, and $V$ is a **scattering potential**
...
1. A particle state $|\psi\rangle$ obeys the time-independent Schrödinger equation
$$\hat{H} |\psi\rangle = E |\psi\rangle,$$
where $E$ is the incoming particle energy.

2. The state $|\psi\rangle$ can be decomposed into two pieces,
...
4. The scattered state is an ``outgoing'' state.  What this means will
be explained later.
