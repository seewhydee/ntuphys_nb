...
In linear algebra, there is an concept called the **tensor product**
...
Yet Alice's measurement evidently affects Bob's.  Could this be a
...
In the present scenario, Alice and Bob each experiences a 50-50
...
whereas if she got $-z$, the hidden-variable state was $[-|+]$.  When
...
where the sum runs over all 8 possibile hidden-variable states.  We
...
of **quantum cryptography** is poised to be one of the most
...
state, we call it a **mixed state**, and this implies that the
...
2. Consider two spin-1/2 subsystems, with the density operator
