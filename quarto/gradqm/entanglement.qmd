# Quantum Entanglement {#sec-chapter-entanglement}
...
and counterintuitive phenomena, such as **quantum entanglement**.

## States of multi-particle systems
...
In linear algebra, there is a concept called the **tensor product**
that happens to be exactly what we need.  Given $|a\rangle \in
\mathscr{H}_A$ and $|b\rangle \in \mathscr{H}_B$, consider an ordered
pair of these two vectors, which we call "the tensor product of
$|a\rangle$ and $|b\rangle$".  We denote this by
...
$\mathscr{H}_B$, in the sense that they obey the factorization rules

$$\begin{aligned}\Big(\lambda |a\rangle
...
+ \lambda' |b'\rangle\Big) &=
\lambda |a\rangle \otimes |b\rangle \,+\,
\lambda' |a\rangle \otimes |b'\rangle.\end{aligned}$$ {#eq-tensorrules}

(Note: the expressions inside the parentheses are vector operations in
$\mathscr{H}_A$ and $\mathscr{H}_B$ respectively.)  These are
reminiscent of the rules of algebra, with $\otimes$ acting like
multiplication, except that $|a\rangle\otimes|b\rangle$ is not
...
But even as we write the tensor product this way, as a product of two
kets, the ordering remains important.  The left slot is implicitly
understood to contain the $A$-type ket, while the right slot contains
the $B$-type ket.

The space of all tensor products, *and linear combinations thereof*,
forms a Hilbert space.  We call this the **tensor product space** of
$\mathscr{H}_A$ and $\mathscr{H}_B$, which is denoted by
...
The preceding factorization rules (@eq-tensorrules) imply that their
tensor product is
...
Conversely, we can take this as the definition of the tensor product,
which is consistent with the basis of @eq-c4basis, and use it to
derive the factorization rules.  For instance,
...
the four basis states in @eq-c4basis form an orthonormal set.

### General tensor products

The key features from the above example generalize to more complicated
...
(@eq-tensorrules), and construct a basis for the
...
The tensor product concept can also be extended to more than two
subsystems.  Given $N$ subsystems with Hilbert spaces $\mathscr{H}_1,
\mathscr{H}_2, \dots, \mathscr{H}_N$, the combined state space
is^[Here, we are assuming that the subsystems in question are
distinguishable.  There are other complications that arise if the
subsystems are "identical" particles.  This will be the subject of the
next chapter, so just read on.]
...
combined Hilbert space has dimension $2^{20} =1\,048\,576$.  (As a
consequence, quantum systems containing only a modest number of
...
$$|a\rangle |b\rangle$$ {#eq-tprod_generic}
...
products.  The latter are *not* guaranteed to have the form of
@eq-tprod_generic.

In quantum mechanics, a tensor product like @eq-tprod_generic is
...
This too is a valid vector in $\mathscr{H}_A \otimes \mathscr{H}_B =
\mathbb{C}^2\otimes\mathbb{C}^2$.  However, it turns out to be
impossible to re-write it in the form $|a\rangle|b\rangle$!  (We will
prove this later, in @sec-entropy.)  In such a situation, $A$ and $B$
do not individually have stand-alone quantum states; only the combined
system has a well-defined quantum state, $|\psi\rangle$.  We then say
$A$ and $B$ are **entangled**.  Entanglement leads to many
counterintuitive phenomena, which we will explore in the rest of this
chapter.

## Partial measurements {#sec-partialmeasurements}
...
$$|\psi\rangle = \sum_q |q\rangle \psi_q, \;\;\mathrm{where}\;\, \psi_q = \langle q|\psi\rangle.$$ {#eq-psiexpansion}

The measurement postulate of quantum mechanics states that if we
measure $Q$, then:

* The probability of obtaining each possible outcome $q$ is
$$P(q) = |\psi_q|^2.$$ {#eq-Pq}
* Upon obtaining this outcome, the system collapses into state
$|q\rangle$.

Mathematically, these two rules can be summarized using the projection
operator

$$\hat{\Pi}(q) = |q\rangle\langle q|.$$ {#eq-project}
...
* The probability of obtaining this outcome, which is $P(q) = \langle\psi'|\psi'\rangle = |\langle q|\psi\rangle|^2$.

* The post-measurement state, which is obtained by renormalizing
$|\psi'\rangle$ to $|q\rangle$.


For combined systems (e.g., systems of multiple particles), there is a
new complication: *what if a measurement is performed on just one
subsystem*?

Suppose, once again, that we have two subsystems $A$ and $B$, whose
combined Hilbert space is $\mathscr{H}_A \otimes \mathscr{H}_B$.  Let
...
&= \sum_\alpha |\alpha\rangle \underbrace{\left( \sum_\beta \psi_{\alpha\beta}\,|\beta\rangle\right)}_{\displaystyle \equiv |\varphi_\alpha\rangle}.\end{aligned}$$ {#eq-psi_twop}

Evidently, the vector $|\varphi_\alpha\rangle \in \mathscr{H}_B$
appearing in this equation plays a role similar to $\psi_q$ in
@eq-psiexpansion, except that the former is a vector and the latter is
a scalar.  By analogy with the probability rule from @eq-Pq, the
probability for measurement outcome $\alpha$ ought to be

$$P(\alpha) = \langle\varphi_\alpha|\varphi_\alpha\rangle
= \sum_\beta |\psi_{\alpha\beta}|^2.$$ {#eq-Palpha0}

To make this prediction more solid, recall how we previously showed
that each measurement outcome is associated with a projection operator
as given by @eq-project.  For a partial measurement, we can
accordingly define the **partial projector**

$$\hat{\Pi}(\alpha) \,=\, |\alpha\rangle\langle \alpha| \otimes  \hat{I}.$$

The $A$ slot contains the projection operator associated with the
measurement outcome $\alpha$.  The $B$ slot contains the identity
operator for $\mathscr{H}_B$, indicating that no measurement is being
done on $B$.  Applying this partial projector to @eq-psi_twop gives
...
in agreement with @eq-Palpha0.  The post-measurement state is found by
...
### Example: a singlet state {#sec-singlet}

Suppose a system of two spin-$1/2$ particles is in a so-called
**singlet state**,
...
::: {.callout-note}
In the rest of this chapter, unless stated otherwise, we will report
the spin-1/2 eigenvalues $+\hbar/2$ and $-\hbar/2$ as $+$ and $-$
respectively.  In other words, we won't write the $\hbar/2$.
:::

If we measure $S_z$ on $A$, what is the probability of each outcome,
and the post-measurement state?  Based on our preceding discussion,
the options are as follows:

* **First outcome**: $+$.
    * The projected state is $\displaystyle |\psi'\rangle = \left(|\!\uparrow\rangle\langle\uparrow| \otimes \hat{I}\right)|\psi\rangle =
\frac{1}{\sqrt{2}}\,|\!\uparrow\rangle|\!\downarrow\rangle$.
    * The outcome probability is $\displaystyle
P(+) = \langle \psi'|\psi'\rangle = \frac{1}{2}$.
    * The post-collapse state is $\displaystyle |\!\uparrow\rangle |\!\downarrow\rangle$
* **Second outcome**: $-$.
    * The projected state is $\displaystyle |\psi'\rangle = \left(|\!\downarrow\rangle\langle\downarrow| \otimes \hat{I}\right)|\psi\rangle =
-\frac{1}{\sqrt{2}}\,|\!\downarrow\rangle|\!\uparrow\rangle$.
    * The outcome probability is $\displaystyle P(-) = \langle \psi'|\psi'\rangle = \frac{1}{2}$.
    * The post-collapse state is $\displaystyle |\!\downarrow\rangle |\!\uparrow\rangle$.
...
$B$ are no longer entangled after the collapse.

## The Einstein-Podolsky-Rosen paradox

In 1935, Einstein, Podolsky, and Rosen (EPR) formulated a thought
experiment, now called the **EPR paradox**, highlighting the
counter-intuitive features of quantum entanglement [@epr].  They intended for
this to show that quantum theory cannot be a fundamental description
of reality.  Subsequently, it was shown that the EPR paradox is not an
actual paradox---physical systems *really do* behave like the thought
experiment!  Nowadays, the EPR "paradox" is regarded not as an
argument against quantum mechanics, but as an important tool for
...
the tensor product respectively.  We prepare $|\psi\rangle$ in a
laboratory on Earth.  Then, *very carefully* (so as not to disturb the
singlet state), we transport particle $A$ to Alice in the Alpha
Centauri star system, and particle $B$ to Bob in the Betelgeuse
system:

![](fig/epr.png){fig-align="center" width="500" fig-alt="Schematic of an EPR experiment, showing how a singlet state's two spins are transported to different star systems light years apart"}

At a pre-scheduled time, Alice measures $S_z$ on her particle $A$.  As
we saw in @sec-singlet, her measurement yields two equally probable
outcomes.  If her result is $+$, the singlet state collapses to
$|\!\uparrow\rangle |\!\downarrow\rangle$; if her result is $-$, the
singlet state collapses to $|\!\downarrow\rangle|\!\uparrow\rangle$.
The postulates of quantum mechanics indicate that this collapse occurs
instantaneously.  Immediately afterward, Bob measures $\hat{S}_z$ on
his particle $B$.  He then obtains, with 100% probability, the result
opposite to Alice's.

During the time interval between Alice's and Bob's measurements, no
classical signal could have passed between them, even at light speed.
Yet Alice's measurement apparently affects Bob's.  Could this be a
violation of the theory of relativity?

Before answering, let us pause to note that this thought experiment
dispels some commonsensical but mistaken explanations for state
collapse.  For instance, it is sometimes claimed that in order to
measure a particle's position, we must do something like shining light
on it, which disturbs the particle's state.  The EPR thought
experiment shows that such stories do not capture the full weirdness
of quantum measurement.  We can collapse a particle's quantum state
without interacting directly with it, but by doing a measurement on
*another* particle far away!

Now, let us get back to the question of whether Alice and Bob have
violated relativity.  Before concluding that a violation has occurred,
we must prove that Alice and Bob can use state collapse to send
signals to each other.  After all, it is *faster-than-light
communication* that generates cause-and-effect inconsistencies in
relativity, like the [grandfather
paradox](https://en.wikipedia.org/wiki/Temporal_paradox#Grandfather_paradox).
In the present scenario, Alice and Bob each experience a 50-50
probability distribution (i.e., a coin flip) for the $+$ and $-$
outcomes.  Even if the experiment is repeated many times with many
independent singlet states, each participant just receives a string of
random bits, which carries no information and cannot be used for
communication.

Alice and Bob might try to get around this by varying their choice of
measurement.  For instance, Alice might try to send a signal by
choosing to measure either $S_z$ (as before), or $S_x$ .  The latter
would collapse the singlet state as follows:

$$\begin{aligned}
|\!\rightarrow\rangle \otimes |\!\leftarrow\rangle &\qquad(\textrm{if Alice got $S_x = +$}) \\
|\!\leftarrow\rangle \otimes |\!\rightarrow\rangle &\qquad(\textrm{if Alice got $S_x = -$}).\end{aligned}$$

Suppose there is a way for Bob to figure out, from this, that Alice
measured $S_x$ rather than $S_z$ (the method could even be
probabilistic, by having the two parties repeat the procedure on many
singlet states).  Then Alice would have successfully transmitted one
bit of information---i.e., the choice between $S_x$ and $S_z$.

Upon closer inspection, however, it turns out this scheme is *not*
workable.  The reason is that quantum states themselves cannot be
measured; only observables are measured.  By calculating the various
measurement probabilities, we can prove the following (see [Exercise
1](#sec-entanglement-exercises)):
...
or $-$) is 0.5, regardless of the choices of $n$ and $n'$.

* If $n = n'$, Bob always gets the opposite of Alice's result.

Either way, Bob always has equal probability to get $+$ or $-$, and he
cannot squeeze out any additional information about what Alice did.
The two participants therefore cannot set up a grandfather paradox,
nor any of the other strange scenarios enabled by faster-than-light
communication.

Still, Einstein, Podolsky, and Rosen were dissatisfied.  They argued
that the situation violates the *spirit* of relativity, if not the
letter.  After all, a central element of the theory---the quantum
state---is changing faster than the speed of light, even if that
change is not directly measurable.

They suggested an alternative: maybe quantum mechanics is an
approximation of some deeper theory, whose details are currently
unknown.  Such a **hidden-variable theory** could give the appearance
of quantum state collapse, but without any faster-than-light changes
actually happening.

Suppose, contrary to quantum theory, that each particle has a definite
value of $S_z$, i.e., $+$ or $-$.  For simplicity, let us denote these
options as $+$ or $-$.  Let us hypothesize that the two-particle
quantum state $|\psi\rangle$ is actually a statistical distribution of
two kinds of **hidden-variable states**: $[+|-]$ (i.e., $S_z = +$ for
$A$ and $S_z = -$ for $B$), or $[-|+]$ (vice versa).  When the state
...
![](fig/hiddenvariables.png){fig-align="center" width="500" fig-alt="Cartoon showing a singlet by drawing from a distribution of up/down and down/up hidden-variable states"}

When Alice measures $S_z$, the hidden variable is revealed.  If she
got $+$, that means the hidden-variable state must have been $[+|-]$,
whereas if she got $-$, the hidden-variable state was $[-|+]$.  When
Bob subsequently measures $S_z$, his result will be the opposite of
Alice's.  These outcomes were predetermined when the hidden-variable
state was drawn, so there is no physical effect traveling between
Alice and Bob akin to the instantaneous state collapse of quantum
theory.
...
## Bell's theorem {#sec-bell}
<!-- \label{sec:bell} -->

In 1964, J. S. Bell published a bombshell paper showing that the
predictions of quantum theory are inherently inconsistent with
hidden-variable theories [@bell].  The amazing thing about this proof, which
is called **Bell's theorem**, is that it requires no knowledge about
the details of the hidden-variable theory, merely the fact that it is
deterministic and local.  Here, we present a streamlined version of
Bell's theorem due to Mermin [@mermin].

Yet again, we consider pairs of spin-1/2 particles, with particle $A$
sent to Alice and particle $B$ to Bob, with the two-particle singlet
state

$$|\psi\rangle = \frac{1}{\sqrt{2}} \Big(|\!\uparrow\rangle|\!\downarrow\rangle \,-\, |\!\downarrow\rangle|\!\uparrow\rangle\Big).$${#eq-bellsinglet}
<!-- \label{bellsinglet} -->
...
experimentalist chooses $S_1$, $S_2$, or $S_3$, *randomly and
...
axis choices and results.  Here is an example:

```{=html}
<table>
    <tr>
	  <th rowspan="2"></th>
	  <th colspan="2" style="text-align: center; color: #1445a0">Alice</th>
	  <th colspan="2" style="text-align: center; color: #aa5800">Bob</th>
    </tr>
    <tr>
	  <th>Axis</th>
	  <th>Result</th>
    ...
    <tr>
	  <td>Round 1</td>
	  <td style="color: #1445a0"><em>S</em><sub>1</sub></td>
	  <td style="color: #1445a0; font-size: 120%">&minus;</td>
	  <td style="color: #aa5800"><em>S</em><sub>2</sub></td>
	  <td style="color: #aa5800; font-size: 120%">+</td>
    </tr>
    <tr>
	  <td>Round 2</td>
	  <td style="color: #1445a0"><em>S</em><sub>3</sub></td>
	  <td style="color: #1445a0; font-size: 120%">+</td>
	  <td style="color: #aa5800"><em>S</em><sub>2</sub></td>
	  <td style="color: #aa5800; font-size: 120%">+</td>
    </tr>
    <tr>
	  <td>Round 3</td>
	  <td style="color: #1445a0"><em>S</em><sub>2</sub></td>
	  <td style="color: #1445a0; font-size: 120%">+</td>
	  <td style="color: #aa5800"><em>S</em><sub>1</sub></td>
	  <td style="color: #aa5800; font-size: 120%">&minus;</td>
    </tr>
    <tr>
	  <td>Round 4</td>
	  <td style="color: #1445a0"><em>S</em><sub>3</sub></td>
	  <td style="color: #1445a0; font-size: 120%">&minus;</td>
	  <td style="color: #aa5800"><em>S</em><sub>3</sub></td>
	  <td style="color: #aa5800; font-size: 120%">+</td>
    </tr>
    <tr>
	  <td>Round 5</td>
...
axis $S_3$ during Round 4, and Alice got $-$, while Bob got $+$.

Suppose a hidden-variable theory aims to reproduce this behavior.  For
instance, a hidden-variable state might consist of $A$ having the spin
observables $S_1 = +$, $S_2 = +$, and $S_3 = -$, while $B$ has the
corresponding opposite values; we can denote this state as
$[++-|--+]$.  In total, there are eight possible hidden-variable
states:
...
The hidden-variable theory postulates that the two-particle singlet
state is actually some kind of statistical distribution involving
these eight states:

$$\Big\{ P\big([+++|---]\,\big),
\;P\big([++-|--+]\,\big), \dots, P\big([---|+++]\,\big)
\Big\}.$$

Let us focus on the rounds in which Alice and Bob *chose different
spin axes*.  Within this subset, what is the probability *for their
measurement results to have opposite signs*?  To answer this question,
we first consider the following six states:
...
These are the cases for which each particle's spins are not all $+$ or
all $-$.  Take the first case, $[++-|--+]$.  Under our assumption that
Alice and Bob chose different measurement axes, there are two choices
that give opposite measurement signs: $(S_1,S_2)$ or $(S_2,S_1)$.
Conversely, there are four choices that give the same sign:
$(S_1,S_3)$, $(S_2,S_3)$, $(S_3,S_1)$ and $(S_3, S_2)$.  Since the
axis choices are random, the probability to obtain opposite signs is
1/3.  Going through the rest of the six hidden-variable states listed
above, we find that the probability to get opposite signs is also 1/3
for each of those cases.  In the notation of conditional
probabilities,

$$P\big(\textrm{opp.} | [++-|--+]\,\big) =
  P\big(\textrm{opp.} | [+-+|-+-]\,\big) = \cdots
  = \frac{1}{3},$$ {#eq-bell1}

where "opp." stands for opposite signs being obtained in the two
measurement results.

Next, consider the two remaining hidden-variable states,
...
$$P\big(\textrm{opp.} | [+++|---]\,\big) =
P\big(\textrm{opp.} | [---|+++]\,\big) = 1.$$ {#eq-bell2}
...
where the sum runs over all 8 possible hidden-variable states.  We
can use @eq-bell1 and @eq-bell2 to insert the conditional
probabilities, resulting in

$$\begin{aligned}
P\big(\textrm{opp.}\big) &=
\frac{1}{3} \Big[\underbrace{P\big([++-|--+]\,\big)
+ P\big([+-+|-+-]\,\big)
+ \cdots + P\big([--+|++-]\,\big)}_{\textrm{First 6 hidden-variable states}}\Big] \\
&\quad + \underbrace{P\big([+++|---]\,\big)
+ P\big([---|+++]\,\big)}_{\textrm{Last 2 hidden-variable states}} \\
&\ge
\frac{1}{3} \Big[\underbrace{P\big([++-|--+]\,\big)
+ \cdots + P\big([---|+++]\,\big)}_{\textrm{all 8 hidden-variable states}} \Big].\end{aligned}$$
<!-- \label{bellprob} -->

The probabilities in the square brackets must sum to one, so

$$P\big(\textrm{opp.}\big) \ge \frac{1}{3}.$$
<!-- \label{bellresult} -->

This result is called **Bell's inequality**.  It can be summarized as
follows:

::: {.callout-note}
**Bell's inequality.** In a hidden-variable theory, for the rounds where Alice and Bob choose different spin axes, their measurement results have opposite signs with probability $\ge 1/3$.
:::
...
![](fig/bellaxes.png){fig-align="center" width="250" fig-alt="Three spin measurement axes S1, S2, and S3 in the x-z plane, separated by 120 degrees"}

Referring to this diagram, we can express the three spin operators as

$$\begin{aligned}
\hat{S}_1 &= \frac{\hbar}{2} \, \hat{\sigma}_3 &&= \hat{S}_z \\
...
\end{aligned}$$ {#eq-bell-axes}

where $\hat{U}$ is the following unitary rotation operator:

$$\hat{U} = e^{(i\pi/3) \hat{\sigma}_2} = \begin{pmatrix} 1/2 & \sqrt{3}/2\\ -\sqrt{3}/2 & 1/2 \end{pmatrix}.$$ {#eq-Uform}

Suppose Alice chooses $S_1$, while Bob chooses $S_2$.  Note that the
eigenstates of $\hat{S}_2$ are

$$|\!\pm \!s_2\rangle = \hat{U}^\dagger |\!\updownarrow\rangle,$$ {#eq-s2eigs}

since

$$\hat{S}_2 \left(\hat{U}^\dagger |\!\updownarrow\rangle\right) = \hat{U}^\dagger \hat{S}_z \hat{U} \left(\hat{U}^\dagger |\!\updownarrow\rangle\right) = \pm \frac{\hbar}{2} \; \hat{U}^\dagger |\!\updownarrow\rangle.$$
...
$$\begin{aligned}
P(\textrm{opp}.) &= \frac{1}{2} \, \big|\langle -s_2 | \!\downarrow\rangle\big|^2 + \frac{1}{2} \, \big|\langle +s_2 | \!\uparrow\rangle\big|^2 \\
&= \frac{1}{2} \left( \big|\langle \downarrow\! | \hat{U} | \!\downarrow\rangle\big|^2 + \big|\langle \uparrow \!| \hat{U} | \!\uparrow\rangle\big|^2 \right) \\
&= \frac{1}{4}.
\end{aligned}$$ {#eq-bell-opp}

In the second line, we have used @eq-s2eigs, and in the final line we
have used @eq-Uform.
...
result as @eq-bell-opp.  In other words, the probability for Alice and
Bob to obtain opposite results is $1/4$, so Bell's inequality is
violated!
...
But in the end, the experimental consensus is a clear *yes*: Nature
agrees with quantum mechanics, not the hidden-variable theories!  The
experimental evidence is reviewed in a paper by Aspect [@aspect].
...
in certain respects, than conventional cryptography.  **Quantum
cryptography** is poised to be one of the most important technological
applications of quantum entanglement.  Here, we describe one of the
earliest quantum cryptography schemes, devised by Ekert in 1991 [@ekert].
...
The scheme closely follows the Bell thought experiment from @sec-bell.
Alice and Bob take part in multiple rounds of
measurements; during each round, a pair of spin-$1/2$ particles is
prepared in the singlet state @eq-bellsinglet, with particle $A$
...
inequality, which would provide evidence that no eavesdropping has
taken place.
...
The so-called **no-cloning theorem** can be proven as follows.
...
$$\hat{U} |\psi\rangle | 0\rangle = e^{i\phi} |\psi\rangle |\psi\rangle$${#eq-clone}
...
their inner product.  According to @eq-clone,
$$\begin{aligned}
  ...
  &= e^{-i(\phi_1-\phi_2)} \Big( \langle\psi_1 | \psi_2\rangle \Big)^2. \end{aligned}$${#eq-clone1}
Here, $\phi_1$ and $\phi_2$ are the phase factors from
@eq-clone for the two chosen states.  On the other hand, since
$\hat{U}$ is unitary,
$$\begin{aligned}
  ...
  \\ &= \langle\psi_1 | \psi_2\rangle. \end{aligned}$${#eq-clone2}
Comparing the magnitudes of @eq-clone1 and @eq-clone2,
...
We now introduce **density operators**, which help streamline
...
@sec-partialmeasurements the probabilities of these
...
the **reduced density operator**, as follows:
$$\hat{\rho}_A = \mathrm{Tr}_B \,\big[\,\hat{\rho}\,\big].$${#eq-rhoa-def}
Here, $\hat{\rho}$ is the density matrix for the combined system, as
defined in @eq-rho-pure.  $\mathrm{Tr}_B[\cdots]$ denotes a
**partial trace**, which means tracing out the $B$ part of the
...
$$\hat{\rho}_A =
    ...
    \Big( \hat{I}_A \otimes | \beta\rangle \Big).$${#eq-rhoa-explicit}
...
@eq-rhoa-explicit, we end up with the same operator.
...
(@sec-partialmeasurements), the probability of obtaining
$\alpha$ is
$$\begin{aligned}
  P(\alpha)
    &=
    ...
    \end{aligned}$${#eq-rho-prob}
This result is clearly analogous to @eq-Pi-rho.  Hence, we can
...
$$\langle Q_A \rangle = \sum_\alpha \alpha
  \langle \alpha | \hat{\rho}_A | \alpha \rangle
  = \mathrm{Tr}\Big[\hat{Q}_A \, \hat{\rho}_A \Big],$${#eq-rho-expect}
which is analogous to @eq-Qexpt.
...
probabilities $\{P_\beta\}$.  This called an **ensemble** of
states.  Note that the states in the ensemble need not be orthogonal
to each other.

Although we have derived @eq-rhoform from $|\psi\rangle$, the
...
@eq-rho-pure, is the special case of an ensemble of one
quantum state $|\psi\rangle$ with probability $P = 1$.  We call such
an ensemble a **pure state**.  If an ensemble is not a pure
state, we call it a **mixed state**.

Using @eq-rhoform, we can show that the density matrix
$\hat{\rho}_A$ (be it for a pure state or a mixed state) has the
following properties:

1. $\hat{\rho}_A$ is Hermitian.

2. $\langle\varphi|\hat{\rho}_A|\varphi\rangle \ge 0$ for any
  $|\varphi\rangle \in \mathscr{H}_A$ (i.e., the operator is positive
  semidefinite).

3. For any observable $\hat{Q}_A$ acting on $\mathscr{H}_A$,
  $$\begin{aligned}
      \langle Q_A \rangle
      &= \mathrm{Tr}_A\left[\,\hat{Q}_A \,\hat{\rho}_A\,\right].
    \end{aligned}$${#eq-prop3}
  ...
  consistent with @eq-rho-prob.  To see this, take $\hat{Q}_A
  = |\alpha\rangle \langle \alpha|$.

4. The eigenvalues of $\hat{\rho}_A$, denoted by $\{p_1, p_2,
  \dots, p_{d_A}\}$, form a set of probabilities.  In other words,
  they satisfy
  $$p_n \in \mathbb{R} \;\;\;\mathrm{and}\;\; 0 \le p_n \le 1 \;\;
    \mathrm{for}\;\; n = 1,\dots,d_A,
    \quad\mathrm{with}\;\; \sum_{n=1}^{d_A} p_n = 1.$${#eq-trrho-reduced}
...
## Entanglement entropy {#sec-entropy}
...
**entanglement entropy**, which is closely related to the
...
define a quantity called the **entanglement entropy**:
$$S_{A} = - k_b \, \mathrm{Tr}_A \Big\{ \hat{\rho}_A\, \ln\!\big[\hat{\rho}_A\big]\Big\}.$${#eq-entropy}
...
$$S_{\mathrm{cl.}} = - k_b \sum_{i=1}^W p_i \ln(p_i).$${#eq-Scl}
It can be shown (see Appendix C) that for *any* probability
distribution,
...
The entanglement entropy @eq-entropy similarly aims to quantify
...
formula @eq-Scl, since they are basis-dependent.
@eq-entropy bypasses this problem by using the trace, which is
basis-independent.
...
@eq-trrho-reduced, the corresponding eigenvalues $\{p_n\}$ form a
set of probabilities.  Then
$$\begin{aligned}
...
$$0 \le S_A \le k_b\ln(d_A), \quad
  \mathrm{where}\; d_A = \mathrm{dim}\left[\mathscr{H}_A\right].$${#eq-Sabounds}
The lower bound, $S_A = 0$, holds if and only if one of the $p_n$'s
in @eq-SAmu is 1, and all the others are zero, meaning $A$ is
in a pure state.  Conversely, $S_{A} \ne 0$ when $\hat{\rho}_A$
describes a mixed state---i.e., $A$ is entangled with some other
quantum system.

A system is said to be **maximally entangled** if it saturates
the upper bound of @eq-Sabounds.  This occurs if and only if the
eigenvalues of the density operator are all equal to $1/d_A$.

::: {.callout-note}
**Example**---Consider the singlet state
$$|\psi\rangle = \frac{1}{\sqrt{2}} \Big(|\!\uparrow\rangle|\!\downarrow\rangle \,-\, |\!\downarrow\rangle|\!\uparrow\rangle\Big).$$
...
ideas related to the entanglement phenomenon: the **Many Worlds
  Interpretation** of quantum mechanics.

So far, when describing the phenomenon of state collapse, we have
relied on the measurement postulate (see
@sec-partialmeasurements), which is part of the
**Copenhagen Interpretation** of quantum mechanics.  This is how
...
1. It stands apart from the other postulates of quantum mechanics
  as the only place where randomness (or "indeterminism") creeps
  into quantum theory.  The other postulates do not refer to
  probabilities.  In particular, the Schrödinger equation
...
2. The measurement postulate is silent on what constitutes a
  ...
  Schrödinger equation, or performs a measurement?

The Many Worlds Interpretation of quantum mechanics [@everett] seeks to resolve
these problems by positing that the measurement postulate is
*not* a fundamental postulate of quantum mechanics.  It says
...
Schrödinger equation.  They can be derived, rather than posited, by
adopting quantum descriptions for both the system being measured
*and* the system doing the measurement.

We can use a toy model to illustrate this idea [@albrecht].
...
$$|\psi(0)\rangle = \Big(a_1 |\!\uparrow\rangle + a_2 |\!\downarrow\rangle\Big) \otimes |\Psi\rangle,$${#eq-psi0-albrecht}
...
of the form
$$\hat{H} = \hat{S}_z \otimes \hat{V}.$${#eq-V-albrecht}
This will only involve unitary time evolution via the Schrödinger
equation, *without* invoking the measurement postulate.  Also,
it turns out that we will not need to make any special choices for
$|\Psi\rangle$ in @eq-psi0-albrecht, or $\hat{V}$ in
@eq-V-albrecht, so we leave these unspecified for now.

The assumed Hamiltonian @eq-V-albrecht is time-independent, so the
state after some time $t$ is
$$|\psi(t)\rangle = e^{-i\hat{H}t/\hbar}\, |\psi(0)\rangle.$${#eq-mwi-psit}
To find this, observe how the Hamiltonian @eq-V-albrecht acts on
each term of the initial state defined in @eq-psi0-albrecht:
$$\begin{aligned}
...
Applying this to @eq-mwi-psit yields
$$|\psi(t)\rangle =
  a_1 |\!\uparrow\rangle |\Psi_\uparrow(t)\rangle + a_2 |\!\downarrow\rangle  |\Psi_\downarrow(t)\rangle,$${#eq-psit}
where
$$|\Psi_{\updownarrow} (t)\rangle = e^{\mp i\hat{V}t/2} |\Psi\rangle
  \;\;\in\, \mathscr{H}_A.$$

Based on @eq-psit, we can interpret $|\Psi_\uparrow(t)\rangle$
...
$$\langle\Psi_\uparrow(t) | \Psi_\downarrow(t)\rangle \rightarrow 0$${#eq-overlapzero}
after the passage of a short time $t$.

In the Many Worlds Interpretation, the two terms in @eq-psit
...
specified by the initial state @eq-psi0-albrecht.
...
$|\Psi\rangle$ in @eq-psi0-albrecht, and the operator
$\hat{V}$ in @eq-V-albrecht, in a generic way.  For
...
$$\hat{V} = \frac{1}{2\sqrt{d_A}} \left(\hat{A} + \hat{A}^\dagger\right),
  \;\; \mathrm{where} \;\; \mathrm{Re}(A_{ij}), \mathrm{Im}(A_{ij}) \sim N(0,1).$${#eq-gue}
...
theory of random matrices [@edelman].)  In this example, we
will choose $d_A = 100$.

Now we simply have to pick the inital state (say, $a_1 = 0.7$ and $a_2
= \sqrt{1-a_1^2}$), and solve the Schrödinger equation numerically
(for details, refer to Appendix D).  Then we can generate plots like
this:

![](fig/decoherence.png){fig-align="center" width="600" fig-alt="Numerical results for the Many Worlds toy model: the upper plot shows the squared overlap of the two apparatus states versus time, and the lower plot shows the entanglement entropy of spin and apparatus versus time"}
...
apparatus (see @sec-entropy).  At $t = 0$, this is zero,
consistent with the two subsystems being initially unentangled
[@eq-psi0-albrecht].  Subsequently, $S_A$ increases up to
...
universal state evolves according to the Schrödinger equation, it
repeatedly "branches" to produce more and more worlds, just like in
@eq-psit.  The world we inhabit, with its well-defined
...
Copenhagen Interpretation and the Many Worlds Interpretation have
...
## Exercises {#sec-entanglement-exercises}

1. Consider the singlet state
$$|\psi\rangle = \frac{1}{\sqrt{2}} \Big(|\!\uparrow\rangle|\!\downarrow\rangle \,-\, |\!\downarrow\rangle|\!\uparrow\rangle\Big),$$
...
eigenvalue $\pm\hbar/2$.
    ...
    a. Suppose we perform a measurement on $A$ using a rotated spin
  observable
...
that it is unitary.
    ...
    b. Denote the probabilities of the two outcomes in part (a) by
  $P(A_+)$ and $P(A_-)$.  Derive expressions for these two
  probabilities.

    c. For each of the two outcomes from measuring $S_n$ on $A$, find
...
2. A spin-1/2 system, which is a subsystem of a larger entity, is
...
where
$$|\!\rightarrow\rangle = \frac{1}{\sqrt{2}} \Big(|\!\uparrow\rangle +
  |\!\downarrow\rangle\Big).$$
Find the entropy of entanglement.

3. Consider two distinguishable particles, $A$ and $B$.  The 2D Hilbert
