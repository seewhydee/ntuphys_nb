...
problem at positions far from the scatterer (@sec-scattering).  The
...
When studying the scattering problem in @sec-scattering, we saw that
...
@eq-appA-helmholtz.  Let us go over these solutions for the $d=2$ and
$d=3$ cases, before discussing how they can be used to solve scattering
problems.
...
This is called a **circular wave**, and the integer $m$
...
For large $kr$, the functions behave as decaying complex sinusoidals,
...
conservation of flux in the $r$ direction in 2D (see
@sec-scattering-amplitudes); moreover, the Hankel functions for
different $m$'s are all normalized to carry the same flux.
...
function of the angular coordinates $\theta$ and $\phi$; i.e.,
...
@eq-appA-psirdecomp with the "incident" and "scattered" waves defined
in the scattering problem.  As discussed in @sec-scattering, a
...
scattering matrix to the formulation of the scattering problem.  Let
us go through the details for 3D, leaving the 2D case as an
[exercise](#sec-appendix-a-exercises).
...
The quantity in square brackets is the scattering amplitude (@sec-scattering-amplitudes):
...
$$s_{\ell} = e^{2i\Delta_{\ell}},$${#eq-appA-sell}
for some real phase $\Delta_{\ell}$.  This has a simple physical
interpretation: since each scattering channel is independent and
...
$$A_\ell(r) = c^-_\ell \Big(h^-_\ell(kr) + e^{2i\Delta_\ell}\, h^+_\ell(kr)\Big),
...
$$\begin{aligned} \alpha_\ell\, j_\ell(qR)
    ...
    + e^{2i\Delta_\ell} h^+_\ell(kR)\Big) \\
    \alpha_\ell\, q j_\ell'(qR)
    &= c^-_\ell k \Big({h^-_\ell}'(kR) + e^{2i\Delta_\ell} {h^+_\ell}'(kR)\Big).\end{aligned}$$
...
$$e^{2i\Delta_\ell} = - \frac{k{h_\ell^-}'(kR) j_\ell(qR) - qh_\ell^-(kR)j_\ell'(qR)}{k{h_\ell^+}'(kR) j_\ell(qR) - qh_\ell^+(kR)j_\ell'(qR)}.$$
...
scattering amplitude plots shown @sec-uniform-spherical-well.
