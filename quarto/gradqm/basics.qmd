...
$$\hat{A} = \alpha_0 \hat{\sigma}_0 + \alpha_1 \hat{\sigma}_1 + \alpha_2 \hat{\sigma}_2 + \alpha_3 \hat{\sigma}_3,\;\;\;\alpha_{n} \in \mathbb{R},$$ {#eq-pauli-superposition}

where $\hat{\sigma}_0 = \hat{I} =
\begin{pmatrix}1&0\\0&1\end{pmatrix}$ is the $2\times2$ identity
matrix, and $\sigma_1, \dots, \sigma_3$ are the **Pauli matrices**:
...
Note that the four coefficients in @eq-pauli-superposition must be
real; were we to use a complex linear combination, the resulting
operator may not be Hermitian.

The identity and Pauli matrices are not the only possible basis for
the observables of $\mathbb{C}^2$, but they have some unique
advantages:

* They are linearly independent: each matrix cannot be written as any
  linear combination of the others.
  
* The leading term, $\alpha_0\hat{\sigma}_0$, represents a trivial
  term, in the sense that it can be taken out of the eigenproblem for
  $\hat{A}$:
  $$\begin{aligned}&\hat{A} |\psi\rangle \;=\; \lambda |\psi\rangle \\
&= \left(\alpha_0 \hat{\sigma}_0 + \alpha_1 \hat{\sigma}_1 + \alpha_2 \hat{\sigma}_2 + \alpha_3 \hat{\sigma}_3\right)|\psi\rangle \\ \Leftrightarrow \;\;\;&
\left(\alpha_1 \hat{\sigma}_1 + \alpha_2 \hat{\sigma}_2 + \alpha_3 \hat{\sigma}_3\right)|\psi\rangle = \left(\lambda - \alpha_0\right) |\psi\rangle.\end{aligned}$$
  ...
  eigenstates of $\hat{A}$, while $\alpha_0$ merely shifts the
  eigenvalues (e.g., shifting the zero of energy for the Hamiltonian).
