...
Here is a brief summary of this topic, which requires jumping ahead a
bit to the concept of wavefunctions (to be covered in
@sec-wavefunctions).  Suppose we look at a wavefunction
$\psi(\mathbf{r})$, where $\mathbf{r}$ is a 3D position vector, and
ask how it transforms under the various possible coordinate rotations
that are possible in 3D.  One finds that rotation by an angle $\phi$
(anticlockwise), around each Cartesian axis $i \in \{1,2,3\}$, is
given by a unitary operator of the form

$$\hat{U}_i(\phi) = e^{i\phi\hat{J}_i/\hbar},$$

where $\hat{J}_1$, $\hat{J}_2$, and $\hat{J}_3$ are so-called
**orbital angular momentum** observables that must satisfy the
...
degrees of freedom, *even if they aren't really spins*.

As an example, consider the following model of a **two-level atom**,
consisting of a ground state $|g\rangle$ and an excited state
$|e\rangle$, whose energies differ by $\hbar\Delta \omega$.  If the
atom is illuminated by an electromagnetic wave, called a “driving
field”, its Hamiltonian takes the form
$$\hat{H}(t) = \left(E_0 - \frac{\hbar\Delta \omega}{2}\right) |g\rangle\langle g| + \left(E_0 + \frac{\hbar\Delta \omega}{2}\right) |e\rangle\langle e| - \frac{\hbar \Omega_d}{2} \Big(e^{-i\omega_d t} |e\rangle\langle g| + e^{i\omega_d t} |g\rangle\langle e| \Big),$$
where $E_0$ is a baseline energy, $\omega_d$ is the angular frequency
of the driving field, and $\Omega_d$ is a parameter quantifying the
strength of the driving field, expressed as a frequency unit and
called the **Rabi frequency**.  Thus, the third term in this
Hamiltonian (the one proportional to $\Omega_d$) describes the effect
of the driving field; we will skip over its derivation, but you can
convince yourself that it is reasonable, since it (i) is Hermitian,
(ii) is time-dependent with angular frequency $\omega_d$, and (iii)
...
maps into
$$\hat{H}(t) = E_0 - \lambda \mathbf{B}(t) \cdot \hat{\mathbf{S}}, \;\;\mathrm{where}\;\; \lambda \mathbf{B}(t) = \begin{pmatrix}\Omega_d\cos(\omega_dt)\\ -\Omega_d\sin(\omega_dt) \\ \Delta \omega\end{pmatrix}.$$
This is equivalent to the Hamiltonian for a magnetic dipole moment
$\lambda \hat{\mathbf{S}}$ (the spin times a scaling factor), embedded
in a magnetic field $\mathbf{B}(t)$ that is rotating around the $z$
axis (i.e., the field has a static piece oriented along $z$, and a
time-dependent piece that rotates in the $x$-$y$ plane with
...
Next, we want to solve the Schrödinger equation for this Hamiltonian.
To do this, it is convenient to move to a frame that co-rotates with
the magnetic field, with the same angular frequency $\omega_d$.  Let
us define the following unitary rotation operator around the $z$ axis
(remember, this is a rotation in an *abstract* space defined by the
spin mapping, not a physical space):
$$\hat{U}_R(\theta) = e^{-i\theta\,\hat{\sigma}_3/2}.$$
The Pauli matrices can be shown to satisfy the identity
$$\hat{U}_R^\dagger(\theta) \,\hat{\sigma}_1\, \hat{U}_R(\theta) = \hat{\sigma}_1\cos\theta - \hat{\sigma}_2\sin\theta.$$
Hence, the Hamiltonian in @eq-rabi-time-dependent can be rewritten as
$$\hat{H}(t) = \hat{U}_R^\dagger(\omega_d t)\, \left(E_0 - \frac{\hbar\Delta \omega}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2} \,\hat{\sigma}_1 \right) \hat{U}_R(\omega_d t).$$ {#eq-rabi-rotated-h}
Note that the factor in the center is independent of $t$.  We then define the co-rotating state vector
$$|\psi_R(t)\rangle = \hat{U}_R(\omega_d t)\,|\psi(t)\rangle \;\;\;\Leftrightarrow \;\;\; |\psi(t)\rangle = \hat{U}_R^\dagger(\omega_d t)\,|\psi_R(t)\rangle.$$
The Schrödinger equation, @eq-schrodinger, can be re-written as
$$i\hbar\frac{d}{dt}\left(\hat{U}_R^\dagger(\omega_d t)\,|\psi_R(t)\rangle\right) 
= i\hbar \left(\frac{d\hat{U}_R^\dagger(\omega_d t)}{dt} + \hat{U}_R^\dagger(\omega_d t) \frac{d}{dt}\right) |\psi_R(t)\rangle = \hat{H}(t) \hat{U}_R^\dagger(\omega_d t) |\psi_R(t)\rangle.$$
Noting that $(d/dt)\hat{U}_R^\dagger(\omega_d t) = (i\omega_d/2) \, \hat{\sigma}_3 \, \hat{U}_R^\dagger(\omega_d t)$, and making use of @eq-rabi-rotated-h on the right-hand side, we obtain
$$i \hbar \frac{d}{dt} |\psi_R(t)\rangle
= \left(E_0 + \frac{\hbar(\omega_d - \Delta \omega)}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2} \,\hat{\sigma}_1 \right) |\psi_R(t)\rangle.$$


- Interpretation: in this frame the rotating transverse field is frozen (along $x$), and the **detuning** $\delta$ survives as a residual static field along $z$.  The driven atom has become a spin in a *static* effective field.

- Start the atom in $|g\rangle$ at $t=0$.  The state then precesses about the effective field, giving
  $$P_e(t) = \Big|\langle e|\psi(t)\rangle\Big|^2 = \frac{\Omega_d^2}{\Omega_R^2}\,\sin^2\!\left(\frac{\Omega_R\,t}{2}\right).$$
- On resonance ($\delta = 0$, i.e. $\omega_d = \Delta\omega$): $\Omega_R = \Omega_d$, so
  $$P_e(t) = \sin^2\!\left(\frac{\Omega_d\,t}{2}\right) = \frac{1 - \cos(\Omega_d t)}{2}.$$
  The atom oscillates *fully* between $|g\rangle$ and $|e\rangle$ at the **Rabi frequency** $\Omega_d$.
- Off resonance ($\delta \ne 0$): the oscillation is faster ($\Omega_R > \Omega_d$) but incomplete — the maximum excited-state population is $(\Omega_d/\Omega_R)^2 < 1$, so the atom never quite reaches $|e\rangle$.

**Why the spin analogy is useful (the punchline).**
- A messy time-dependent two-level problem collapsed to plain **Larmor precession** about a fixed axis: the entire dynamics is just a rotation.
- Geometric picture: every spin-1/2 state is a point on the unit sphere (the **Bloch sphere**); time evolution rotates that point about the effective-field axis $\mathbf{n} = (\Omega_d, 0, -\delta)/\Omega_R$.  On resonance the trajectory is a great circle through the two poles (full transfer); off resonance it is a smaller circle (incomplete transfer).
- Concrete utility — pulses as gates:
  - $\Omega_d t = \pi$ ("$\pi$-pulse") swaps $|g\rangle \leftrightarrow |e\rangle$: a NOT gate.
  - $\Omega_d t = \pi/2$ ("$\pi/2$-pulse") makes an equal superposition $(|g\rangle \pm |e\rangle)/\sqrt2$: a Hadamard-like gate.
  - This is exactly how qubits are manipulated in quantum computing.
- Same mathematics as NMR/MRI: a resonant radio-frequency pulse flips a spin at the Larmor frequency, and the detuning limits how far the flip can go.
- Hook: this is a first, elementary **resonance** — a strong response when the drive matches the system's natural frequency — developed in full generality in Chapter 3.
