...
particles can carry (even point particles like electrons, which have
no internal structure as far as we know).
...
$$\hat{J_i} = \frac{\hbar}{2}\, \hat{\sigma}_i.$$

Due to the $\hbar/2$ prefactor, the Pauli matrices describe a
so-called spin-1/2 system.  We can thus re-interpret the basis
expansion of a general observables, @eq-pauli-superposition, in
angular momentum terms.  The expansion has the same form as the
Hamiltonian for a magnetic dipole in a magnetic field $\mathbf{B}$:
...
Here, $E_0$ is a baseline energy, and $\lambda \hat{\mathbf{S}}$ is a
magnetic moment operator (which is proportional to the spin).
