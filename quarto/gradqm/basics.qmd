...
3.  **The Schrödinger equation**---A system's state,
    ...
    total energy of the system, and its eigenvalues are called
    **eigenenergies**.
...
However, let us put them aside for now.  In the rest of this chapter,
we will use the quantum postulates to develop some elementary models
to describe two-level systems, particles in discrete space, and
...
Lie algebra also pops up in the quantum theory of 3D angular momentum.
...
Next, we take a conceptual leap and postulate that any set of angular
momentum observables tied to 3D rotations must also satisfy
@eq-Lie-algebra.  This opens the door to another form of angular
momentum: **spin angular momentum**, which is intrinsically possessed
by a particle independent of its motion (remarkably, even point
particles like electrons, which have no internal structure as far as
we know, can carry spin).
...
representation.  Note: that 3D representation refers to a
representation for $\mathfrak{su}(2)$, *not* $\mathfrak{su}(3)$.  The
latter refers to another Lie algebra with different commutation
...
the $\hbar/2$ prefactor, the Pauli matrices are said to describe a
