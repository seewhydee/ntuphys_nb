...
These are foundational problems that continue to dog quantum theory to
this day, despite all its undeniable successes.  Still, let us put
these qualms aside for now.  In the remainder of this chapter, we will
use the quantum postulates to develop some important elementary models
describing two-level systems, particles in discrete space, and
...
written as a linear combination of the others.  Thus, in
@eq-pauli-superposition we are using these matrices as a linear basis
to construct all other observables.  The coefficients $\alpha_0,
\dots, \alpha_3$ are the four real parameters required for a general
observable in $\mathbb{C}^2$; since they are real, the linear
combination is guaranteed to be Hermitian.
...
* The three remaining matrices are $2\times2$, traceless, and
  ...
  matrices, then a set of reasonable choices
  (see [Exercise 1](#sec-basics-exercises)) leads to the
  ...
  matrices listed in @eq-pauli indeed satisfy @eq-Lie-commutators.
  Often, when manipulating formulas containing Pauli matrices, it is
  advantageous to use these algebraic relations instead of working
  with the explicit matrices.
