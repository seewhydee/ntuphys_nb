...
continuous real variables.  For simplicity, we focus on the case of
functions are defined over the real line (the
generalizations to other cases are fairly straightforward):
$$|\psi\rangle \;\; \leftrightarrow \;\; \psi(x), \;x\in\mathbb{R}.$$
Such functions obey the axioms for vector spaces, since they
...
meet these additional requirements, we require the complex functions
to be **square-integrable**, i.e.,
$$\int_{-\infty}^\infty |\psi(x)|^2 \; dx < \infty.$$ {#eq-l2-normalizable}
...
$$\langle \psi_1 | \psi_2 \rangle \equiv \int_{-\infty}^\infty \; \psi_1^*(x) \psi_2(x) \; dx,$$
and show that this satisfies the inner product axioms; thereafter, a
result called the [Riesz–Fischer theorem](https://en.wikipedia.org/wiki/Riesz%E2%80%93Fischer_theorem)
ensures that the vector space is complete.  Hence, square-integrable
complex functions form a complex Hilbert space.  The customary
normalization of state vectors can be expressed as
