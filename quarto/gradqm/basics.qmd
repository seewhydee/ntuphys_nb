...
## States and Observables {#sec-states-and-observables}
...
3.  **The Schrödinger equation**---A system's state,
    ...
    $$i\hbar \frac{d}{dt}|\psi(t)\rangle = \hat{H}(t) \, |\psi(t)\rangle,$${#eq-schrodinger}
    where $\hat{H}(t)$ is an observable called the **Hamiltonian**
    (which also may be time-dependent).  The Schrödinger equation is
    norm-preserving: $\frac{d}{dt}\langle\psi(t)|\psi(t)\rangle = 0$.^[*Proof*: By
...
prepared in state $|\psi\rangle$.  Using the Born rule,
@eq-measurement, we derive this as
...
$\langle A\rangle$ is real.  In the next-to-last step, we
...
using the eigenstates of $\hat{A}$:
$$\hat{I} = \sum_n |n\rangle\, \langle n |.$$
This works because $\hat{A}$ is Hermitian, so by the [spectral
theorem](https://en.wikipedia.org/wiki/Spectral_theorem) its
eigenstates form an orthonormal basis for the Hilbert space (the
“eigenbasis of $\hat{A}$”).

### Time evolution {#sec-time-evolution}

The Schrödinger equation, @eq-schrodinger, is a first-order homogenous
linear differential equation.  Therefore, if we specify an initial
state $|\psi(t')\rangle$, the state at any other time,
$|\psi(t)\rangle$, is uniquely determined (assuming no measurement
process occurring in the interim).  Formally, we can write
$$|\psi(t)\rangle = \hat{U}(t,t')\,|\psi(t')\rangle,$$
where $\hat{U}$ is called the **time-evolution operator**.  Because the
Schrödinger equation is norm-preserving, it follows that
$\hat{U}$ is a **unitary operator** (see [Exercise 1](#sec-basics-exercises)), i.e.,
$$\hat{U}(t,t')^\dagger = \hat{U}(t,t')^{-1}\;\;\textrm{for any}\;\;t,t'.$$

Let us suppose the system has a time-independent Hamiltonian
$\hat{H}$.  Then it can be shown that
$$\hat{U}(t, t') = \exp\!\left(-\frac{i}{\hbar}\hat{H} \cdot (t-t')\right),$$
where “$\exp$” denotes an operator exponential.^[If the Hamiltonian is
time-dependent, the relation between $\hat{U}(t,t')$ and
$\hat{H}(t)$ becomes more complicated.  The main difficulty is that
$\hat{H}(t)$ at different times may not commute, and this may be resolved
by introducing a so-called “time-ordered exponential”.  The details
will not concern us here.]  Consequently, if we prepare the system at time $t'$ in an energy eigenstate $|n\rangle$, with eigenenergy $E_n$, then
$$|\psi(t)\rangle = e^{-iE_n(t-t')/\hbar}|n\rangle.$$
This implies that an enery eigenstate of a time-independent
Hamiltonian is a “stationary” state: as time progresses, the state
vector accumulates a phase $\Delta\phi = E_n(t-t')/\hbar$, but it
remains an energy eigenstate with the same eigenenergy.  If we
subsequently perform any measurement on the system, the outcome
probabilities do not depend on $t$ or $t'$.  (*After* the measurement,
of course, it undergoes state collapse in the usual way).
...
and has two eigenvalues.]  Nowadays, with the advent of quantum
computing, they are also commonly referred to as **qubits** (i.e.,
quantum bits); we will adopt this term.

Let us choose an arbitrary basis for the 2D Hilbert space, consisting of
...
(Here, $\leftrightarrow$ denotes the representation of an abstract
vector in a given basis.)  Any vector in $\mathbb{C}^2$ can be written
as
...
vectors can also be employed in the resolution of the identity:
$$\hat{I} = |+\rangle \, \langle +| \;+\; |-\rangle \, \langle -| \;\;\leftrightarrow \;\;
\begin{pmatrix}1&0\\0&1\end{pmatrix}.$$

### Pauli matrices {#sec-Pauli-matrices}

What observables can a qubit have?  Its observables consist of
$2\times2$ Hermitian matrices, which are characterized by four real
parameters (the diagonal elements are real, which gives two
parameters, and the off-diagonal elements are conjugates of each
other, which gives another two).  We can write any such matrix as the
linear combination
...
$\mathbb{C}^2$, but it has some very nice properties:
...
* The three remaining matrices are $2\times2$, traceless, and
  ...
  matrices, a set of reasonable choices (see [Exercise 2](#sec-basics-exercises)) leads to the anticommutation and
  ...
  matrices, @eq-pauli, satisfy these relations.  Often, when
  manipulating formulas containing Pauli matrices, it is advantageous
  to use these relations instead of working with the explicit
  matrices.

### Pauli matrices and spin

The commutation relation in the second line of @eq-Lie-commutators
defines a [**Lie algebra**](https://en.wikipedia.org/wiki/Lie_algebra)
called $\mathfrak{su}(2)$.  Remarkably, this Lie algebra also pops up
in the quantum theory of angular momentum in three-dimensional space.

Here is a brief sketch of this topic, which requires jumping ahead to
the concept of wavefunctions (covered in @sec-wavefunctions).  Given a
wavefunction $\psi(\mathbf{r})$, where $\mathbf{r}$ is a 3D position
vector, suppose we ask how it transforms under coordinate rotations.
After analyzing this systematically, we find that a rotation around
each Cartesian axis $i \in \{1,2,3\}$, by an anticlockwise angle
$\phi$, is described by a unitary operator of the form

$$\hat{U}_i(\phi) = e^{i\phi\hat{J}_i/\hbar},$$ {#eq-rotation-operator}

where $\hat{J}_1$, $\hat{J}_2$, and $\hat{J}_3$ are **angular
momentum** observables that satisfy the following commutation relations
(see @sec-orbital-angular-momentum):

$$\left[\hat{J}_i, \hat{J}_j\right] = i\hbar \sum_{k=1}^3 \epsilon_{ijk} \, \hat{J}_k,\qquad i,j,k\in\{1,2,3\}.$$ {#eq-Lie-algebra}

Now, we take a conceptual leap by postulating that @eq-Lie-algebra
*defines* the set of angular momentum observables tied to 3D
rotations.  In other words, these commutation relations are more
fundamental than the wavefunctions we used to derive them.

This opens the door to other forms of angular momentum!  In
particular, particles like the electron can possess a type of angular
momentum called **spin**, which is distinct from orbital angular
momentum (so far as we know, the electron is a point particle, with no
internal structure that can "rotate" in the usual sense).
...
$$\begin{aligned}\hat{\mathbf{S}}^2 \equiv \hat{S}_1^2 + \hat{S}_2^2 + \hat{S}_3^2 &= \left(\frac{\hbar}{2}\right)^2 \big(\hat{\sigma_1}^2 + \hat{\sigma_2}^2 + \hat{\sigma_3}^2\big) \\ &= \left(\frac{\hbar}{2}\right)^2 \cdot 3 \\&= \hbar^2 s(s+1),\end{aligned}$$
...
to interpret qubits as spin-1/2 degrees of freedom, even if they
aren't really *actually* spin.

An important example of this practice can be found in the theory of a
**two-level atom**.  Suppose an atom has a ground state $|g\rangle$
and an excited state $|e\rangle$, whose energies differ by
$\hbar\Delta \omega$.  Its Hamiltonian is
$$\hat{H}_0 = \left(E_0 - \frac{\hbar\Delta \omega}{2}\right) |g\rangle\langle g| + \left(E_0 + \frac{\hbar\Delta \omega}{2}\right) |e\rangle\langle e| = E_0 - \frac{\hbar \Delta\omega}{2} \hat{\sigma}_3,$$
where $E_0$ is the average energy of the two states.  Now suppose
this atom is illuminated by an electromagnetic wave, which functions
as a “driving field” that induces transitions from the ground state to
the excited state or vice versa.  The Hamiltonian then becomes
$$\hat{H}(t) = \hat{H}_0 - \frac{\hbar \Omega_d}{2} \Big(e^{-i\omega_d t} |e\rangle\langle g| + e^{i\omega_d t} |g\rangle\langle e| \Big).$$
Here, $\omega_d$ is the angular frequency of the driving field, and
$\Omega_d$ quantifies the strength of the driving field, expressed as
a frequency unit and called the **Rabi frequency**.  We will skip over
the derivation of how this new term in the Hamiltonian arises from the
driving field, but you can convince yourself that it makes sense,
since it has the right features: (i) it is Hermitian, (ii) it is
time-dependent with angular frequency $\omega_d$, and (iii) it induces
transitions between the two atomic states.

Using the basis $\{|g\rangle, |e\rangle\}$, we can write the
full Hamiltonian as
$$\hat{H}(t) = E_0 - \frac{\hbar\Delta \omega}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2}\left[\cos(\omega_d t) \hat{\sigma}_1 - \sin(\omega_d t) \hat{\sigma}_2\right].$$ {#eq-rabi-time-dependent}
If we introduce the spin operator from @eq-spin-map, the Hamiltonian
maps into
$$\hat{H}(t) = E_0 - \begin{pmatrix}\Omega_d\cos(\omega_dt)\\ -\Omega_d\sin(\omega_dt) \\ \Delta \omega\end{pmatrix} \cdot\, \hat{\mathbf{S}}.$$ {#eq-rotating-field}
This is equivalent to the Hamiltonian for a magnetic dipole moment
(consisting of the spin $\hat{\mathbf{S}}$ times some scale factor) in
a *rotating* magnetic field.  As depicted below, the magnetic field
has a static part pointing along $z$, and a time-dependent part that
rotates with angular frequency $\omega_d$ in the $x$-$y$
plane.
...
This insight can be used to solve the Schrödinger equation for this
system: since the magnetic field is rotating (albeit in the abstract
space defined by the spin mapping, not a physical space), we should go
to a co-rotating frame!  Using @eq-rotation-operator, we define the
rotation operator
$$\hat{U}_3(\theta) = e^{i\theta\,\hat{\sigma}_3/2}.$$
It can be shown that the Pauli matrices satisfy the following identity
(see [Exercise 3](#sec-basics-exercises)):
$$\hat{U}_3(\theta) \,\hat{\sigma}_1\, \hat{U}_3^\dagger(\theta) = \hat{\sigma}_1\cos\theta - \hat{\sigma}_2\sin\theta.$$
Hence, @eq-rabi-time-dependent can be rewritten as
$$\hat{H}(t) = \hat{U}_3(\omega_d t)\, \left(E_0 - \frac{\hbar\Delta \omega}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2} \,\hat{\sigma}_1 \right) \hat{U}_3^\dagger(\omega_d t).$$ {#eq-rabi-rotated-h}
Note that the factor in the center is independent of $t$.  Next, define the state vector in the co-rotating frame, $|\psi_R\rangle$, by
$$|\psi_R(t)\rangle = \hat{U}_3^\dagger(\omega_d t)\,|\psi(t)\rangle \;\;\;\Leftrightarrow \;\;\; |\psi(t)\rangle = \hat{U}_3(\omega_d t)\,|\psi_R(t)\rangle.$$ {#eq-co-rotating-state}
Therefore we can rewrite the Schrödinger equation, @eq-schrodinger, as
$$\begin{aligned}i\hbar\frac{d}{dt}\left(\hat{U}_R(\omega_d t)\,|\psi_R(t)\rangle\right) 
&= i\hbar \left(\frac{d\hat{U}_3(\omega_d t)}{dt} + \hat{U}_3(\omega_d t) \frac{d}{dt}\right) |\psi_R(t)\rangle \\&= \hat{H}(t) \hat{U}_3(\omega_d t) |\psi_R(t)\rangle.\end{aligned}$$
Taking $(d/dt)\hat{U}_3(\omega_d t) = (i\omega_d/2) \, \hat{\sigma}_3
\, \hat{U}_3(\omega_d t)$, along with @eq-rabi-rotated-h on the right
side, we arrive at a transformed Schrödinger equation for the
co-rotating frame:
$$i \hbar \frac{d}{dt} |\psi_R(t)\rangle
= \left(E_0 + \frac{\hbar \delta\omega}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2} \,\hat{\sigma}_1 \right) |\psi_R(t)\rangle, \quad \delta\omega \equiv \omega_d - \Delta \omega.$$ {#eq-static-field-hamiltonian}
This features a static magnetic field pointing in the $x$-$z$ plane,
with the exact direction depending on $\Omega_d$ and the “frequency
detuning” $\delta\omega$.  We will skip the remaining steps in solving
@eq-static-field-hamiltonian, which can be accomplished analytically;
afterwards, we can use @eq-co-rotating-state to retrieve the state in
the original (non-co-rotating) frame.  If the atom is prepared in
$|g\rangle$ at $t=0$, the analytic solution shows that the occupation
probability for the ground state varies sinusoidally:
$$P_e(t) = \Big|\langle e|\psi(t)\rangle\Big|^2 \propto \sin^2\! \left(\frac{\Omega_R\,t}{2}\right).$$
This phenomenon is called **Rabi oscillation**.  The derivation
is worked out in [Exercise 4](#sec-basis-exercises).

Aside from atomic physics, this two-level model and its associated
spin interpretation are frequently employed in quantum computing.  For
example, qubits based on superconducting circuits are commonly
manipulated by radio-frequency pulses, which act as driving fields.
By adjusting the pulse duration and/or the qubit's $\delta\omega$ and
$\Omega_R$ parameters, it is possible to rotate the spin in various
directions, corresponding to various kinds of quantum logic gates.
...
The sites on the ring are indexed by an integer $n \in [0, N-1]$ (we
start from 0 for later convenience), with $N$ sites in total.  The
fact that the number of sites is finite lets us postpone the
mathematical complications that come with having an
infinite-dimensional Hilbert space.

### Position eigenstates

We wish to consider a *single* particle existing within the lattice.
Let us define a set of **position eigenstates**, denoted by

$$\big\{|0\rangle,\; |1\rangle,\; \dots |N-1\rangle \big\},$$
...
$$\langle m|n\rangle = \delta_{mn}, \;\;\; m,n\in [0,N-1],$$

and that they form a complete orthonormal basis for the particle's
Hilbert space.  Thus, the Hilbert space is $\mathscr{H} =
\mathbb{C}^N$ (the space of $N$-component complex vectors).  We can
use the position eigenstates to construct a position observable,

$$\begin{aligned}\hat{x} &\equiv \sum_{n} |n\rangle \,x_n\, \langle n|, \;\;\;\mathrm{where}\;\;x_n \equiv na. \\
\Rightarrow\;\; \hat{x} |n\rangle &= x_n |n\rangle.\end{aligned}$$

To keep the notation clean, we will omit the bounds of the sum over
$n$, and implicitly assume that such sums run from $0$ to $N-1$,
unless explicitly stated otherwise.  The variable $a$ is a length unit
we have introduced to represent the equal spacings between lattice
sites.  Strictly speaking, we could take $a = 1$, but it is nice to
retain it so that the formulas are dimensionally consistent; thus, the
position eigenvalues are $x_n \equiv n a$, with explicit units of
length.

A general state $|\psi\rangle \in \mathscr{H}$ can be written in the
position basis as
$$|\psi\rangle = \sum_{n} \psi_n |n\rangle, \;\;\;\psi_n \in \mathbb{C},$$
and describes a *single* particle that occupies a superposition of
different sites.  The complex numbers $\psi_n$ constitute the
particle's **discrete wavefunction**—a discrete version of the
wavefunctions to be discussed in @sec-wavefunctions.  It follows that:

* The state $|\psi\rangle$ is normalized if and only if
    $$\sum_{n} \left|\psi_n\right|^2 = 1.$$
* The inner product between $|\psi\rangle$ and another state
    $|\varphi\rangle$ (with discrete wavefunction $\varphi_n$) is
    $$\langle\varphi|\psi\rangle = \sum_{n} \varphi^*_n\psi_n.$${#eq-discrete-wavefunction-product}

### Momentum eigenstates {#sec-lattice-momentum}

Consider the following states on the ring lattice:

$$|k_p\rangle = \frac{1}{\sqrt{N}} \sum_{n} e^{ik_p x_n} |n\rangle.$$ {#eq-discrete-k-state}

As before, $x_n \equiv na$ denotes positions along the ring.  These
states' discrete wavefunctions are simple waves of the form $e^{ikx}$,
sampled at discrete points $x = x_n$.  But rather than allowing
arbitrary wavenumbers, we should take

$$k_p = \frac{2\pi p}{Na}, \;\; p = 0, 2, \dots, N-1,$$ {#eq-k-quantization}

in order to ensure that the waves satisfy the periodicity of the ring.
To see why, consider putting $n = N$ into the wavefunction; if
@eq-k-quantization holds, then

$$\begin{aligned}\exp(ik_p x_{N}) &= \exp\left[i \left(\frac{2\pi p}{Na}\right) Na\right] \\ &= e^{2\pi i p} \\ &= 1 \quad =\; \exp(ik_p x_0). \end{aligned}$$ {#eq-discrete-wave-periodicity}

Thus, site $N$ is equivalent to site 0, as we require.  For the same
reason, we see that the wavenumber $k_p$ is defined modulo $2\pi/a$:
shifting it by multiples of $2\pi/a$ leaves @eq-discrete-k-state
unchanged.

By the orthogonality of [Fourier series](https://en.wikipedia.org/wiki/Fourier_series), we can show that:^[*Proof*: Using the discrete wavefunction from @eq-discrete-k-state, along with the inner product rule in @eq-discrete-wavefunction-product,
$$\begin{aligned}\langle k_n | k_{n'} \rangle &= \frac{1}{N} \sum_{m} e^{-ik_n x_m} \, e^{ik_{n'}x_m} \\ &= \frac{1}{N} \sum_{m=0}^{N-1} \exp\left[\frac{2\pi i (n'-n) m}{N}\right].\end{aligned}$$
If $n \ne n'$, we can use the geometric series formula.  Writing $\Delta n = n'-n$, we find that the sum goes to zero:
$$\sum_{m=0}^{N-1} \exp\left[\frac{2\pi i \,\Delta n\, m}{N}\right]
= \frac{\left[\left(e^{2\pi i \Delta n/N}\right)^N - 1\right]}{ e^{2\pi i \Delta n/N} - 1} = \frac{1-1}{(\cdots)} = 0.$$
If $n = n'$, we cannot use the geometric series formula because the
numerator and denominator are both zero.  In this case, though, each
term in the sum is $1$, so the result is $(1/N)\cdot N = 1$.
Combining the two cases, we arrive at @eq-k-discrete-ortho.]

$$\langle k_n | k_{n'} \rangle = \delta_{nn'},\;\;\;\mathrm{where}\;\;n,n' = 0, \dots, N-1.$$ {#eq-k-discrete-ortho}
...
understand this, we look at translations on the ring lattice.  Let us
define **translation operators** $\hat{T}_q$, where $q \in
\mathbb{Z}$, such that
...
Applying it to $|k_p\rangle$ gives

$$\begin{aligned}\hat{T}_q |k_p\rangle
&= \frac{1}{\sqrt{N}} \sum_{n} e^{ik_p x_n} \hat{T}_q |n\rangle \\
&= \frac{1}{\sqrt{N}} \sum_{n} e^{ik_p x_n} |n+q\rangle \\
&= \frac{1}{\sqrt{N}} \sum_{n} e^{ik_p (n-q) a} |n\rangle \\
&= \exp(-ik_p qa) \, |k_p\rangle.
\end{aligned}$$

In the third step, we made use of the discrete wavefunction's
periodicity (see @eq-discrete-wave-periodicity).  We discover that
$\hat{T}_q$ leaves $|k_p\rangle$ unchanged except for a phase factor
$e^{-ik_p qa}$, for any $q$ and $n$.  In classical mechanics,
...
association, we identify the $|k_p\rangle$ states as **momentum
eigenstates**; the corresponding momentum is $\hbar k_p$, with the
factor of $\hbar$ inserted to achieve units of momentum.  Just as
$k_p$ is defined modulo $2\pi/a$, this momentum is defined modulo
$2\pi\hbar/a$; it is also called **quasimomentum**, to distinguish it
from the non-periodic form of momentum we will study in
@sec-wavefunctions.

### Quantum walks

The time-evolution of a particle on a lattice is often governed by a
“hopping” Hamiltonian that describes how the particle transitions to
neighboring sites.  For instance, on a 1D lattice, we might take
$$\hat{H} = - J \sum_{n = 1}^N \big(|n\rangle \, \langle n+1| \, + \, |n+1\rangle \, \langle n| \big),$$ {#eq-hamiltonian-1dring}
where $J$ is some energy scale (the minus sign is customary), and $\pm
1$ is taken modulo $N$ to account for the lattice being a ring: i.e.,
$|0\rangle \equiv |N\rangle$.  To understand this Hamiltonian,
consider how it acts on a particle prepared with state $|n_0\rangle$,
on a single site, at time $t = 0$:

* The first term in the parentheses, acting on $|n_0\rangle$, gives
  $-J|n_0+1\rangle$, coming from the term with $n+1=n_0$; all other
  terms (with $n+1\ne n_0$) vanish.

* The second term in the parentheses is the Hermitian conjugate of the
  first, ensuring that $\hat{H}$ is Hermitian. Applied to the state
  $|n_0\rangle$, it yields $-J|n_0- 1\rangle$ (from the $n = n_0$ term).

Therefore, following the Schrödinger equation @eq-schrodinger, the
state after time $\delta t$ is
$$\begin{aligned}|\psi(\delta t)\rangle &= |n_0\rangle + \frac{\delta t}{i\hbar} \hat{H} |\psi(0)\rangle + O(\delta t^2) \\ &= |n_0\rangle - \frac{J \delta t}{i\hbar}\big(|n_0+1\rangle + |n_0-1\rangle\big) + O(\delta t^2).\end{aligned}$$ {#eq-quantum-walk-hop}
Evidently, part of the particle's state has “hopped” to the left and
right of the initial site $n_0$.  With increasing $t$, the particle
continues hopping to sites further away, in a process called a
**quantum walk**.

In fact, we can do better than @eq-quantum-walk-hop and derive the
full quantum walk solution for all $t$.  To do this, we exploit the
fact that the Hamiltonian is translationally symmetric.  Applying
@eq-hamiltonian-1dring to the quasimomentum eigenstates from
@sec-lattice-momentum yields
$$\begin{aligned}\hat{H} |k_p\rangle &= - \frac{J}{\sqrt{N}} \sum_{m,q} e^{ik_p x_m} \big(|q\rangle \, \langle q+1 | m\rangle  + |q+1\rangle \, \langle q | m\rangle \big) \\ &= - \frac{J}{\sqrt{N}} \sum_{q} \big( e^{ik_p x_{q+1}} + e^{ik_p x_{q-1}} \big) |q\rangle \\ &= - 2J \, \cos(k_p a) \, |k_p\rangle.
\end{aligned}$$
Hence, each quasimomentum eigenstate is also an energy eigenstate, with eigenenergy
$$E_p = - 2J \, \cos(k_p a), \;\;\; \mathrm{where}\;\;k_p = \frac{2\pi p}{Na}.$$ {#eq-momentum-energy-1dring}
This $E$-vs-$k$ equation is called a **dispersion relation**. One of
the things it tells us is the **group velocity**, which specifies how
quickly a wavepacket can travel in the lattics (see [Exercise
5](#sec-basics-exercises)):
$$v_g=\frac{1}{\hbar}\frac{dE}{dk}=\frac{2Ja}{\hbar}\sin(ka),$$

Now, suppose the particle is prepared in an arbitrary initial state at
$t = 0$:
$$|\psi(0)\rangle = \sum_{n} \psi_n |n\rangle,$$
where $\{\psi_n\}$ is a discrete wavefunction.  We can express this
in the quasimomentum basis @eq-discrete-k-state:
$$\begin{aligned}|\psi(0)\rangle &= \sum_{n} \psi_n \Bigg(\sum_p |k_p\rangle \, \langle k_p | \Bigg) |n\rangle \\ &= \frac{1}{\sqrt{N}} \sum_{p} \Psi_p \, |k_p\rangle,\end{aligned}$$
where
$$\Psi_p \;\equiv\; \sum_{n=0}^{N-1} \psi_n e^{-ik_p x_n} \;\equiv\;\mathrm{DFT}_p\{\psi_n\}$$
is the [discrete Fourier transform](https://en.wikipedia.org/wiki/Discrete_Fourier_transform)
of the discrete wavefunction.  Next, each term can be evolved forward
in time, using the fact that the quasimomentum states are also energy
eigenstates: each quasimomentum state receives a phase factor
$\exp(-iE_pt/\hbar)$, where $E_p$ is given by
@eq-momentum-energy-1dring.  We thereby obtain the solution at time
$t$:
$$\begin{aligned}|\psi(t)\rangle &= \frac{1}{\sqrt{N}} \sum_{p} \Psi_p
\, e^{-iE_pt/\hbar} \, |k_p\rangle \\ \Rightarrow \langle n
|\psi(t)\rangle &= \frac{1}{N} \sum_{p} \Psi_p \, e^{-iE_pt/\hbar} \, e^{ik_px_n} \equiv \mathrm{IDFT}_n\{ \Psi_p \, e^{-iE_pt/\hbar} \}, \end{aligned}$$
where $\mathrm{IDFT}(\cdots)$ denotes the
[inverse discrete Fourier transform](https://en.wikipedia.org/wiki/Discrete_Fourier_transform).  From this, we see that the quantum walk is a discrete form of [diffraction](https://en.wikipedia.org/wiki/Diffraction): in classical optics, the same “Fourier transform, propagate, then inverse Fourier transform” sequence is the standard way to calculate diffraction patterns.

## Wavefunctions {#sec-wavefunctions}

Wavefunction are quantum states represented by complex functions of
continuous real variables:
$$|\psi\rangle \;\; \leftrightarrow \;\; \psi(x), \;x\in\mathbb{R}$$
For simplicity, let us focus on wavefunctions of a single real
variable, $x\in\mathbb{R}$, rather than multiple variables like
$\mathbf{r} \in \mathbb{R}^3$; also, we assume the functions are
defined over the entire real line, rather than a half-space or finite
segment.

Complex functions obey the usual axioms for vector spaces, since they
can form linear combinations
$$c_1 |\psi_1\rangle + c_2 |\psi_2\rangle \;\;\leftrightarrow\;\; c_1 \psi_1(x) + c_2 \psi_2(x), \;\;\; c_1, c_2 \in \mathbb{C}.$$
However, the quantum axioms (@sec-states-and-observables) demand that
the state space is not merely a vector space but a *Hilbert space*,
meaning it must also have a valid inner product and be complete.  To
meet these requirements, we restrict ourselves to complex functions
that are **square-integrable**, i.e.,
$$\int_{-\infty}^\infty |\psi(x)|^2 < \infty.$$ {#eq-l2-normalizable}
We will say more later about what functions satisfy this condition,
but an example of a function that does *not* is the plane wave
$\psi(x) = \exp(ikx)$, since
$$\int_{-\infty}^\infty |e^{ikx}|^2 dx = \int_{-\infty}^\infty dx \rightarrow \infty.$$
For functions that *do* satisfy @eq-l2-normalizable, we can define
$$\langle \psi_1 | \psi_2 \rangle \equiv \int_{-\infty}^\infty dx \;\; \psi_1^*(x) \psi_2(x),$$
whereupon it can be shown that this satisfies the inner product
axioms; thereafter, a mathematical result called the [Riesz–Fischer theorem](https://en.wikipedia.org/wiki/Riesz%E2%80%93Fischer_theorem)
assures us that the vector space is complete.  Thus, square-integrable
complex functions form a complex Hilbert space.
...
$|\mathbf{k}\rangle$'s as the basis:
...
1. In @sec-states-and-observables, we noted that the Schrödinger
   equation is norm-preserving (given that the Hamiltonian is
   Hermitian): i.e., any solution $|\psi(t)\rangle$ satisfies
   $\frac{d}{dt}\langle\psi(t)|\psi(t)\rangle = 0$.  In
   @sec-time-evolution, we further defined the time evolution operator
   $\hat{U}$: if $|\psi(t)\rangle$ is a solution to the Schrödinger
   equation, then
   $$|\psi(t)\rangle = \hat{U}(t,t')\,|\psi(t')\rangle$$
   for any $t,t'$.  Prove that $\hat{U}(t,t')$ is a unitary operator.

2. In this exercise, we work through a line of reasoning that yields
...
4. Solve the Schr\"odinger equation @eq-static-field-hamiltonian:
   $$i \hbar \frac{d}{dt} |\psi_R(t)\rangle
= \left(E_0 + \frac{\hbar \delta\omega}{2} \hat{\sigma}_3 - \frac{\hbar\Omega_d}{2} \,\hat{\sigma}_1 \right) |\psi_R(t)\rangle, \quad \delta\omega \equiv \omega_d - \Delta \omega.$$
   Then derive Rabi oscillation.

5. (Quantum walk) Derive the group velocity of a wavepacket in the lattice.

6. Probability conservation on the lattice

  Site probabilities $P_n(t)=|\psi_n(t)|^2$ sum to one:
  $\sum_n P_n=1$ at all $t$. With the hopping Hamiltonian, probability flows between neighboring
...
7. In @sec-momentum-eigenstates, we derived the eigenstates of a
...
8. (Time evolution) For a two-level system with
