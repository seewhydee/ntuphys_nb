...
$$i\hbar\frac{d}{dt}\left(\hat{U}_R(\omega_d t)\,|\psi_R(t)\rangle\right) 
= i\hbar \left(\frac{d\hat{U}_3(\omega_d t)}{dt} + \hat{U}_3(\omega_d t) \frac{d}{dt}\right) |\psi_R(t)\rangle = \hat{H}(t) \hat{U}_3(\omega_d t) |\psi_R(t)\rangle.$$
...
This phenomenon is called **Rabi oscillation**.  The full derivation
is worked through in [Exercise 4](#sec-basis-exercises).
...
$n$; unless explicitly stated, such sums are always implicitly assumed
to run from $0$ to $N-1$.  The variable $a$ is a length unit we have
introduced to represent the equal spacings between lattice sites.
Strictly speaking, we could take $a = 1$, but it is nice to retain it
so that the formulas are dimensionally consistent; thus, the position
eigenvalues are $x_n \equiv n a$, with explicit units of length.
...
Therefore, according to the Schrödinger equation @eq-schrodinger, the
particle's state after time $\delta t$ will be
...
right of the initial site $n_0$.  As $t$ further increases, the
particle continues hopping to sites further away, in a process known
as a **quantum walk**.
...
@sec-lattice-momentum, we obtain
...
$$E_p = - 2J \, \cos(k_p a), \;\;\; \mathrm{where}\;\;k_p = \frac{2\pi p}{Na}.$$
...
where $\{\psi_n\}$ is a discrete wavefunction.  We can decompose this
state into the quasimomentum basis @eq-discrete-k-state:
...
eigenstates.  We thereby obtain the solution at time $t$:
