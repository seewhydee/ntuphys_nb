...
1. In this exercise, we work through a line of reasoning that gives
   ...
   characterized by three real parameters.  We seek a linear
   decomposition
   $$\alpha_1 \hat{\sigma}_1 + \alpha_2 \hat{\sigma}_2 + \alpha_3 \hat{\sigma}_3,\;\; \alpha_i \in \mathbb{R},$$
   where each $\hat{\sigma}_i$ is a traceless Hermitian matrix.
    ...
    Note that this implies $\hat{\sigma}_i$ is a unitary (not just
    Hermitian) matrix with determinant $-1$.
    ...
    Then using the results of part A, prove that
