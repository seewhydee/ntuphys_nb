...
Let the position eigenstates form a complete orthonormal basis
...
We can then also use them to construct a position
observable,
...
to keep it around so that our formulas are dimensionally consistent.)
Thus, the position eigenvalues are $\{ x_1, x_2, \dots, x_N\}$.
...
phase factor $e^{-ik_{n} qa}$ (and this holds for every $q$ and $n$).
...
of momentum (note that $k_n$ has units of inverse length).  The
momentum eigenvalues are thus $\{\hbar k_1, \dots, \hbar k_N\}$.
However, as noted above, $k_n$ is actually defined modulo $2\pi/a$, so
the momentum is likewise defined modulo $2\pi\hbar/a$.  (In
condensed-matter physics, this sort of periodic momentum is sometimes
called **crystal momentum**, to distinguish it from the non-periodic
momentum observables we will see in @sec-wavefunctions.)
