...
Thus, the number of sites is some finite integer $N$.  This allows us
to postpone the mathematical complications that come with taking the
limit $N\rightarrow \infty$.

### Position eigenstates

The sites of the 1D lattice, which lie along a ring, can be labeled by
an integer $n \in [1, N]$.  We wish to consider a *single* particle
existing within the lattice.  Let us define a set of **position
eigenstates**,

$$\big\{|1\rangle,\; |2\rangle,\; \dots |N\rangle \big\},$$

with $|n\rangle$ meaning that the particle is located precisely on
site $n$.  These eigenstates are normalized to unity and mutually
orthogonal:

$$\langle m|n\rangle = \delta_{mn}, \;\;\; m,n\in [1,N].$$

We assume the position eigenstates form a complete orthonormal basis
for the particle's Hilbert space, $\mathscr{H} = \mathbb{C}^N$.
<!-- Thus, we can use them to resolve the identity: -->
<!-- $$\hat{I} = \sum_{n=1}^N |n\rangle\langle n|.$$ -->
We can also use them to explicitly construct a position
observable
...
lattice sites.  (Strictly speaking, we could just set $a = 1$, but it
is convenient to keep it around for dimensional analysis.)  Thus, the
position eigenvalues are $\{ x_1, x_2, \dots, x_N\}$.
...
*single* particle) that occupies a superposition of different sites.
...
* The inner product between $|\psi\rangle$ and another state
    ...
    $$\langle\varphi|\psi\rangle = \sum_{n=1}^N \varphi^*_n\psi_n.$$

### Momentum eigenstates

Consider the following states on the 1D ring lattice:

$$|k_n\rangle = \frac{1}{\sqrt{N}} \sum_{m=1}^N e^{ik_n x_m} |m\rangle,$$ {#eq-discrete-k-state}

where, as before, $x_m \equiv ma$.  These states' discrete
wavefunctions are plane waves in 1D---i.e., functions of the form
$e^{ikx}$, with $x_m$ playing the role of the position $x$.  Rather
than letting the wavenumber $k$ be arbitrary, we allow only

$$k_n = \frac{2\pi n}{Na}, \;\; n \in [1,N].$$

This ensures that the waves satisfy the periodicity of the ring: e.g.,
putting $m = N+1$ into the wavefunction yields
...
consistent with the requirement that site $N+1$ be equivalent to site
1 on the ring.  Similarly, the wavenumber is defined modulo $2\pi/a$:
i.e., if we increment or decrement $k_n$ by multiples of $2\pi/a$,
@eq-discrete-k-state is left unchanged.

By the orthogonality of [Fourier series](https://en.wikipedia.org/wiki/Fourier_series), 
we can show that:^[*Proof*: Starting from @eq-discrete-k-state,
$$\begin{aligned}\langle k_n | k_{n'} \rangle &= \frac{1}{N} \sum_{m=1}^N \sum_{m'=1}^N e^{-ik_n ma} \, e^{ik_{n'}m' a} \langle m| m'\rangle \\
&= \frac{1}{N} \sum_{m=1}^N e^{-ik_n m a} \, e^{ik_{n'}m a} \\
...
Combining the two cases, we arrive at the orthonormality relations
@eq-k-discrete-ortho.]
...
These states take on a special significance when we consider
translation operators along the 1D ring lattice.  Let us define a
**translation operator** $\hat{T}_q$, where $q \in \mathbb{Z}$, such
that

$$\hat{T}_q |m\rangle = |(m+q)\rangle,$$

with the understanding that the $(m+q)$ on the right-hand side is
taken modulo $N$.  In other words, $\hat{T}_q$ shifts each position
eigenstate by $q$ places along the ring:
...
Evidently, such an operator is norm-preserving (i.e., unitary).  When
it is applied to a $|k_{n}\rangle$ state, we get
...
&= \frac{e^{-ik_{n} q a}}{\sqrt{N}} \sum_{m=1}^N e^{ik_{n} m a} |m\rangle \\
...
result that every $|k_{n}\rangle$ is an eigenstate of every
translation operator.  Consequently, any observable with
$\{|k_{n}\rangle\}$ as its eigenbasis must be conserved under
translations.
...
conserved physical quantity called momentum.  Drawing on this analogy,
we identify identify the $|k_{n}\rangle$ states as **momentum
eigenstates**.

We can use the momentum eigenstates to construct the momentum
observable,

$$\hat{p} = \sum_{n=1}^N |k_{n}\rangle \, \hbar k_n \, \langle k_{n}|.$$

The factor of $\hbar$ is inserted simply so that $\hat{p}$ has units
of momentum (note that $k_n$ has units of inverse length).
