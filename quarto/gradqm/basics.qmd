...
1.  **States**---A system's state is a vector $|\psi\rangle \in \mathscr{H}$, where
    ...
    are well-defined: if we construct a convergent infinite sum of
    vectors from $\mathscr{H}$, the result remains in $\mathscr{H}$.
    By analogy, consider the Leibniz series for $\pi$: $$\pi =
    4\left(1-\frac{1}{3}+\frac{1}{5} - \cdots\right).$$ The series on
    the right can be constructed entirely from rational numbers, but
    the result is irrational.  By contrast, the set of real numbers
    $\mathbb{R}$ is complete---a convergent infinite series of reals
    remains in $\mathbb{R}$.  The Hilbert space concept applies the
    same property to vectors.]  The choice of Hilbert space, including
...
$\mathbb{C}^2$ are represented by $2\times2$ Hermitian matrices, and
it can be shown that any such matrix can be written as a superposition
...
Note that the coefficients in @eq-pauli-superposition are real; were
we to take a *complex* linear combination, the result is not be
guaranteed to be Hermitian.  The identity and Pauli matrices thus form
a basis for the observables of $\mathbb{C}^2$.
