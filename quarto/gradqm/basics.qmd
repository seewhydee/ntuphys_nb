...
For simplicity, let us first focus on the 1D lattice shown on the left.

### Position eigenstates

The sites of the 1D lattice lie along a line, so we can label them by
an integer $n \in \mathbb{Z}$.  We then define a set of **position
eigenstates**,

$$\big\{\dots, |\!-\!2\rangle,\; |\!-\!1\rangle,\; |0\rangle,\; |1\rangle,\; |2\rangle,\; \dots \big\},$$

where $|n\rangle$ means the particle is located precisely on site $n$.
These eigenstates are normalized to unity and mutually orthogonal,
i.e.,

$$\langle m|n\rangle = \delta_{mn}, \;\;\; m,n\in\mathbb{Z}.$$

We assume the position eigenstates form a complete orthonormal basis
for the Hilbert space, $\mathscr{H}$.  It follows that $\mathscr{H}$
is an *infinite-dimensional* space (since the number of basis vectors
is infinite).  We can also use the position eigenstates for the
resolution of the identity, which yields the following infinite sum of
outer products:

$$\hat{I} = \sum_{n\in\mathbb{Z}} |n\rangle\langle n|.$$

Bear in mind that $\mathscr{H}$ serves as the Hilbert space for a
*single* particle, albeit one that can occupy a superposition of
different lattice sites.  Any state $|\psi\rangle \in \mathscr{H}$ can
be decomposed as

$$|\psi\rangle = \sum_{n\in\mathbb{Z}} \psi_n |n\rangle, \;\;\;\psi_n \in \mathbb{C}.$$

In other words, it can be specified by the complex numbers $\psi_n$,
which constitute a **discrete wavefunction** for the particle (this is
a discrete version of the wavefunctions we will discuss in
@sec-wavefunctions).  It follows that:

* The state $|\psi\rangle$ is normalized if and only if
    $$\sum_{n\in\mathbb{Z}} \left|\psi_n\right|^2 = 1.$$
* The inner product between $|\psi\rangle$ and another state
    $|\varphi\rangle$ (with discrete wavefunction $\varphi_n$) is
    $$\langle\varphi|\psi\rangle = \sum_{n\in\mathbb{Z}} \varphi^*_n\psi_n.$$

### Momentum eigenstates

As a further simplification, let us turn the 1D lattice into a ring:

This is equivalent to applying **periodic boundary conditions** on a
finite lattice of $N$ sites (e.g., site $N+1$ is treated as
*equivalent* to site $1$).  The position eigenstates and their various
relationships remain the same as before, except that the $n$ labels
now only run over a finite range.  For instance, the resolution of the
identity is

$$\hat{I} = \sum_{n=1}^N |n\rangle\langle n|.$$

The Hilbert space is now $\mathscr{H} = \mathbb{C}^N$, which is
finite-dimensional.
