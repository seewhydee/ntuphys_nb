...
* Given that each Pauli matrix is traceless and Hermitian, its
  eigenvalues are two real numbers that sum to zero.  We scale the
  matrix so that these eigenvalues are 1 and -1, which implies that:^[*Proof*: let $|+\rangle$ and $|-\rangle$ denote the eigenstates of $\hat{\sigma}_i$, with eigenvalues +1 and -1 respectively.  By the spectral theorem, these eigenstates are orthonormal, and the eigenbasis decomposition of the matrix is$$\hat{\sigma}_i = |+\rangle\langle+| \,-\, |-\rangle\langle-|.$$ It follows that $$\hat{\sigma}_i^2 = |+\rangle\langle+| \,+\, |-\rangle\langle-| = \hat{I}.$$The last step is the "resolution of the identity", i.e., the statement that the identity is equivalent to a sum of projectors taken over an orthonormal basis.]
  $$\hat{\sigma}_i^2 = \hat{I}, \;\; i=1,2,3.$$
  Moreover, products of *different* Pauli matrices satisfy
  $$\begin{aligned}
  ...
  \end{aligned}$$
  This set of algebraic relations is called the Pauli algebra, and is
  an instance of a **Lie algebra**.
  
  set of relations is called the Pauli algebra, 

  Moreover, we choose the three matrices so that they mutually
  anticommute:
  $$\left\{ \hat{\sigma}_i, \hat{\sigma}_j \right\} \equiv
  \hat{\sigma}_i \hat{\sigma}_j + \hat{\sigma}_j \hat{\sigma}_i = 0,
  \;\;\;i\ne j.$$
  The above two relations can be combined into the anticommutation relation
  $$\left\{\hat{\sigma}_i, \hat{\sigma}_j\right\} = 2\delta_{ij}, \;\;\;i,j \in \{1,2,3\}.$$
