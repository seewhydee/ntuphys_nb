...
A wavefunction is a type of quantum state represented by a complex
function of a continuous real variable:
$$|\psi\rangle \;\; \leftrightarrow \;\; \psi(x), \;x\in\mathbb{R}$$
(Wavefunctions can also be defined with respect to multiple variables,
such as $\mathbf{r} \in \mathbb{R}^3$, but we will stick to the
single-variable case for simplicity.  The generalization to the
multi-variable case is straightforward.)  Such functions obey the
usual axioms governing vectors; for example, we can construct linear
combinations
$$c_1 |\psi_1\rangle + c_2 |\psi_2\rangle \;\;\leftrightarrow\;\; c_1 \psi_1(x) + c_2 \psi_2(x),$$
where $c_1, c_2 \in \mathbb{C}$.  We can also constuct an inner
product,
$$\langle \psi_1 | \psi_2 \rangle = \int_{-\infty}^\infty dx \;\; \psi_1^*(x) \psi(2)$$
