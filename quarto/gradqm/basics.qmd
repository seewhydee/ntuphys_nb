...
Here is a brief summary of this topic, which requires jumping ahead to
the concept of wavefunctions (covered in @sec-wavefunctions).  Suppose
we look at a wavefunction $\psi(\mathbf{r})$, where $\mathbf{r}$ is a
position vector, and ask how it transforms under the various
coordinate rotations possible in 3D.  One finds that rotation by an
angle $\phi$ anticlockwise, around each Cartesian axis $i \in
\{1,2,3\}$, is given by a unitary operator of the form

$$\hat{U}_i(\phi) = e^{i\phi\hat{J}_i/\hbar},$$ {#eq-rotation-operator}

where $\hat{J}_1$, $\hat{J}_2$, and $\hat{J}_3$ are so-called
**angular momentum** observables satisfying the commutation relation
...
fundamental, not the wavefunctions (and the specific operators acting
on those wavefunctions) we used in deriving them.
...
where $E_0$ is the average energy of the two states.  Next, suppose
...
$$\hat{U}_3(\theta) = e^{i\theta\,\hat{\sigma}_3/2}.$$
  It can be shown that the Pauli
matrices satisfy the identity
...
Using this, the Schrödinger equation, @eq-schrodinger, can
be re-written as
...
Taking $(d/dt)\hat{U}_3(\omega_d t) = (i\omega_d/2) \,
\hat{\sigma}_3 \, \hat{U}_3(\omega_d t)$, along with
@eq-rabi-rotated-h on the right side, we obtain
...
Thus, the co-rotating frame features a static magnetic field pointing
in the $x$-$z$ plane; the exact direction depends on $\Omega_d$ and
the “frequency detuning” $\delta\omega$.  We will skip the remaining
steps in solving @eq-static-field-hamiltonian, except to note that the
solution can be obtained analytically, and we can then use
@eq-co-rotating-state to retrieve the state in the original
(non-co-rotating) frame.  If the atom is prepared in $|g\rangle$ at
$t=0$, the analytic solution yields
$$P_e(t) = \Big|\langle e|\psi(t)\rangle\Big|^2 \propto \sin^2\! \left(\frac{\Omega_R\,t}{2}\right).$$
Thus, the occupation probability for the ground state varies
sinusoidally.  This is called **Rabi oscillation**.
...
manipulated by radio-frequency pulses, which function as the driving
field in the model.  By adjusting the pulse duration and/or the
qubit's $\delta\omega$ and $\Omega_R$ parameters, it is possible to
rotate the spin in various directions, corresponding to various kinds
...
We can use the position eigenstates to construct a position
observable,

$$\begin{aligned}\hat{x} &\equiv \sum_{n=1}^N |n\rangle \,x_n\, \langle n|, \;\;\;\mathrm{where}\;\;x_n \equiv na. \\
...
The time-evolution of a particle on a lattice is typically governed by
a “hopping” Hamiltonian that describes how the particle transitions to
neighboring sites.  For instance, on a 1D lattice, take
$$\hat{H} = - J \sum_{n = 1}^N \big(|n\rangle \, \langle n+1| \, + \, |n+1\rangle \, \langle n| \big),$$ {#eq-hamiltonian-1dring}
where $J$ is some energy scale (the minus sign is customary), and
$``\pm 1''$ is taken modulo $N$ to account for the lattice being a
ring: i.e., $|0\rangle \equiv |N\rangle$.

We can understand this Hamiltonian by considering how it would act on
a particle initialized in a position eigenstate $|n'\rangle$ at time
$t = 0$:

* The first term in the parentheses, acting on $|n'\rangle$, gives
  $-J|n'+1\rangle$, coming from the term with $n+1=n'$; all other
  terms (with $n+1\ne n'$) vanish.
...
* The second term in the parentheses is the Hermitian conjugate of the
  ...
  $|n'\rangle$, it yields $-J|n'- 1\rangle$ (from the $n = n'$ term).

Thus, when the particle evolves under the Schrödinger equation
@eq-schrodinger, its state after time $\delta t$ is
$$\begin{aligned}|\psi(\delta t)\rangle &= |n'\rangle + \frac{\delta t}{i\hbar} \hat{H} |\psi(0)\rangle + O(\delta t^2) \\ &= |n'\rangle - \frac{J \delta t}{i\hbar}\big(|n'+1\rangle + |n'-1\rangle\big) + O(\delta t^2).\end{aligned}$$
This leaves the particle in a superposition of different sites, with
part of its quantum state involving “hops” to the left and right of
the initial site $n'$.  As $t$ further increases, the particle
continues “hopping” to sites further from $n'$.  Such a process is
called a **quantum walk**.

In fact, we can solve the Schrödinger equation by making use of the fact that the Hamiltonian is translationally symmetric.  Applying @eq-hamiltonian-1dring  to the momentum eigenstates from @sec-lattice-momentum gives
$$\begin{aligned}\hat{H} |k_n\rangle &= - \frac{J}{\sqrt{N}} \sum_{m,q=1}^N e^{ik_n x_m} \big(|q\rangle \, \langle q+1 | m\rangle  + |q+1\rangle \, \langle q | m\rangle \big) \\ &= - \frac{J}{\sqrt{N}} \sum_{q=1}^N \big( e^{ik_n x_{q+1}} + e^{ik_n x_{q-1}} \big) |q\rangle \\ &= - 2J \, \cos(k_n a) \, |k_n\rangle.
\end{aligned}$$
Hence, each momentum eigenstate is simultaneously an energy eigenstate, with eigenenergy
$$E_n = - 2J \, \cos(k_n a), \;\;\; k_n = \frac{2\pi n}{Na}.$$
...
3. In @sec-momentum-eigenstates, we derived the eigenstates of a
...
4. (Time evolution) For a two-level system with
