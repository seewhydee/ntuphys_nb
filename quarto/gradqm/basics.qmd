...
etc.).  Quantum theory, however, treats them fundamentally differently.
...
1.  **States**---A system's state is a vector $|\psi\rangle \in \mathscr{H}$, where
    ...
    It is customary to normalize state vectors to unity, by
    scaling them so that
...
2.  **Observables**---An observable is a linear operator, $\hat{A}$,
    acting on state vectors.^[We use hats, $\hat{\cdots}$, to
    denote operators---both Hermitian operators (observables) and
    non-Hermitian ones.] This operator is Hermitian, or self-adjoint:
    $\hat{A} = \hat{A}^{\dagger}$.  Its eigenvalues are the
    observable's possible measurement results.  The corresponding
...
3.  **The Schrödinger equation**---A system's state,
    $|\psi(t)\rangle$, varies with time $t$ according to the vectorial
    differential equation
    $$i\hbar \frac{d}{dt}|\psi(t)\rangle = \hat{H}(t) \, |\psi(t)\rangle,$$
    where $\hat{H}(t)$ is an observable called the **Hamiltonian**
    (which itself may also be time-dependent).  This time-variation is
    ...
    total energy of the system; its eigenvalues are called energy
    eigenvalues, or eigenenergies.
...
4.  **The measurement postulate**---If an observable $\hat{A}$ is
    ...
    After the measurement, the system's state abruptly switches, or
    "collapses", to $|n\rangle$.
    ...
    eigenstate (i.e., the post-collapse phase) is also irrelevant.

From these postulates flow all the predictions of quantum mechanics,
which are possibly the most successful set of scientific results of
all time.  Highly-accurate quantum mechanical calculations form the
basis for our modern understanding of chemistry, materials science,
nuclear physics, and many other fields, and are crucial of the
functioning of many pieces of modern technology that we rely on
everyday---lasers, LEDs, computers, and more.
...
* In postulate 3, the Schrödinger equation gives a privileged role to
  ...
  quantum field theories.  The second, however, is a fundamental
  unsolved problem.  This theoretical difficulty is called the
...
to develop some important elementary models: two-level systems,
particles on lattices, and wavefunctions.
...
Note that the identity and Pauli matrices are Hermitian (i.e.,
observables), and linearly independent (i.e., each cannot be written
as a linear combination of the others).  Thus, @eq-pauli-superposition
is essentially using these matrices as a linear basis for constructing
all possible observables for $\mathbb{C}^2$.  The four coefficients
$\alpha_0, \dots, \alpha_3$ must be real; were we to use a complex
linear combination, $\hat{A}$ would not be guaranteed to be Hermitian.

@eq-pauli-superposition is not the only possible parameterization for
observables of $\mathbb{C}^2$, but it has some particularly nice
properties:
...
* The leading term, $\alpha_0\hat{I}$, represents the "trivial" part
  ...
  as $\hat{A}$, so the remaining part of the matrix (i) is traceless,
  and (ii) satisfies a simplified eigenvalue equation:
  ...
  In other words, only the Pauli matrix terms determine the
  eigenstates of $\hat{A}$, while the $\alpha_0$ part merely shifts
  the eigenvalues (e.g., shifting the zero of energy for a
  Hamiltonian).

* The Pauli matrices are traceless, Hermitian, and satisfy the
  following anticommutation and commutation relations:
  $$\begin{aligned}
  ...
  Here, $\epsilon_{ijk}$ denotes the Levi-Civita symbol.  These
  algebraic relations are no accident: they can be derived from a
  minimal set of reasonable assumptions about how to form an
  "orthonormal basis" of $2\times2$ traceless Hermitian matrices, as
  shown in [Exercise 1](#sec-basics-exercises).  The Pauli matrices
  then arise as a particular convenient representation for the
  algebra.  Often, when manipulating formulas containing Pauli
  matrices, it is advantageous to use @eq-Lie-commutators instead of
  working with the explicit matrices.

### Spin mapping

The commutation relation in the second line of @eq-Lie-commutators
defines a so-called **Lie algebra**, $\mathfrak{su}(2)$.  Notably,
this Lie algebra also appears in the quantum theory of angular
momentum in 3D space.  If $\hat{J}_i$ denotes the angular momentum
operator in direction $i \in \{1,2,3\}$, one can show that

$$\left[\hat{J}_i, \hat{J}_j\right] = i\hbar \sum_{k=1}^3 \epsilon_{ijk} \, \hat{J}_k.$$ {#eq-Lie-algebra}

This can be derived for wavefunctions (@sec-wavefunctions), by using
position and momentum operators to construct angular momentum
operators, which describe *orbital* angular momentum.  More generally,
...
particles can carry (including point particles like electrons, which
...
Due to the $\hbar/2$ prefactor, the Pauli matrices describe a
so-called "spin-1/2 degree of freedom".^[By convention, spin operators
are denoted specifically by $\hat{S}_i$.]  In this context, the basis
expansion for a general observables (@eq-pauli-superposition)
evidently has the same form as the Hamiltonian for a magnetic dipole
in a magnetic field $\mathbf{B}$:

$$\hat{A} = \alpha_0 + \sum_{i=1}^3 \alpha_i \hat{\sigma}_i \;\;\;\leftrightarrow\;\;\;
\hat{H} = E_0 - \lambda \sum_{i=1}^3 B_i \hat{S}_i = E_0 - \lambda \, \mathbf{B} \cdot \hat{\mathbf{S}}.$$

Here, $E_0$ is a baseline energy, and $\lambda$ is defined so that
$\lambda \hat{\mathbf{S}}$ represents the magnetic dipole moment
(which follows the spin).  The real coefficients in
@eq-pauli-superposition are thus imbued with concrete physical
meanings; in particular, we see that the real coefficients $\alpha_1,
\dots, \alpha_3$ represent the components of a magnetic field vector.

It is also common to use this mapping to interpret two-level quantum
systems in terms of spin-1/2 degrees of freedom *even if they're not
spins*.  For example, consider an atom with a ground state $|g\rangle$
...
+ \left(E_0 + \frac{\hbar\Delta \omega}{2}\right) |e\rangle\langle e| + \frac{\hbar \Omega_L}{2} \Big(e^{-i\omega_L t} |g\rangle\langle e| + e^{i\omega_L t} |e\rangle\langle g|\Big),$$

where $E_0$ is a baseline energy, $\Omega_L$ is the so-called Rabi
frequency (a frequency that characterizes how strong the driving field
is), and $\omega_L$ is the frequency of the driving field.  Using the
basis $\{|g\rangle, |e\rangle\}$, this Hamiltonian can be rewritten as

$$\hat{H} = E_0 - \frac{\hbar\Delta \omega}{2} \hat{\sigma}_3 + \frac{\hbar\Omega_L}{2}\left[\cos(\omega_L t) \hat{\sigma}_1 + \sin(\omega_L t) \hat{\sigma}_2\right].$$

If we redefine the Pauli matrices as spin via @eq-spin-map, this
becomes

$$\hat{H} = E_0 - \lambda \mathbf{B} \cdot \hat{\mathbf{S}}, \;\;\;
\lambda \mathbf{B} = \begin{pmatrix}\Omega_L\cos(\omega_Lt)\\ \Omega_L\sin(\omega_Lt) \\ \Delta \omega\end{pmatrix}.$$

In other words, the illuminated two-level atom has the same dynamics
as a spin subjected to a static magnetic field along $z$ and a
rotating magnetic field in the $x$-$y$ plane.

## Lattices

Lattice models are another useful class of quantum models, which allow
us to develop the concept of position without the complications
imposed by continuous variables.  Let suppose that a particle is
constrained to discrete points in a 1D or higher-dimensional space,
called a **lattice**:
